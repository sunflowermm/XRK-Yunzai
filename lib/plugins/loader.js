...
class PluginsLoader {
  ...
  async deal(e) {
    try {
      ...
      // 前置检查
      const hasBypass = await this.checkBypassPlugins(e)
      if (!this.preCheck(e, hasBypass)) return
  ...
  preCheck(e, hasBypassPlugin = false) {
    ...
    if (msg === '#开机') {
      redis.del(`Yz:shutdown:${botUin}`)
    } else if (redis.get(`Yz:shutdown:${botUin}`) === 'true') {
      return false
    }
