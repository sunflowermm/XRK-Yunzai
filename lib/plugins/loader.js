...
import { PluginBridge } from '../multiplugin/PluginBridge.js'
...
/**
 * 插件加载器
 * 管理JavaScript和Python插件的加载、执行和生命周期
 */
class PluginsLoader {
  constructor() {
    /** @type {Array} 普通优先级插件 */
    ...
    /** @type {PluginBridge} 多语言插件桥接器 */
    this.pluginBridge = null
    /** @type {Map<string, Object>} 语言特定插件映射 */
  ...
  async initMultiLanguage() {
    try {
      logger.info('初始化多语言插件系统...')
      
      // 创建插件桥接器
      this.pluginBridge = new PluginBridge({
        pluginsDir: path.join(process.cwd(), this.dir),
        languages: ['python'],
        pythonPath: cfg.python?.path || 'python3'
      })
      
      // 注册事件监听
      this.pluginBridge.on('plugin:loaded', (event) => {
        this.registerLanguagePlugin(event)
      })
      
      this.pluginBridge.on('runtime:error', (event) => {
        logger.error(`[${event.language}] 运行时错误: ${event.error}`)
      })
      
      this.pluginBridge.on('plugin:log', (event) => {
        logger[event.level || 'info'](`[${event.language}] ${event.message}`)
      })
      
      // 启动桥接器
      await this.pluginBridge.start()
      
      logger.success('多语言插件系统初始化完成')
    } catch (error) {
      logger.error(`多语言系统初始化失败: ${error.message}`)
    }
  }

  /**
   * 注册语言特定插件
   * @param {Object} event - 插件事件
   */
  registerLanguagePlugin(event) {
    const { language, plugin: pluginInfo } = event
    
    logger.debug(`注册${language}插件: [${pluginInfo.key}][${pluginInfo.name}]`)
    
    // 创建插件包装类
    const PluginWrapper = this.createPluginWrapper(language, pluginInfo)
    ...
    const pluginEntry = {
      ...
      priority: pluginInfo.priority === 'extended' ? 0 : (pluginInfo.priority ?? 50),
      plugin: new PluginWrapper(),
      language,
      bypassThrottle: pluginInfo.bypassThrottle === true
    }
    
    // 添加到对应列表
    const targetArray = pluginInfo.priority === 'extended' ? this.extended : this.priority
    targetArray.push(pluginEntry)
    
    // 保存到语言映射
    const langPlugins = this.languagePlugins.get(language) || new Map()
    langPlugins.set(pluginInfo.key, pluginEntry)
    this.languagePlugins.set(language, langPlugins)
    
    this.pluginCount++
    
    // 处理定时任务
    if (pluginInfo.task?.length > 0) {
      this.registerTasks(pluginInfo, language)
    }
  }
   ...
   */
  createPluginWrapper(language, pluginInfo) {
    const bridge = this.pluginBridge
    const { key, name, priority, rule } = pluginInfo
    
    class LanguagePluginWrapper {
      constructor(e) {
        this.e = e
        this.name = name
        this.key = key
      ...
      async _callRemote(method, ...args) {
        try {
          const result = await bridge.callPlugin(language, key, method, {
            event: this.e ? this._serializeEvent(this.e) : null,
            args
          })
          ...
          switch (action) {
            case 'sendMsg':
              if (data.group_id && this.e.bot) {
                await this.e.bot.pickGroup(data.group_id).sendMsg(data.message)
                return true
              }
              if (data.user_id && this.e.bot) {
                await this.e.bot.pickFriend(data.user_id).sendMsg(data.message)
                return true
              }
              break
            ...
            case 'uploadFile':
              if (data.file && this.e.reply) {
                await this.e.reply(segment.file(data.file, data.name))
                return true
              }
              break
            ...
            default:
              logger.debug(`未知的插件动作: ${action}`)
          }
        } catch (error) {
          logger.error(`处理插件动作失败: ${error.message}`)
        }
        return false
      }
    ...
    if (rule && Array.isArray(rule)) {
      rule.forEach(r => {
        if (r.fnc && !LanguagePluginWrapper.prototype[r.fnc]) {
          LanguagePluginWrapper.prototype[r.fnc] = async function(e) {
            this.e = e
            return await this._callRemote(r.fnc)
          }
        }
        
        // 编译正则
        if (r.reg) {
          r.reg = this.createRegExp(r.reg)
        }
      })
    }
    
    return LanguagePluginWrapper
  }
   ...
   * @param {string} language - 插件语言
   */
  registerTasks(pluginInfo, language) {
    const { task, name, key } = pluginInfo
    ...
    task.forEach(t => {
      if (t.cron && t.fnc) {
        this.task.push({
          ...
          fnc: async () => {
            try {
              const result = await this.pluginBridge.callPlugin(language, key, t.fnc, {})
              if (result?.message) {
                logger.info(`[${language}定时任务] ${result.message}`)
              }
            } catch (error) {
              logger.error(`${language}定时任务执行失败 [${key}][${t.fnc}]: ${error.message}`)
            }
          },
          log: t.log !== false
        })
  ...
  async load(isRefresh = false) {
    ...
    // 统计插件数量
    const jsCount = this.pluginCount - (this.languagePlugins.get('python')?.size || 0)
    const pyCount = this.languagePlugins.get('python')?.size || 0

    logger.success(`加载定时任务[${this.task.length}个]`)
    logger.success(`加载JavaScript插件[${jsCount}个]`)
    if (pyCount > 0) {
      logger.success(`加载Python插件[${pyCount}个]`)
    }
