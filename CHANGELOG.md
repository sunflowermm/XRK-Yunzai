...
# 3.2.0（当前版本）
* **Agent 平台底层精简**：移除 ASR/TTS 工厂、火山 ASR/TTS 配置与 CommonConfig 项；删除 `workflow/device.js` 设备语音专用工作流
...
* **全局裸名规范**：业务层禁止 `global.Bot/cfg/segment`；启动挂载 `globalThis`；`lib/utils/ai-workflow-config.js` 自 AGT 复制
