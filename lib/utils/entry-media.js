...
async function readLocalBuffer(ref) {
  const inline = FileUtils.inlineBinaryFromRef(ref);
  if (inline) return inline;
  const p = String(ref ?? '').replace(/^file:\/\//, '').trim();
  if (!p || isHttpRef(p) || !FileUtils.isPathLike(p)) return null;
  if (!(await FileUtils.exists(p))) return null;
  const buf = await FileUtils.readFileBuffer(p);
  return buf?.length ? buf : null;
}

async function fetchRefBuffer(ref, timeoutMs, opts = {}) {
  if (!ref || !isHttpRef(ref)) return null;
  const fetched = await BotUtil.Buffer(ref, { http: false, timeout: timeoutMs, quiet: opts.quiet });
...
async function getImageViaApi(sendApi, fileRef, timeoutMs) {
  ...
  try {
    ...
    const d = timeoutMs > 0
      ...
      : await api;
    if (d.file && FileUtils.isPathLike(d.file) && await FileUtils.exists(d.file)) {
...
export async function readImageBuffer(refs, sendApi, opts = {}) {
  ...
  const getImageTimeout = opts.getImageTimeout ?? 60000;
  ...
  // OneBot/NapCat get_image 仅接受 file 标识（hash/本地路径），不接受 HTTP URL
  if (sendApi) {
    for (const ref of [file, url]) {
      if (!ref || isHttpRef(ref)) continue;
  ...
  for (const ref of [url, file]) {
    if (!isHttpRef(ref) || isQqCdn(ref)) continue;
...
export async function persistEntryMedia(segment, { baseDir, groupId, sendApi }) {
  ...
  for (const ref of [fileRef, urlRef]) {
    ...
    if (isEntryMediaRelPath(ref)) {
      ...
      if (await FileUtils.exists(existing)) return ref;
    }
    if (FileUtils.isPathLike(ref) && await FileUtils.exists(ref)) {
  ...
  const buffer = await readImageBuffer(data, sendApi);
  ...
  await FileUtils.ensureDir(path.dirname(file.path));
