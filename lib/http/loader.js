...
class ApiLoader {
  ...
  async _getApiDirs() {
    ...
    // 1. 默认API目录
    const apiDir = path.join(cwd, 'plugins/api');
    if (!FileUtils.existsSync(apiDir)) {
      await FileUtils.ensureDir(apiDir);
    }
    dirs.push(apiDir);
    
    // 2. 从 plugins 下的每个子目录加载 http（每个插件可以有独立的 HTTP API）
      ...
      try {
        ...
        for (const entry of entries) {
          if (entry.isDirectory() && !entry.name.startsWith('.')) {
            const httpDir = path.join(pluginsDir, entry.name, 'http');
            if (FileUtils.existsSync(httpDir)) {
              dirs.push(httpDir);
            }
          }
        }
    ...
    if (FileUtils.existsSync(coreDir)) {
      try {
        ...
        for (const entry of entries) {
          if (entry.isDirectory() && !entry.name.startsWith('.')) {
            const httpDir = path.join(coreDir, entry.name, 'http');
            if (FileUtils.existsSync(httpDir)) {
              dirs.push(httpDir);
            }
          }
  ...
  async getApiKey(filePath) {
    ...
    // 尝试从 plugins/api 目录计算相对路径
    const pluginsApiDir = path.join(cwd, 'plugins/api');
    if (normalizedPath.startsWith(pluginsApiDir)) {
      const relativePath = path.relative(pluginsApiDir, normalizedPath);
      return relativePath.replace(/\\/g, '/').replace(/\.js$/, '');
    }
    
    // 尝试从 core 目录计算相对路径
    const coreDir = path.join(cwd, 'core');
    if (normalizedPath.startsWith(coreDir)) {
      const relativePath = path.relative(coreDir, normalizedPath);
      return relativePath.replace(/\\/g, '/').replace(/\.js$/, '');
    }
    
    // 如果找不到对应的目录，使用文件名作为 key
  ...
  sortByPriority() {
    this.priority = Array.from(this.apis.values())
      ...
      .sort((a, b) => {
        if (!ObjectUtils.isPlainObject(a) || !ObjectUtils.isPlainObject(b)) {
          return 0;
        }
        return getApiPriority(b) - getApiPriority(a); // 降序：优先级高的在前
      });
  }
