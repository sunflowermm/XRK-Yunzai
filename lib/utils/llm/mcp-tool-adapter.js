 ...
 * MCP 工具适配器
 *
 * 将 AiWorkflowLoader.mcpServer 上的工具转换为 OpenAI tools 格式，并处理 tool_calls 调用。
 */
...
export class MCPToolAdapter {
  ...
  /**
   * 是否为用户可见的对外发送工具（reply / at / 表情图等）
   */
  static USER_VISIBLE_SEND_TOOLS = new Set([
    'reply', 'emotion', 'send_file', 'send_image', 'poke',
    'relayPrivate', 'relayPrivateImage', 'relayPrivateEmotion', 'relayPrivateFile',
    'thumbUp', 'sign', 'emojiReaction', 'forgeForward'
  ]);

  /** 私聊传话类：与同轮 reply 等并行时须先执行，失败则跳过其余对外发送 */
  static RELAY_PRIVATE_TOOLS = new Set([
    'relayPrivate', 'relayPrivateImage', 'relayPrivateEmotion', 'relayPrivateFile'
  ]);

  /** MCP 名或 OpenAI 合法名（chat_emotion）→ 工具基名（emotion） */
  static _mcpToolBaseName(name) {
    ...
    if (parts.length >= 2) {
      ...
      if (MCPToolAdapter.USER_VISIBLE_SEND_TOOLS.has(tail2)) return tail2;
    }
    return parts[parts.length - 1] || s;
  }

  static isUserVisibleSendToolName(name) {
    const resolved = this.resolveOpenAiToolNameToMcp(String(name ?? ''));
    const mcp = String(resolved || name || '').trim();
    if (!mcp) return false;
    return MCPToolAdapter.USER_VISIBLE_SEND_TOOLS.has(MCPToolAdapter._mcpToolBaseName(mcp));
  }

  static _toolCallBaseName(toolCall) {
    const openAiName = toolCall?.function?.name;
    return MCPToolAdapter._mcpToolBaseName(MCPToolAdapter.resolveOpenAiToolNameToMcp(openAiName));
  }

  static _shouldSerializeRelayWithSend(toolCalls) {
    const list = Array.isArray(toolCalls) ? toolCalls : [];
    if (list.length < 2) return false;
    const bases = list.map((tc) => MCPToolAdapter._toolCallBaseName(tc));
    const hasRelay = bases.some((b) => MCPToolAdapter.RELAY_PRIVATE_TOOLS.has(b));
    const hasOtherVisible = bases.some(
      (b) => MCPToolAdapter.USER_VISIBLE_SEND_TOOLS.has(b) && !MCPToolAdapter.RELAY_PRIVATE_TOOLS.has(b)
    );
    return hasRelay && hasOtherVisible;
  }

  static _orderRelayPrivateFirst(toolCalls) {
    const relay = [];
    const other = [];
    for (const tc of toolCalls) {
      const base = MCPToolAdapter._toolCallBaseName(tc);
      if (MCPToolAdapter.RELAY_PRIVATE_TOOLS.has(base)) relay.push(tc);
      else other.push(tc);
    }
    return [...relay, ...other];
  }

  static _toolResultFailed(content) {
    if (typeof content !== 'string' || !content) return false;
    if (/私聊未发出|不是机器人好友|不是好友|请先添加对方为好友|success"\s*:\s*false/i.test(content)) {
      return true;
    }
    try {
  ...
  static convertMCPToolsToOpenAI(streamsOrOptions = []) {
    ...
    Bot.makeLog('debug', `[MCPToolAdapter] 工具列表 streams=[${streamsKey}] 工具数=${mcpTools.length} names=${toolNames.slice(0, 120)}`, 'MCPToolAdapter');
  ...
  static async handleToolCalls(toolCalls, options = {}) {
    ...
    const relayMixed = MCPToolAdapter._shouldSerializeRelayWithSend(list);
    const sequential = parallel === false || options.sequentialToolCalls === true || relayMixed;
    const ordered = relayMixed ? MCPToolAdapter._orderRelayPrivateFirst(list) : list;
    ...
    if (sequential) {
      ...
      for (let i = 0; i < ordered.length; i++) {
        ...
        if (
          relayFailed &&
          MCPToolAdapter.USER_VISIBLE_SEND_TOOLS.has(base) &&
          !MCPToolAdapter.RELAY_PRIVATE_TOOLS.has(base)
        ) {
          results.push({
            role: 'tool',
            tool_call_id: tc.id,
            content: '私聊传话未成功，本轮已跳过 reply/emotion 等对外发送。请下一轮向用户说明失败原因，勿声称已私聊发出。'
          });
          continue;
        }
        const row = await runOne(tc);
        results.push(row);
        if (MCPToolAdapter.RELAY_PRIVATE_TOOLS.has(base) && MCPToolAdapter._toolResultFailed(row.content)) {
          relayFailed = true;
        }
      }
      return results;
    ...
    return Promise.all(list.map(runOne));
  }
