...
export class FileUtils {
  ...
  static _logFsDebug(op, filePath, err) {
    try {
      Bot.makeLog('debug', `[FileUtils] ${op} 失败: ${filePath} | ${err?.message || err}`, 'FileUtils');
    } catch {
      // noop
    }
  }
  ...
  static readDirSync(dirPath, options = {}) {
    try {
      ...
      FileUtils._logFsDebug('readDirSync', dirPath, err);
      return [];
    }
  }
  ...
  static async readDir(dirPath, options = {}) {
    try {
      ...
      FileUtils._logFsDebug('readDir', dirPath, err);
      return [];
    }
  }
