...
export default class Bot extends EventEmitter {
  ...
  _setupProxyMiddleware() {
    // 使用 cfg.server.proxy 而不是 cfg.proxy
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async _displayProxyInfo() {
    ...
    for (const domainConfig of domains) {
      ...
      console.log(`    ${chalk.cyan('•')} 访问地址: ${chalk.white(`${protocol}://${domainConfig.domain}${port}`)}`);
...
<VirtualHost *:443>
  ...
  async httpsLoad() {
    ...
    try {
      ...
      if (proxyConfig?.enabled && proxyConfig?.domains) {
        for (const domainConfig of proxyConfig.domains) {
          if (domainConfig.ssl?.enabled && domainConfig.ssl?.certificate) {
            ...
            this.domainCertificates.set(domainConfig.domain, certData);
            BotUtil.makeLog("info", 
      ...
      // SNI回调
      if (this.domainCertificates.size > 0) {
        httpsOptions.SNICallback = (servername, cb) => {
          const certData = this.domainCertificates.get(servername) || 
                          this._findWildcardCertificate(servername);
          
          if (certData) {
            // 使用tls模块创建安全上下文
            const tls = require('tls');
            try {
              const context = tls.createSecureContext(certData);
              cb(null, context);
            } catch (err) {
              BotUtil.makeLog("error", 
                chalk.red(`✗ 创建SSL上下文失败 [${servername}]: ${err.message}`), 
                'Server');
              cb(err);
            }
          } else {
  ...
  _findWildcardCertificate(servername) {
    for (const [domain, certData] of this.domainCertificates) {
      if (domain.startsWith('*.')) {
        const baseDomain = domain.substring(2);
