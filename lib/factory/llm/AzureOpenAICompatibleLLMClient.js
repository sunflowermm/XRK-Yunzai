...
export default class AzureOpenAICompatibleLLMClient {
  ...
  async chat(messages, overrides = {}) {
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      ...
      return packNonStreamReturn(replyTracker, content);
    }
  ...
  async chatStream(messages, onDelta, overrides = {}) {
    ...
    while (round < maxToolRounds) {
      ...
      if (collector.toolCalls.length > 0) {
        ...
        const toolResults = await MCPToolAdapter.handleToolCalls(collector.toolCalls);
        ...
        if (round >= maxToolRounds) {
          Bot.makeLog('warn', `[AzureOpenAICompatibleLLMClient] 达到最大工具调用轮数: ${maxToolRounds}`, 'LLMFactory');
          break;
        }
        continue;
      }
