...
class ApiLoader {
  ...
  async load() {
    ...
    BotUtil.makeLog('mark', '[ApiLoader] 开始加载API模块', 'ApiL.');
    ...
    try {
      ...
      BotUtil.makeLog('info', `[ApiLoader] 加载完成，共${this.apis.size}个API，耗时${loadTime}ms`, 'ApiL.');
      
      return this.apis;
    } catch (error) {
      BotUtil.makeLog('error', '[ApiLoader] 加载失败', 'ApiL.', error);
      throw error;
    }
  }
  ...
  async getApiFiles(dir) {
    ...
    try {
      ...
      BotUtil.makeLog('error', `[ApiLoader] 读取目录失败: ${dir}`, 'ApiL.', error);
    }
  ...
  async loadApi(filePath) {
    try {
      ...
      // 检查是否是有效的API模块
      if (!module.default) {
        BotUtil.makeLog('warn', `[ApiLoader] 无效的API模块: ${key} (缺少default导出)`, 'ApiL.');
        return false;
      }
      ...
      if (typeof module.default === 'function') {
        ...
        try {
          ...
          // 如果不能实例化，可能是普通函数，跳过
          BotUtil.makeLog('warn', `[ApiLoader] 无法实例化API模块: ${key}`, 'ApiL.');
          return false;
        }
        ...
        BotUtil.makeLog('warn', `[ApiLoader] 无效的API模块: ${key} (导出类型错误)`, 'ApiL.');
        return false;
      }
      
      // 验证API实例
      if (!apiInstance || typeof apiInstance !== 'object') {
        BotUtil.makeLog('warn', `[ApiLoader] API实例创建失败: ${key}`, 'ApiL.');
        return false;
      }
      ...
      BotUtil.makeLog('debug', `[ApiLoader] 加载API: ${apiInstance.name || key}`, 'ApiL.');
      return true;
    } catch (error) {
      BotUtil.makeLog('error', `[ApiLoader] 加载API失败: ${filePath}`, 'ApiL.', error);
      return false;
    }
  }
  ...
  async unloadApi(key) {
    ...
    BotUtil.makeLog('debug', `[ApiLoader] 卸载API: ${api.name || key}`, 'ApiL.');
  }
  ...
  async register(app, bot) {
    ...
    BotUtil.makeLog('mark', '[ApiLoader] 开始注册API路由', 'ApiL.');
    ...
    for (const api of this.priority) {
      try {
        ...
        BotUtil.makeLog('info', `[ApiLoader] 注册API: ${apiName} (优先级: ${api.priority || 100})`, 'ApiL.');
      } catch (error) {
        const apiName = api.name || api.key || 'undefined';
        BotUtil.makeLog('error', `[ApiLoader] 注册API失败: ${apiName}`, 'ApiL.', error);
      }
    }
    ...
    BotUtil.makeLog('info', '[ApiLoader] API注册完成', 'ApiL.');
  }
  ...
  async changeApi(key) {
    const api = this.apis.get(key);
    if (!api) {
      BotUtil.makeLog('warn', `[ApiLoader] API不存在: ${key}`, 'ApiL.');
      return false;
    }
    
    try {
      BotUtil.makeLog('info', `[ApiLoader] 重载API: ${api.name || key}`, 'ApiL.');
      ...
      BotUtil.makeLog('info', `[ApiLoader] API重载成功: ${api.name || key}`, 'ApiL.');
      return true;
    } catch (error) {
      BotUtil.makeLog('error', `[ApiLoader] API重载失败: ${api.name || key}`, 'ApiL.', error);
      return false;
    }
  }
  ...
  getApiList() {
    ...
    for (const api of this.apis.values()) {
      ...
      try {
        ...
        BotUtil.makeLog('error', `[ApiLoader] 获取API信息失败: ${api.name || api.key || 'undefined'}`, 'ApiL.', error);
      }
    }
  ...
  async watch(enable = true) {
    if (!enable) {
      ...
      this.watcher = {};
      BotUtil.makeLog('info', '[ApiLoader] 文件监视已停止', 'ApiL.');
      return;
    }
    ...
    try {
      ...
      this.watcher.api
        .on('add', filePath => {
          BotUtil.makeLog('info', `[ApiLoader] 检测到新文件: ${filePath}`, 'ApiL.');
        ...
        .on('change', filePath => {
          const key = path.relative(apiDir, filePath).replace(/\\/g, '/').replace(/\.js$/, '');
          BotUtil.makeLog('info', `[ApiLoader] 检测到文件变更: ${key}`, 'ApiL.');
          this.changeApi(key);
        })
        .on('unlink', filePath => {
          const key = path.relative(apiDir, filePath).replace(/\\/g, '/').replace(/\.js$/, '');
          BotUtil.makeLog('info', `[ApiLoader] 检测到文件删除: ${key}`, 'ApiL.');
          this.unloadApi(key);
          this.sortByPriority();
        });
      
      BotUtil.makeLog('info', '[ApiLoader] 文件监视已启动', 'ApiL.');
    } catch (error) {
      BotUtil.makeLog('error', '[ApiLoader] 启动文件监视失败', 'ApiL.', error);
    }
  }
}
