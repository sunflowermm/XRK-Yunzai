  ...
  async chat(messages, overrides = {}) {
    ...
    for (let i = 0; i < maxRounds; i++) {
      ...
      if (msg.tool_calls?.length > 0) {
        noteReplyFromModelCalls(replyTracker, msg.tool_calls);
        msgs = [...msgs, msg, ...await MCPToolAdapter.handleToolCalls(msg.tool_calls)];
        continue;
      }
      return packNonStreamReturn(replyTracker, msg.content ?? '');
    }
    return packNonStreamReturn(replyTracker, msgs[msgs.length - 1]?.content ?? '');
  }
