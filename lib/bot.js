...
export default class Bot extends EventEmitter {
  ...
  async _initProxyApp() {
    ...
    BotUtil.makeLog('info', '正在初始化反向代理服务...', '代理');
    ...
    this.proxyApp.use(async (req, res, next) => {
      ...
      if (!domainConfig) {
        BotUtil.makeLog('warn', `未配置的域名访问：${hostname}`, '代理');
  ...
  _createProxyMiddleware(domainConfig) {
    const proxyOptions = {
      ...
      secure: false, // [允许自签名证书](https://www.jiangdefu.com/archives/746)
      logLevel: 'warn',
  ...
  _setupStaticServing() {
    ...
    BotUtil.makeLog('info', '⚡ 静态文件服务已启用', '服务器');
  }
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async startProxyServers() {
    ...
    BotUtil.makeLog('info', '⚡ 正在启动代理服务器...', '代理');
