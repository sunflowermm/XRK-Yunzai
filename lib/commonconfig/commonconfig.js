...
import { DEFAULT_CONFIG_DIR, resolveProjectPath } from '../config/config-constants.js';
import { flattenData as flattenConfigData, unflattenData as expandConfigData, mergeConfigLayers, cleanConfigData, unwrapRootArrayFromSchemaShape } from './config-utils.js';
...
export default class ConfigBase {
  ...
  constructor(metadata = {}) {
    ...
    this.defaultFilePath = metadata.defaultFilePath ?? null;
    ...
    if (typeof this.filePath === 'function') {
      ...
      // 完整文件路径
      this.fullPath = resolveProjectPath(this.filePath);
    } else {
      this.fullPath = undefined;
    }
    
    // 缓存：_cache 为合并后的有效配置；_storedCache 仅为 data 层原始内容
    this._cache = null;
    this._cacheTime = 0;
    this._storedCache = null;
    this._storedCacheTime = 0;
    this._cacheTTL = 5000; // 5秒缓存
  }
  
  /**
   * 严格校验 schema：default 与 type/enum 一致；SubForm 必须提供非空 fields；object/array 递归校验
   */
  _assertSchemaStrict(schema) {
    ...
    const check = (fields) => {
      for (const [key, fs] of Object.entries(fields)) {
        ...
        // 校验 enum 与 default（与运行时 validate 同一规则：非必填允许 ''）
        ...
        if (fs.type === 'object') {
          // SubForm 无 fields 时由 config-utils 标 _noFields，前端显示「键值对无需在此编辑」提示，不抛错
          if (fs.fields && Object.keys(fs.fields).length > 0) check(fs.fields);
        }
        if (fs.type === 'array' && fs.itemType === 'object' && fs.itemSchema?.fields) check(fs.itemSchema.fields);
      }
    };
    check(schema.fields);
  }
  ...
  _resolveFilePath() {
    if (this._getFilePath) {
      ...
      return resolveProjectPath(dynamicPath);
    }
  ...
  async exists() {
    ...
    return FileUtils.existsSync(filePath);
  }

  /**
   * 读取 data 层原始配置（不含 default_config 模板与 schema 默认值）
   * 写入合并时使用，避免把模板字段全量落盘到 data。
   * @param {boolean} useCache
   * @returns {Promise<Object>|Object}
   */
  async readStored(useCache = true) {
    if (useCache && this._storedCache != null && (Date.now() - this._storedCacheTime < this._cacheTTL)) {
      return this._storedCache;
    }

    let stored;
    if (this.multiFile) {
      stored = this._readMultiFileLayer('stored');
    } else {
      stored = this._readStoredData();
    }

    this._storedCache = stored;
    this._storedCacheTime = Date.now();
    return stored;
  }

  /**
   * 读取有效配置（default_config → data → schema 默认值 → 类型标准化）
  ...
  async read(useCache = true) {
    ...
    const template = this._readTemplateData();
    const stored = this._readStoredData();
    this._assertConfigSourcesExist(template, stored);

    let data = mergeConfigLayers(template, stored, this.schema);
    data = cleanConfigData(data, this.schema);
  ...
  /**
   * 读取多文件有效配置（各子项：default → data 深合并）
   * @private
   * @returns {Promise<Object>}
   */
  async _readMultiFile() {
    const effective = this._readMultiFileLayer('effective');
    this._cache = effective;
    this._cacheTime = Date.now();
    this._storedCache = this._readMultiFileLayer('stored');
    this._storedCacheTime = Date.now();
    return effective;
  }
   ...
   * 多文件配置按层读取
   * @private
   * @param {'effective'|'stored'} layer
   * @returns {Object}
   */
  _readMultiFileLayer(layer) {
    const { keys, getFilePath, getDefaultFilePath } = this.multiFile;
    ...
    const result = {};
    for (const key of keys) {
      const template = getDefaultFilePath
        ? (this._readParsedFile(getDefaultFilePath(key)) ?? {})
        : {};
      const storedPath = getFilePath(key);
      const stored = this._readParsedFile(storedPath) ?? {};
      ...
      result[key] = ObjectUtils.deepMergeImmutable(
        ObjectUtils.isPlainObject(template) ? template : {},
        ObjectUtils.isPlainObject(stored) ? stored : {}
      );
    }
    return result;
  }

  _getDefaultTemplatePath() {
    if (typeof this.defaultFilePath === 'function') {
      return resolveProjectPath(this.defaultFilePath(cfg));
    }
    if (typeof this.defaultFilePath === 'string' && this.defaultFilePath) {
      return resolveProjectPath(this.defaultFilePath);
    }
    const ext = this.fileType === 'yaml' ? 'yaml' : 'json';
    return resolveProjectPath(DEFAULT_CONFIG_DIR, `${this.name}.${ext}`);
  }

  _parseFileContent(content) {
    if (content == null) return null;
    if (this.fileType === 'yaml') return yaml.parse(content);
    if (this.fileType === 'json') return JSON.parse(content);
    throw new Error(`不支持的文件类型: ${this.fileType}`);
  }

  _readParsedFile(absolutePath) {
    if (!absolutePath || !FileUtils.existsSync(absolutePath)) return null;
    try {
      const content = FileUtils.readFileSync(absolutePath, 'utf8');
      if (content == null) return null;
      return this._parseFileContent(content);
    } catch (error) {
      Bot.makeLog('warn', `读取配置失败 [${this.name}]: ${absolutePath} — ${error.message}`, 'ConfigBase');
      return null;
    }
  }

  _readTemplateData() {
    return this._readParsedFile(this._getDefaultTemplatePath()) ?? {};
  }

  _readStoredData() {
    return this._readParsedFile(this._resolveFilePath()) ?? {};
  }

  _assertConfigSourcesExist(template, stored) {
    const hasContent = (value) => {
      if (value == null) return false;
      if (Array.isArray(value)) return value.length > 0;
      if (ObjectUtils.isPlainObject(value)) return Object.keys(value).length > 0;
      return true;
    };
    if (hasContent(template) || hasContent(stored)) return;

    const dataPath = this._resolveFilePath();
    const templatePath = this._getDefaultTemplatePath();
    if (!FileUtils.existsSync(templatePath) && !FileUtils.existsSync(dataPath)) {
      throw new Error(`配置文件不存在: ${dataPath}`);
    }
  }
  ...
  async write(data, options = {}) {
    ...
    const { backup = true, validate = true, mergeWithExisting = true } = options;

    try {
      // 与 default_config 对齐：写入时合并已有内容，保留 YAML 中未在 schema 声明的字段，避免前端只提交部分字段时丢失其余项
      if (mergeWithExisting && data && typeof data === 'object' && !Array.isArray(data)) {
        let current = {};
        try {
          current = await this.readStored(false);
        } catch {
          // 文件不存在或读失败，用空对象
        }
        if (current && typeof current === 'object' && !Array.isArray(current)) {
          data = ObjectUtils.deepMergeImmutable(current, data);
        }
      }
      ...
      const payload = this.fileType === 'json'
        ? unwrapRootArrayFromSchemaShape(data, this.schema)
        : data;

      // 序列化数据
      let content;
      if (this.fileType === 'yaml') {
        content = yaml.stringify(payload, {
          indent: 2,
          lineWidth: 0,
          minContentWidth: 0
        });
      ...
      // 写入文件
      if (!FileUtils.writeFileSync(filePath, content, 'utf8')) {
        throw new Error(`写入配置失败: ${filePath}`);
      }
      ...
      Bot.makeLog('info', `配置已保存 [${this.name}]`, 'ConfigBase');
      this._emitConfigWrite();
      return true;
    } catch (error) {
      Bot.makeLog('error', `写入配置失败 [${this.name}]: ${error.message}`, 'ConfigBase');
      throw error;
    }
  }
  ...
  async _writeMultiFile(data, options = {}) {
    ...
    try {
      ...
      Bot.makeLog('info', `多文件配置已保存 [${this.name}]`, 'ConfigBase');
      this._emitConfigWrite({ multiFile: true });
      return true;
    } catch (error) {
      Bot.makeLog('error', `写入多文件配置失败 [${this.name}]: ${error.message}`, 'ConfigBase');
      throw error;
    }
  }

  /**
   * 删除该配置文件对应的旧备份，只保留一份（对齐 XRK-AGT，避免备份膨胀）
   * @private
  ...
  async _pruneBackups(filePath) {
    try {
      ...
      const entries = FileUtils.readDirSync(dir) || [];
      for (const name of entries) {
        if (typeof name === 'string' && name.startsWith(prefix)) {
          await FileUtils.unlink(path.join(dir, name));
          Bot.makeLog('debug', `已删除旧备份 [${this.name}]: ${name}`, 'ConfigBase');
        }
      }
    } catch (err) {
      if (err?.code !== 'ENOENT') {
        Bot.makeLog('debug', `清理旧备份时忽略 [${this.name}]: ${err.message}`, 'ConfigBase');
      }
    }
  }

  /**
   * 备份配置文件（新备份前删除同前缀旧备份，仅保留一份）
  ...
  async backup() {
    try {
      ...
      Bot.makeLog('debug', `配置已备份 [${this.name}]: ${backupPath}`, 'ConfigBase');
      return backupPath;
    } catch (error) {
      Bot.makeLog('error', `备份配置失败 [${this.name}]: ${error.message}`, 'ConfigBase');
      throw error;
    }
  }
  ...
  async merge(newData, options = {}) {
    ...
    const mergedData = deep 
      ? ObjectUtils.deepMergeImmutable(currentData, newData)
      : { ...currentData, ...newData };
  ...
  /** 配置落盘后通知运行时（Bot.on('config:write', …)） */
  _emitConfigWrite(extra = {}) {
    try {
      globalThis.Bot?.emit?.('config:write', { name: this.name, ...extra });
    } catch (err) {
      Bot.makeLog('debug', `config:write 派发失败 [${this.name}]: ${err?.message || err}`, 'ConfigBase');
    }
  }
  ...
  flattenData(obj, prefix = '') {
    return flattenConfigData(obj, prefix);
  }

  expandFlatData(flat) {
    return expandConfigData(flat);
  }
  ...
  _normalizeValueBySchema(value, schema = {}) {
    ...
    if (expectedType === 'object' || expectedType === 'map') {
      ...
      // 典型场景：object/json 组件传入 JSON 字符串时尝试解析
  ...
  _checkType(value, expectedType) {
    switch (expectedType) {
      ...
      case 'object':
      case 'map':
        return ObjectUtils.isPlainObject(value);
      default:
        return true;
    }
  }
