...
export default class Bot extends EventEmitter {
  ...
  constructor() {
    ...
    this.bot = this;
    this.bots = {};
    // 协议适配器列表，由 plugins/adapter/*.js 通过 Bot.adapter.push 注册
    this.adapter = [];
  ...
  _setStaticHeaders(res, filePath) {
    ...
    // CDN配置
    const cdnConfig = cfg.server.cdn || {};
    const cdnEnabled = cdnConfig.enabled === true;
    ...
    if (['.html', '.htm'].includes(ext)) {
      ...
      const maxAge = cacheConfig.static ? parseCacheTime(cacheConfig.static) : parseCacheTime(cfg.server.static.cacheTime || '1d');
      res.setHeader('Cache-Control', `public, max-age=${maxAge}, immutable`);
      if (cdnEnabled && cdnConfig.cacheControl?.static) {
        res.setHeader('Cache-Control', `public, max-age=${cdnConfig.cacheControl.static}, immutable`);
      }
    } else if (['.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp', '.ico'].includes(ext)) {
      const maxAge = cacheConfig.images ? parseCacheTime(cacheConfig.images) : parseCacheTime(cfg.server.static.cacheTime || '7d');
      res.setHeader('Cache-Control', `public, max-age=${maxAge}, immutable`);
      if (cdnEnabled && cdnConfig.cacheControl?.images) {
        res.setHeader('Cache-Control', `public, max-age=${cdnConfig.cacheControl.images}, immutable`);
      }
    } else {
      const maxAge = cacheConfig.default ? parseCacheTime(cacheConfig.default) : parseCacheTime(cfg.server.static.cacheTime || '1d');
      res.setHeader('Cache-Control', `public, max-age=${maxAge}`);
      if (cdnEnabled && cdnConfig.cacheControl?.default) {
        res.setHeader('Cache-Control', `public, max-age=${cdnConfig.cacheControl.default}`);
      }
    }
    
    // CDN域名设置
    if (cdnEnabled && cdnConfig.domain && req.path.startsWith(cdnConfig.staticPrefix || '/static')) {
      const cdnUrl = `${cdnConfig.https ? 'https' : 'http'}://${cdnConfig.domain}${req.path}`;
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
   ...
   */
  _setupRedirects() {
    // no-op: 后续如果需要 HTTP->HTTPS 或路径重定向，可在此实现
  }
  ...
  /**
   * 加载协议适配器
   * 通过动态导入 plugins/adapter 目录下的所有适配器文件，
  ...
  async loadAdapters() {
    try {
      ...
      for (const file of jsFiles) {
        try {
          const moduleUrl = new URL(`../plugins/adapter/${file}`, import.meta.url);
          await import(moduleUrl);
        } catch (err) {
          BotUtil.makeLog('error', `适配器文件加载失败: ${file} - ${err.message}`, 'AdapterLoader');
        }
      }
    } catch {
      // 没有适配器目录时静默跳过
  ...
  _setupFinalHandlers() {
    ...
    this.express.use((req, res) => {
      ...
      if (req.domainConfig && req.domainConfig.defaultRoute) {
