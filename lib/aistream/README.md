...
| **aistream.js** | `AIStream`：构造期默认配置与 **XRK-AGT** 对齐（仅 `enabled/temperature/maxTokens/topP/presencePenalty/frequencyPenalty`，不含 `model`/`baseUrl`/`apiKey`/`timeout`）；`resolveLLMConfig` 合并单次调用、工作流 `this.config`、`LLMFactory.getProviderConfig(provider)`、`cfg.aistream.llm`；`callAI` / `callAIStream` 使用 `LLMFactory.createClient` 与 `{ ...config, stream, streams }`。 |
