...
} from '../../lib/ai-workflow/agent-session.js';
import {
  buildWorkflowCacheKey,
  shouldCacheWorkflowResult,
  clearWorkflowResultCache,
} from '../../lib/ai-workflow/workflow-cache.js';
...
describe('stream-cache 与会话 revision', () => {
  it('有副作用的 chat 流默认不缓存', () => {
    ...
    assert.equal(shouldCacheWorkflowResult(stream, e, '你好', {}), false);
  });
  ...
  it('无工具流在全局 cache 未启用时不缓存', () => {
    ...
    assert.equal(shouldCacheWorkflowResult(stream, e, '摘要这段', {}), false);
  });
  ...
  it('会话 revision 变化会改变 cache key', () => {
    ...
    clearWorkflowResultCache();
    const e = { self_id: 1, group_id: 99, user_id: 2 };
    const k1 = buildWorkflowCacheKey('summarize', e, 'q', {});
    bumpAgentSessionForEvent(e);
    const k2 = buildWorkflowCacheKey('summarize', e, 'q', {});
