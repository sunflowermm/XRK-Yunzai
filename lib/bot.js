...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  get fl() {
    ...
    for (const bot_id of this.uin) {
      ...
      } else if (bot.fl && typeof bot.fl === 'object') {
        for (const [id, friend] of Object.entries(bot.fl)) {
          map.set(id, { ...friend, bot_id });
        }
  ...
  get gl() {
    ...
    for (const bot_id of this.uin) {
      ...
      } else if (bot.gl && typeof bot.gl === 'object') {
        for (const [id, group] of Object.entries(bot.gl)) {
          map.set(id, { ...group, bot_id });
        }
  ...
  get gml() {
    ...
    for (const bot_id of this.uin) {
      ...
      } else if (bot.gml && typeof bot.gml === 'object') {
        for (const [group_id, members] of Object.entries(bot.gml)) {
          let newMemberMap;

          if (members instanceof Map) {
            newMemberMap = new Map(members);
          } else if (typeof members === 'object' && !Array.isArray(members)) {
            newMemberMap = new Map(Object.entries(members));
          } else if (Array.isArray(members)) {
            newMemberMap = new Map();
            for (const member of members) {
              if (member && member.user_id) {
                newMemberMap.set(member.user_id, member);
              }
            }
          } else {
            continue;
