...
export default class Bot extends EventEmitter {
  ...
  constructor() {
    ...
    this.httpPort = null;
    this.httpsPort = null;
    this.url = cfg.server?.server?.url || '';  // 兼容旧配置
  ...
  _initializeMiddlewareAndRoutes() {
    ...
    if (serverConfig.security?.helmet?.enabled !== false) {
      this.express.use(helmet({
        ...
        hsts: serverConfig.hsts?.enabled === true ? {
          maxAge: serverConfig.hsts.maxAge || 31536000,
          includeSubDomains: serverConfig.hsts.includeSubDomains !== false,
          preload: serverConfig.hsts.preload === true
        } : false
      }));
    }
      ...
      if (proxyConfig?.enabled && proxyConfig?.domains) {
        for (const domainConfig of proxyConfig.domains) {
          if (domainConfig.ssl?.enabled && domainConfig.ssl?.certificate) {
            ...
            // 读取域名证书
            const context = crypto.createSecureContext({
              key: await fs.readFile(cert.key),
              cert: await fs.readFile(cert.cert),
