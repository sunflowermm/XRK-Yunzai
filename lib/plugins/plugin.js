 ...
 * @property {string|string[]} [event='message'] - 监听事件
 ...
 * @property {boolean} [bypassThrottle=false] - 绕过节流
 * @property {Function|Object|string} [handler] - 扩展处理器
 * @property {string} [namespace=''] - 扩展命名空间
 */
...
export default class plugin {
    ...
    if (!globalWorkflowManager) {
      ...
      for (const stream of streams) {
        ...
        globalWorkflowManager.registerWorkflow(name, async (params, context) => {
          ...
          return await Bot.StreamLoader.executeStream(
            stream,
            e || params.e,
  ...
  async executeWorkflow(streamName, question, config = {}) {
    ...
    const e = this.e;
    return await Bot.StreamLoader.executeStream(stream, e, question, config || {});
  }
