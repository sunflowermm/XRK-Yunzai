...
export default function setLog() {
  ...
  log4js.configure({
    ...
    categories: {
      ...
      command: { 
        appenders: ['consoleFilter', 'command'], 
        level: 'warn',
        enableCallStack: false
      },
      error: { 
        appenders: ['consoleFilter', 'command', 'error'], 
        level: 'error',
        enableCallStack: false
      },
      trace: {
  ...
  const createLogMethod = (loggerInstance, level) => (...args) => {
    ...
    if ((level === 'error' || level === 'fatal') && args[0] instanceof Error) {
      ...
      logMethod.call(loggerInstance, errorMessage);
      
      if (error.stack && cfg.bot.log_level === 'trace') {
        traceLogger.trace(`Stack trace: ${error.stack}`);
      }
    } else {
      const logMethod = loggerInstance[level] || loggerInstance.info;
  ...
  async function cleanExpiredLogs() {
    ...
    const maxCommandFiles = cfg.bot.max_command_files || 10; // 最多保留的command日志文件数
    const maxErrorSize = 50 * 1024 * 1024; // error.log最大50MB
    const maxTraceSize = 30 * 1024 * 1024; // trace.log最大30MB
    const now = Date.now();
    ...
    try {
        ...
        if (file.startsWith('command.') && file.endsWith('.log')) {
          ...
          if (now - stats.mtime.getTime() > maxAgeMs) {
            try {
              await fsPromises.unlink(filePath);
              deletedCount++;
              logger.debug(`已删除过期压缩日志: ${file}`);
            } catch (err) {
              logger.error(`删除压缩日志失败: ${file}`, err.message);
            }
          }
        }
        else if (file === 'error.log') {
          const stats = await fsPromises.stat(filePath);
          if (stats.size > maxErrorSize) {
            try {
              const backupName = `error.${new Date().toISOString().split('T')[0]}.log`;
              await fsPromises.rename(filePath, path.join(logDir, backupName));
              await fsPromises.writeFile(filePath, '');
              cleanedCount++;
              logger.info(`error.log已超过大小限制，已备份为${backupName}`);
            } catch (err) {
              logger.error('处理error.log失败:', err.message);
            }
          }
        }
        // 检查trace.log大小
        else if (file === 'trace.log') {
          const stats = await fsPromises.stat(filePath);
          if (stats.size > maxTraceSize) {
            try {
              const backupName = `trace.${new Date().toISOString().split('T')[0]}.log`;
              await fsPromises.rename(filePath, path.join(logDir, backupName));
              await fsPromises.writeFile(filePath, '');
              cleanedCount++;
              logger.info(`trace.log已超过大小限制，已备份为${backupName}`);
            } catch (err) {
              logger.error('处理trace.log失败:', err.message);
            }
          }
        }
      }
      
      const validCommandFiles = commandFiles.filter(f => f.age <= maxAgeMs);
      if (validCommandFiles.length > maxCommandFiles) {
        validCommandFiles.sort((a, b) => b.date - a.date);
        
        const filesToDelete = validCommandFiles.slice(maxCommandFiles);
        for (const fileInfo of filesToDelete) {
          try {
            await fsPromises.unlink(fileInfo.path);
            deletedCount++;
            logger.debug(`已删除多余的日志文件: ${fileInfo.name}`);
          } catch (err) {
            logger.error(`删除多余日志文件失败: ${fileInfo.name}`, err.message);
          }
        }
      }
      ...
      for (const file of files) {
        if (file.endsWith('.log')) {
          ...
          try {
            const stats = await fsPromises.stat(filePath);
            if (stats.size === 0 && !['error.log', 'trace.log'].includes(file)) {
              await fsPromises.unlink(filePath);
              deletedCount++;
              logger.debug(`已删除空日志文件: ${file}`);
            }
          } catch (err) {
            // 忽略错误
          }
      ...
      if (deletedCount > 0 || cleanedCount > 0) {
        logger.info(`日志清理完成，删除${deletedCount}个文件，清理${cleanedCount}个大文件`);
      }
    } catch (err) {
  ...
  global.logger = {
    ...
    platform: function() {
      return {
        ...
        maxCommandFiles: cfg.bot.max_command_files || 10
      };
    },
