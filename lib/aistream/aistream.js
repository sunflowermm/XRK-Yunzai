...
export default class AIStream {
  ...
  resolveLLMConfig(apiConfig = {}) {
    ...
    const maxTokens = pick(apiConfig.maxTokens, apiConfig.max_tokens, this.config?.maxTokens, this.config?.max_tokens, providerConfig.maxTokens, providerConfig.max_tokens);
  ...
  async callAI(messages, apiConfig = {}) {
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        if (config.maxTokens !== undefined || config.max_tokens !== undefined) overrides.max_tokens = config.maxTokens ?? config.max_tokens;
        if (config.topP !== undefined || config.top_p !== undefined) overrides.top_p = config.topP ?? config.top_p;
        if (config.presencePenalty !== undefined || config.presence_penalty !== undefined) overrides.presence_penalty = config.presencePenalty ?? config.presence_penalty;
        if (config.frequencyPenalty !== undefined || config.frequency_penalty !== undefined) overrides.frequency_penalty = config.frequencyPenalty ?? config.frequency_penalty;
