...
class HandlerManager {
  ...
  del(ns, key = '') {
    if (!ns) {
      Bot.makeLog('error', '[Handler][Del]: 缺少命名空间参数', 'PluginsHandler')
      return 0
    }
  ...
  async call(key, e, args, allHandler = false) {
    ...
    if (!handlers?.length) {
      Bot.makeLog('debug', `[Handler][Call]: 没有找到 [${key}] 的处理器`, 'PluginsHandler')
      return
    }
  ...
  clear() {
    ...
    Bot.makeLog('mark', '[Handler][Clear]: 已清空所有处理器', 'PluginsHandler')
  }
  ...
  _validateParams(key, fn, ns) {
    if (!key || typeof key !== 'string') {
      Bot.makeLog('error', '[Handler][Add]: 事件键名无效', 'PluginsHandler')
      return false
    }

    if (typeof fn !== 'function') {
      Bot.makeLog('error', `[Handler][Add]: [${ns}][${key}] 处理函数必须是函数类型`, 'PluginsHandler')
      return false
    }

    if (!ns) {
      Bot.makeLog('error', `[Handler][Add]: [${key}] 缺少命名空间`, 'PluginsHandler')
      return false
    }
  ...
  _deleteHandler(ns, key) {
    ...
    if (deletedCount > 0) {
      ...
      Bot.makeLog('debug', `[Handler][Del]: 删除了 [${ns}][${key}] 的 ${deletedCount} 个处理器`, 'PluginsHandler')
    }
  ...
  async _executeHandler(handler, e, args) {
    ...
    const reject = (msg = '') => {
      if (msg) {
        Bot.makeLog('mark', `[Handler][Reject]: [${ns}][${key}] ${msg}`, 'PluginsHandler')
      }
      done = false
    }
    ...
    try {
      ...
      if (done) {
        Bot.makeLog('mark', `[Handler][Done]: [${ns}][${key}]`, 'PluginsHandler')
      }
      
      return { done, value }
    } catch (error) {
      Bot.makeLog('error', `[Handler][Error]: [${ns}][${key}] 执行出错:`, 'PluginsHandler')
      Bot.makeLog('error', error.stack || error, 'PluginsHandler')
