...
function workspaceRelativeSkillPath(filePath, workspaceRoot) {
...
function withWorkspaceRelativePaths(skills, workspaceRoot) {
  return skills.map((s) => ({
    ...s,
    filePath: workspaceRelativeSkillPath(s.filePath, workspaceRoot),
  }));
}
...
function formatSkillsCompact(skills) {
  ...
  const lines = [
    ...
    'tools.run cwd is the agent workspace root: use paths like skills/foo/SKILL.md or docs/report.py — never data/ai-workspace/… or absolute project paths.',
    '',
    '<available_skills>',
  ];
...
export function buildSkillsPromptFromWorkspace(workspaceRootResolved, cfg = {}) {
  ...
  const workspaceRoot = path.resolve(workspaceRootResolved);
  const skillRoots = resolveSkillRoots(cfg, workspaceRoot);

  const merged = new Map();
  for (const abs of skillRoots) {
    if (!FileUtils.existsSync(abs) || !FileUtils.statSync(abs)?.isDirectory()) continue;
    const slug = path.basename(abs).replace(/[^\w.-]+/g, '_');
  ...
  const promptSkills = withWorkspaceRelativePaths(resolvedSkills, workspaceRoot);
  ...
  return buildCombined(skillsForPrompt, compact);
}
