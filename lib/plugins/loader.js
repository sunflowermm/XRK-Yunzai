...
class PluginsLoader {
  constructor() {
    ...
    // 使用 BotUtil 的 Map 管理功能
    this.cooldowns = {
      group: BotUtil.getMap('cooldown_group', { maxSize: 1000, ttl: 300000 }),
      single: BotUtil.getMap('cooldown_single', { maxSize: 5000, ttl: 300000 }),
      device: BotUtil.getMap('cooldown_device', { maxSize: 1000, ttl: 300000 })
    }
    this.msgThrottle = BotUtil.getMap('msg_throttle', { maxSize: 10000, ttl: 5000, autoClean: true })
    this.eventThrottle = BotUtil.getMap('event_throttle', { maxSize: 5000, ttl: 60000, autoClean: true })
  ...
  async parseMessage(e) {
    ...
    for (const val of e.message) {
      ...
      const handler = messageHandlers[val.type]
        ...
        if (plugin?.accept) {
          try {
            ...
            if (e._needReparse) {
              ...
              e.atList = []
              e.atBot = false
              // 保留已修改的 e.msg，只重新解析 message 数组中的其他类型
              const oldMsg = e.msg
              e.msg = ''
              await this.parseMessage(e)
              // 如果 parseMessage 没有生成新的 msg，使用旧的
              if (!e.msg) e.msg = oldMsg
            }
  ...
  async preCheck(e, hasBypassPlugin = false) {
    try {
      ...
      // 获取原始消息并处理
      let msg = e.raw_message || BotUtil.extractTextContent(e.message)
        ...
        if (plugin.rule) {
          for (const rule of plugin.rule) {
            if (rule.reg) {
              ...
              const tempMsg = BotUtil.extractTextContent(e.message)
              if (rule.reg.test(this.dealText(tempMsg))) return true
            }
          }
        }
      } catch (error) {
  ...
  addUtilMethods(e) {
    ...
    e.getSendableMedia = async (media) => {
      ...
      return await BotUtil.Buffer(media, { http: false, file: false })
    }
  ...
  setLimit(e) {
    ...
    if (e.isDevice) {
      ...
      if (deviceCD > 0) {
        this.cooldowns.device.set(e.device_id, Date.now())
      }
    } else {
      if (config.groupGlobalCD > 0) {
        this.cooldowns.group.set(e.group_id, Date.now())
      }
      if (config.singleCD > 0) {
        this.cooldowns.single.set(`${e.group_id}.${e.user_id}`, Date.now())
      }
    }
  }
  ...
  recordEventHistory(eventType, eventData) {
    const historyEntry = {
      event_id: eventData.event_id || BotUtil.uuid('ulid'),
  ...
  async destroy() {
    try {
      ...
      // 销毁 Map
      BotUtil.deleteMap('cooldown_group')
      BotUtil.deleteMap('cooldown_single')
      BotUtil.deleteMap('cooldown_device')
      BotUtil.deleteMap('msg_throttle')
      BotUtil.deleteMap('event_throttle')
