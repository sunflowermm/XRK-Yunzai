...
export class PluginDirScanner {
  /**
   * @param {string} subdir - 子目录名，如 http、workflow、events、adapter
  ...
  /**
   * 工作流目录：plugins/<名>/workflow/
   * @param {string} [pluginsRoot]
   * @returns {string[]}
   */
  static listWorkflowDirs(pluginsRoot) {
    return this.listSubdirPaths('workflow', pluginsRoot);
  }
