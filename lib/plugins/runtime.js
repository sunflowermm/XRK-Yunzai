/**
 * [compat] e.runtime — getMysApi / render / Handler / puppeteer 等
 * 动态解析 miao-plugin / genshin 米游 API，降低对目录结构的依赖
 */
...
export default class Runtime {
  ...
  async render(plugin, tplPath, data = {}, cfg = {}) {
    tplPath = String(tplPath || "").replace(/.html$/, "")
    let paths = lodash.filter(tplPath.split("/"), p => !!p)
    tplPath = paths.join("/")
    await Bot.mkdir(`temp/html/${plugin}/${tplPath}`)
    ...
    data = {
      ...
      copyright: `Created By XRK-Yunzai<span class="version">${Version?.yunzai}</span> `,
      ...
      _htmlPath: tplPath,
      pluResPath,
      tplFile: `./plugins/${plugin}/resources/${tplPath}.html`,
    ...
    let base64 = await this.puppeteer.screenshot(`${plugin}/${tplPath}`, data)
