...
import { flattenData as flattenConfigData, unflattenData as expandConfigData } from './config-utils.js';
...
export default class ConfigBase {
  ...
  async read(useCache = true) {
    ...
    if (!(await this.exists())) {
      ...
      const defaultTemplatePath = resolveProjectPath(DEFAULT_CONFIG_DIR, `${this.name}.${ext}`);
      if (FileUtils.existsSync(defaultTemplatePath)) {
        content = FileUtils.readFileSync(defaultTemplatePath, 'utf8');
        if (content == null) throw new Error(`配置文件不存在: ${filePath}`);
        Bot.makeLog('info', `使用默认配置模板 [${this.name}]`, 'ConfigBase');
      } else {
        throw new Error(`配置文件不存在: ${filePath}`);
      }
    } else {
      content = FileUtils.readFileSync(filePath, 'utf8');
      if (content == null) throw new Error(`读取配置失败: ${filePath}`);
    }
  ...
  async _readMultiFile() {
    ...
    for (const key of keys) {
      ...
      if (defaultFilePath && FileUtils.existsSync(defaultFilePath)) {
        try {
          const content = FileUtils.readFileSync(defaultFilePath, 'utf8');
          if (content != null) {
            config = this.fileType === 'yaml' ? yaml.parse(content) : JSON.parse(content);
          }
        } catch (error) {
          Bot.makeLog('warn', `读取默认配置失败 [${this.name}/${key}]: ${error.message}`, 'ConfigBase');
        }
      }
      ...
      if (FileUtils.existsSync(filePath)) {
        try {
          ...
          Bot.makeLog('warn', `读取配置失败 [${this.name}/${key}]: ${error.message}`, 'ConfigBase');
        }
      }
