...
import { resolveToolWorkflowNames, collectAuxiliaryWorkflowPrompts } from './chat-tool-workflows.js';

/**
 * @typedef {Object} AiWorkflowOptions
 * @property {string} [name='base-stream'] - 工作流名称（必填语义）
 * @property {string} [description='基础工作流']
 * @property {string} [version='1.0.0']
 * @property {string} [author='unknown']
 * @property {number} [priority=100]
 * @property {Object} [config] - LLM/行为配置
 * @property {Object} [functionToggles] - 功能开关
 */
 ...
 * 所有工作流都应继承此类。
 * 
 * 文件路径: lib/ai-workflow/ai-workflow.js
 * 工作流存放路径: plugins/<插件名>/workflow/
 * 
 * @class AiWorkflow
 * @example
...
export default class AiWorkflow {
  ...
  constructor(options = {}) {
    ...
    const L = getAiWorkflowConfigOptional().llm || {};
    // 工作流级默认：来自合并后的 ai-workflow.yaml（llm.temperature 等）；无 cfg 时与 default_config 回落一致
    this.config = {
      enabled: true,
  ...
  async init() {
    ...
    if (this.memorySystem && this.memorySystem.isEnabled() && cfg.masterQQ) {
      await this.memorySystem.initMasters(cfg.masterQQ || []).catch((err) => {
        Bot.makeLog('debug', `[AIStream] initMasters 失败: ${err?.message || err}`, this.name);
      });
    }
  ...
  /**
   * 在 system 文案末尾注入工作区上下文（AGENTS、rules、skills、subagents）。
   * @param {string} text
  ...
  async finalizeSystemPromptContent(text) {
    ...
    const aux = collectAuxiliaryWorkflowPrompts(this);
  ...
  resolveLLMConfig(apiConfig = {}) {
    ...
    if (providerRaw && !LLMFactory.hasProvider(providerRaw)) {
      Bot.makeLog('warn', `[AIStream] 不支持的 LLM 提供商: ${providerRaw}`, 'AIStream');
    }

    const providerConfig = LLMFactory.getProviderConfig(provider) || {};

    // 统一优先级：单次 apiConfig > 工作流 this.config > 提供商 yaml > ai-workflow.yaml 的 llm（及 global.maxTimeout 兜底超时）
    const timeout = pick(
      apiConfig.timeout,
      apiConfig.timeoutMs,
      this.config?.timeout,
    ...
    const headers = ObjectUtils.shallowMergePlain(providerConfig.headers, this.config.headers, apiConfig.headers);
    const extraBody = ObjectUtils.shallowMergePlain(providerConfig.extraBody, this.config.extraBody, apiConfig.extraBody);
    const proxy = ObjectUtils.shallowMergePlain(providerConfig.proxy, this.config.proxy, apiConfig.proxy);
  ...
  _getToolStreamNames() {
    ...
    this._cachedToolStreamNames = resolveToolWorkflowNames(this);
    Bot.makeLog('debug', `[AIStream] 工具白名单 ${this.name}: [${this._cachedToolStreamNames.join(', ')}]`, 'AIStream');
    return this._cachedToolStreamNames;
  }
  ...
  async callAI(messages, apiConfig = {}) {
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
      Bot.makeLog('warn', '[AIStream] callAI 消息数组为空', 'AIStream');
      return null;
    }
    Bot.makeLog('debug', `[AIStream] callAI 入口 messagesLen=${messages.length}`, 'AIStream');

    let config = applyPromptCachePolicy(this.resolveLLMConfig(apiConfig), {
      stream: this,
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const { text, usedReplyTool, toolRoundsExhausted } = unpackFactoryChatRaw(raw);
        const trimmed = text != null ? String(text).trim() : '';
        Bot.makeLog(
          'debug',
          `[AIStream] callAI 返回 attempt=${attempt} resultLen=${trimmed.length} usedReplyTool=${usedReplyTool} toolRoundsExhausted=${toolRoundsExhausted}`,
          'AIStream'
        );
        if (toolRoundsExhausted) {
          return { text: trimmed, usedReplyTool, toolRoundsExhausted: true };
        }
        ...
        if (shouldRetry) {
          ...
          Bot.makeLog('warn', `[AIStream] AI调用失败，${attempt}/${retryConfig.maxAttempts}次重试中: ${error.message}`, 'AIStream');
          await new Promise(r => setTimeout(r, delay));
          continue;
        }

        Bot.makeLog('error', `[AIStream] AI调用失败: ${error.message}`, 'AIStream');
        return null;
      }
    }

    Bot.makeLog('error', `[AIStream] AI调用失败，已重试${retryConfig.maxAttempts}次: ${lastError?.message || '未知错误'}`, 'AIStream');
    return null;
  }
  ...
  async callAIStream(messages, apiConfig = {}, onDelta) {
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
      Bot.makeLog('warn', '[AIStream] 消息数组为空', 'AIStream');
      onDelta?.('[ERROR] 消息数组为空');
      return;
    }

    if (typeof onDelta !== 'function') {
      Bot.makeLog('warn', '[AIStream] onDelta回调函数未提供', 'AIStream');
      return;
    }
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        if (shouldRetry) {
          const delay = this.calculateRetryDelay(attempt, retryConfig);
          Bot.makeLog('warn', `[AIStream] AI流式调用失败，${attempt}/${retryConfig.maxAttempts}次重试中: ${error.message}`, 'AIStream');
          await new Promise(r => setTimeout(r, delay));
          continue;
        }

        Bot.makeLog('error', `[AIStream] AI流式调用失败: ${error.message}`, 'AIStream');
        onDelta?.(`[ERROR] ${error.message}`);
        return;
      }
    }

    Bot.makeLog('error', `[AIStream] AI流式调用失败，已重试${retryConfig.maxAttempts}次: ${lastError?.message || '未知错误'}`, 'AIStream');
  ...
  async cleanup() {
    Bot.makeLog('debug', `[${this.name}] 清理资源`, 'AIStream');
    this._initialized = false;
  }
}