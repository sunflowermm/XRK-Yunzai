...
function getMainPort() {
  try {
    ...
    Bot.makeLog('debug', `[getMainPort] ${err?.message || err}`, 'FrontendLauncher');
  }
  return 8086;
}
...
function getMainOrigin() {
  try {
    ...
    Bot.makeLog('debug', `[getMainOrigin] ${err?.message || err}`, 'FrontendLauncher');
  }
...
async function discoverConfigs() {
  ...
  for (const file of files) {
    try {
      const raw = await FileUtils.readFile(file, 'utf8');
      if (!raw) {
        Bot.makeLog('debug', `sign.json 读取失败: ${file}`, 'Frontend');
        continue;
      }
      const json = tryParseJson(raw);
      if (!json) {
        Bot.makeLog('debug', `sign.json 解析失败: ${file}`, 'Frontend');
        continue;
      }

      if (json.enabled === false) {
        Bot.makeLog('debug', `跳过禁用的前端 sign.json: ${file}`, 'Frontend');
        continue;
      }
      ...
      if (!command || !Number.isFinite(port) || port <= 0) {
        Bot.makeLog('warn', `sign.json 缺少必要字段(command/port): ${file}`, 'Frontend');
        continue;
      }

      const args = Array.isArray(json.args) ? json.args.map(a => String(a)) : [];
      const cwd = json.cwd ? path.resolve(resolveProjectPath(), json.cwd) : dir;
      ...
      Bot.makeLog('warn', `解析 sign.json 失败: ${file} - ${err.message}`, 'Frontend');
    }
  }
...
function startApp(config) {
  ...
  const handleOutput = (data, stream) => {
    ...
    Bot.makeLog('debug', `[${config.id}] ${stream}: ${trimmed}`, 'Frontend');
  };
  ...
  child.on('error', err => {
    ...
    Bot.makeLog('error', `前端项目进程错误: ${config.id} - ${err.message}`, 'Frontend');
  });
  ...
  child.on('exit', (code, signal) => {
    ...
    if (appInfo.restarts >= 3) {
      Bot.makeLog('warn', `前端项目重启次数已达上限，停止重启: ${config.id}`, 'Frontend');
      return;
    }

    appInfo.restarts += 1;
    const delay = 1000 * appInfo.restarts;
    Bot.makeLog('info', `准备重启前端项目(${appInfo.restarts}): ${config.id}, ${delay}ms 后`, 'Frontend');
    setTimeout(() => {
...
async function doInit() {
  ...
  if (configs.length === 0) {
    Bot.makeLog('info', '未发现启用的 sign.json 前端项目，跳过启动', 'Frontend');
    return apps;
  }

  Bot.makeLog('info', `发现前端项目 ${configs.length} 个，开始启动...`, 'Frontend');
  ...
  Bot.makeLog('info', `前端项目启动完成: ${apps.size} 个, 耗时 ${used}`, 'Frontend');
  return apps;
}
...
export async function init() {
  ...
  initializing = doInit()
    .catch(err => {
      Bot.makeLog('error', `前端项目初始化失败: ${err.message}`, 'Frontend');
      return apps;
    })
    .finally(() => {
      initialized = true;
