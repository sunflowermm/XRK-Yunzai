...
import chalk from "chalk"; // 直接导入 chalk
...
export default class BotUtil {
  ...
  static makeLogID(id) {
    ...
    // 优先使用导入的 chalk，如果不可用则尝试从 global.logger 获取
    ...
    if (displayWidth > targetLength) {
      ...
      let truncated = '';
      let currentWidth = 0;
      const maxWidth = targetLength - 2; // 留给 '..' 的空间
  ...
  static async fsStat(filePath) {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        BotUtil.makeLog("trace", ["获取", filePath, "统计信息错误", err.code || err.message]);
      }
      return false;
    }
  }
  ...
  static async Buffer(data, opts = {}) {
    ...
    if (BotUtil.regexCache.url.test(dataStr)) {
      ...
      try {
        ...
        BotUtil.makeLog("error", ["获取 URL 内容错误", dataStr, err.message]);
        return Buffer.alloc(0);
      }
    }
  ...
  static async fileToUrl(file, opts = {}) {
    ...
    try {
      ...
      BotUtil.makeLog("error", ["文件转 URL 错误", err.message]);
      throw err;
    }
  }
  ...
  static async exec(cmd, opts = {}) {
    ...
    BotUtil.makeLog(opts.quiet ? "debug" : "mark", cmdStr, "命令");
    ...
    return new Promise((resolve) => {
      ...
      const callback = (error, stdout, stderr) => {
        ...
        if (!opts.quiet) {
          ...
          BotUtil.makeLog("mark", logMessage, "命令");
        }

        if (error && !opts.quiet) {
          BotUtil.makeLog("error", error, "命令");
        }
  ...
  static async makeChatRecord(e, messages, title, description = ["Bot 消息"]) {
    ...
    try {
      ...
      BotUtil.makeLog("error", ["创建聊天记录错误", err.message]);
      try {
