...
import { resolveSkillLimits } from './skills/config.js';
import {
  AGENTS_MD,
...
function readTextFileUnderWorkspaceRootCached(rootResolved, absolutePath, maxBytes) {
  ...
  if (!st?.isFile()) return { ok: false, reason: 'io' };
...
function readWorkspaceRelIfExists(workspaceRoot, rel, maxBytes) {
  const fp = path.join(workspaceRoot, rel);
  if (!FileUtils.existsSync(fp)) return null;
  const got = readTextFileUnderWorkspaceRootCached(workspaceRoot, fp, maxBytes);
  return got.ok ? got : null;
}

function injectWorkspaceAssistant(workspaceRoot, maxChars, pushProse) {
  const agentsGot = readWorkspaceRelIfExists(workspaceRoot, AGENTS_MD, maxChars * 4);
  if (agentsGot) {
    pushProse(AGENTS_MD, truncate(agentsGot.content, maxChars, AGENTS_MD));
  }

  for (const rel of WORKSPACE_TEMPLATE_RELS) {
    const got = readWorkspaceRelIfExists(workspaceRoot, rel, maxChars * 4);
    if (!got) continue;
  ...
  const memoryGot = readWorkspaceRelIfExists(workspaceRoot, LONG_TERM_MEMORY_REL, maxChars * 4);
  if (memoryGot) {
    pushProse(LONG_TERM_MEMORY_REL, truncate(memoryGot.content, maxChars, LONG_TERM_MEMORY_REL));
  }
}
...
function collectRulesMarkdown(projectRoot, maxRulesChars) {
  ...
  for (const rulesDir of rulesDirs) {
    if (!FileUtils.existsSync(rulesDir)) continue;
...
export async function buildAgentWorkspaceSection(agentWorkspaceCfg = {}, streamName = '') {
  const cfg = {
    ...
    customSkillRoots: [],
    contextFiles: [],
    ...resolveSkillLimits(agentWorkspaceCfg),
    ...agentWorkspaceCfg
  };

  if (cfg.enabled === false) return '';

  const workflowFilter = Array.isArray(cfg.workflows)
    ? cfg.workflows
    : (Array.isArray(cfg.streams) ? cfg.streams : null);
  if (workflowFilter && workflowFilter.length > 0 && streamName) {
    if (!workflowFilter.includes(streamName)) return '';
  }

  let workspaceRoot;
  let projectRoot;
  try {
    const customRoot = cfg.root != null && String(cfg.root).trim() !== '';
    workspaceRoot = realpathSyncOrResolve(
      customRoot
        ? resolveAgentWorkspaceAbs(String(cfg.root).trim())
  ...
  if (FileUtils.existsSync(wsSkillsDir)) {
    const skillsPrompt = buildSkillsPromptFromWorkspace(workspaceRoot, cfg);
...
export async function mergeAgentWorkspaceIntoMessages(messages, aiWorkflowCfg = {}, streamName = '') {
  ...
  if (first?.role === 'system' && typeof first.content === 'string') {
    first.content = await appendAgentWorkspaceToPrompt(first.content, aiWorkflowCfg, streamName);
    return messages;
  }
  const extra = await buildAgentWorkspaceSection(sliceWorkspaceCfg(aiWorkflowCfg), streamName);
