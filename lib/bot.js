...
/**
 * Bot主类 - 管理HTTP/HTTPS服务器、WebSocket连接、反向代理和机器人实例
 * 提供统一的服务器管理、多域名路由、反向代理、认证、静态文件服务等功能
 * @class Bot
...
export default class Bot extends EventEmitter {
  ...
  constructor() {
    ...
    // 反向代理服务器组件
    this.proxyApp = express();  // 主代理应用
    this.proxyServer = null;     // HTTP代理服务器（80端口）
    this.proxyHttpsServer = null; // HTTPS代理服务器（443端口）
    this.domainApps = new Map();  // 域名到Express应用的映射
    this.sslContexts = new Map();  // 域名到SSL上下文的映射
  ...
  _initProxyServers() {
    ...
    this.proxyServer = http.createServer(this.proxyApp)
      .on("error", err => this._handleProxyServerError(err, false))
      .on("upgrade", this._handleProxyWebSocketUpgrade.bind(this));

    // 准备HTTPS代理服务器（443端口）
    // 实际的HTTPS服务器将在加载配置后创建，因为需要SSL证书
  }
  ...
  _setupProxyMiddleware() {
    ...
    this.proxyApp.use((req, res, next) => {
      ...
      res.on('finish', () => {
        ...
        BotUtil.makeLog('debug',
          chalk.cyan(`[Proxy] ${req.method} ${req.hostname}${req.path} - `) +
          chalk[statusColor](res.statusCode) + chalk.gray(` (${duration}ms)`),
          'ProxyServer'
        );
      });
      next();
    ...
    this.proxyApp.use((req, res, next) => {
      ...
      if (domainConfig) {
        // 如果找到域名配置，使用对应的处理器
        const handler = this.domainApps.get(hostname);
        if (handler) {
  ...
  _findDomainConfig(hostname) {
    const domains = cfg.server?.proxy?.domains || [];

    // 精确匹配
    let config = domains.find(d => d.domain === hostname);
    if (config) return config;

    // 通配符匹配
    for (const domainConfig of domains) {
      if (domainConfig.domain.startsWith('*.')) {
        const baseDomain = domainConfig.domain.substring(2);
   ...
   * @async
   */
  async _initializeDomainRouting() {
    const proxyConfig = cfg.server?.proxy || {};
    const domains = proxyConfig.domains || [];

    BotUtil.makeLog('info',
      chalk.cyan(`⚙ 初始化域名路由，共 ${domains.length} 个域名配置`),
      'ProxyServer'
    );

    for (const domainConfig of domains) {
      await this._setupDomainHandler(domainConfig);
    }
  }
   ...
   */
  async _setupDomainHandler(domainConfig) {
    const { domain, type, target, pathRewrite, ssl } = domainConfig;

    BotUtil.makeLog('info',
      chalk.green(`✓ 配置域名: ${domain} -> ${type === 'proxy' ? target : '本地服务'}`),
      'ProxyServer'
    );

    // 处理SSL证书
    if (ssl && ssl.cert && ssl.key) {
      await this._loadSSLContext(domain, ssl);
    }
    ...
    if (type === 'proxy') {
      // 反向代理到目标服务器
      handler = this._createProxyHandler(domainConfig);
    } else if (type === 'local') {
      // 代理到本地Bot服务
      handler = this._createLocalHandler(domainConfig);
    } else if (type === 'static') {
      // 静态文件服务
      handler = this._createStaticHandler(domainConfig);
    } else {
      BotUtil.makeLog('warn',
        chalk.yellow(`⚠ 未知的域名类型: ${type} (${domain})`),
        'ProxyServer'
      );
      return;
    }
    ...
    if (domain.startsWith('*.')) {
      ...
      this.domainApps.set(domain, (req, res, next) => {
        if (req.hostname.endsWith(baseDomain)) {
      ...
      // 精确域名匹配
      this.domainApps.set(domain, handler);
    }
  }
   ...
   */
  _createProxyHandler(domainConfig) {
    const { target, pathRewrite, headers, changeOrigin = true, ws = true } = domainConfig;
    ...
    const proxyOptions = {
      ...
      headers: headers || {},
      logLevel: 'silent', // 我们自己处理日志

      // 错误处理
      onError: (err, req, res) => {
        BotUtil.makeLog('error',
          chalk.red(`✗ 代理错误 (${req.hostname}): ${err.message}`),
          'ProxyServer'
        );
          ...
          res.status(502).send(`
            ...
            <body>
              ...
              <div class="error">
                ...
                <p>错误: <code>${err.code || err.message}</code></p>
              </div>
            </body>
            </html>
          `);
   ...
   * @returns {Function} Express中间件
   */
  _createLocalHandler(domainConfig) {
    const { pathRewrite = {} } = domainConfig;

    // 代理到本地Bot服务
    return createProxyMiddleware({
      target: `http://127.0.0.1:${this.httpPort}`,
      changeOrigin: false,
      pathRewrite,
      logLevel: 'silent',
   ...
   * @returns {Function} Express中间件
   */
  _createStaticHandler(domainConfig) {
    const { root, pathRewrite = {} } = domainConfig;
    const staticPath = path.isAbsolute(root) ? root : path.join(process.cwd(), root);

    // 确保目录存在
    if (!fsSync.existsSync(staticPath)) {
      fsSync.mkdirSync(staticPath, { recursive: true });
      BotUtil.makeLog('info',
        chalk.green(`✓ 创建静态文件目录: ${staticPath}`),
        'ProxyServer'
      );
    }
    ...
    app.use((req, res, next) => {
      ...
      for (const [pattern, replacement] of Object.entries(pathRewrite)) {
        ...
        if (regex.test(req.path)) {
          req.url = req.path.replace(regex, replacement);
          break;
        }
      }
      next();
    ...
    app.use((req, res) => {
      ...
      if (fsSync.existsSync(notFoundPath)) {
   ...
   */
  async _loadSSLContext(domain, ssl) {
    try {
      const context = {
        cert: await fs.readFile(ssl.cert),
        key: await fs.readFile(ssl.key)
      };

      if (ssl.ca) {
        context.ca = await fs.readFile(ssl.ca);
      }

      this.sslContexts.set(domain, context);

      BotUtil.makeLog('info',
        chalk.green(`✓ 加载SSL证书: ${domain}`),
        'ProxyServer'
      );

    } catch (error) {
      BotUtil.makeLog('error',
        chalk.red(`✗ 加载SSL证书失败 (${domain}): ${error.message}`),
        'ProxyServer'
      );
    }
  }
  ...
  async _createHttpsProxyServer() {
    // 如果没有SSL证书配置，则不创建HTTPS服务器
    if (this.sslContexts.size === 0) {
      BotUtil.makeLog('warn',
        chalk.yellow('⚠ 未配置任何SSL证书，跳过HTTPS代理服务器'),
        'ProxyServer'
      );
      return;
    }
    ...
    // 获取默认证书（第一个配置的证书）
    const defaultContext = this.sslContexts.values().next().value;
    ...
    const sniCallback = (servername, callback) => {
      ...
      if (context) {
        ...
        // 检查通配符证书
        for (const [domain, ctx] of this.sslContexts) {
          if (domain.startsWith('*.')) {
            const baseDomain = domain.substring(2);
    ...
    // 创建HTTPS服务器
    this.proxyHttpsServer = https.createServer({
      ...defaultContext,
    ...
    // WebSocket升级处理
    this.proxyHttpsServer.on('upgrade', this._handleProxyWebSocketUpgrade.bind(this));

    BotUtil.makeLog('info',
      chalk.green(`✓ HTTPS代理服务器已创建，支持 ${this.sslContexts.size} 个域名`),
      'ProxyServer'
    );
  }
  ...
  _handleProxyServerError(err, isHttps) {
    ...
    if (err.code === 'EADDRINUSE') {
      BotUtil.makeLog('error',
        chalk.red(`✗ ${serverType}代理端口 ${port} 已被占用`),
        'ProxyServer'
      );

      BotUtil.makeLog('info',
        chalk.yellow(`提示: 请确保没有其他服务占用端口 ${port}，或使用管理员权限运行`),
        'ProxyServer'
      );
      ...
      BotUtil.makeLog('error',
        chalk.red(`✗ 权限不足：无法绑定${serverType}代理端口 ${port}`),
        'ProxyServer'
      );

      BotUtil.makeLog('info',
        chalk.yellow(`提示: 在Linux/Mac上，绑定1024以下端口需要root权限`),
        'ProxyServer'
      );

      BotUtil.makeLog('info',
        chalk.cyan(`可以尝试: sudo node ${process.argv[1]}`),
        'ProxyServer'
      );
      ...
      BotUtil.makeLog('error',
        chalk.red(`✗ ${serverType}代理服务器错误: ${err.message}`),
        'ProxyServer'
      );
    }
  }
   ...
   */
  _handleProxyWebSocketUpgrade(req, socket, head) {
    const hostname = req.headers.host?.split(':')[0] || 'localhost';
    const domainConfig = this._findDomainConfig(hostname);

    if (domainConfig && domainConfig.ws !== false) {
      // 如果是代理类型，转发WebSocket
      if (domainConfig.type === 'proxy') {
        const target = new URL(domainConfig.target);
        ...
        proxyReq.on('error', err => {
          BotUtil.makeLog('error',
            chalk.red(`✗ WebSocket代理错误: ${err.message}`),
            'ProxyServer'
          );
          socket.end();
        });
        ...
        proxyReq.end();
      } else if (domainConfig.type === 'local') {
        // 转发到本地Bot服务
        this.wsConnect(req, socket, head);
      }
    } else {
  ...
  async startProxyServers() {
    // 初始化域名路由
    await this._initializeDomainRouting();

    // 启动HTTP代理服务器（80端口）
    await new Promise((resolve, reject) => {
      this.proxyServer.listen(80, '0.0.0.0', () => {
        BotUtil.makeLog('success',
          chalk.green.bold(`✓ HTTP反向代理服务器已启动 (端口: 80)`),
          'ProxyServer'
        );
        resolve();
      });
    ...
    }).catch(err => this._handleProxyServerError(err, false));

    // 创建并启动HTTPS代理服务器（443端口）
    if (this.sslContexts.size > 0) {
      await this._createHttpsProxyServer();

      await new Promise((resolve, reject) => {
        this.proxyHttpsServer.listen(443, '0.0.0.0', () => {
          BotUtil.makeLog('success',
            chalk.green.bold(`✓ HTTPS反向代理服务器已启动 (端口: 443)`),
            'ProxyServer'
          );
          resolve();
        });

        this.proxyHttpsServer.once('error', reject);
      }).catch(err => this._handleProxyServerError(err, true));
    }
  ...
  _displayProxyInfo() {
    ...
    console.log(chalk.yellow('▶ 已配置的域名:'));
    console.log(chalk.gray('─'.repeat(60)));

    for (const domain of domains) {
      const protocol = domain.ssl ? 'https' : 'http';
      const targetInfo = domain.type === 'proxy' ?
        chalk.gray(` → ${domain.target}`) :
        domain.type === 'local' ?
          chalk.gray(` → 本地服务 (${this.httpPort})`) :
          chalk.gray(` → 静态文件 (${domain.root})`);

      console.log(`  ${chalk.cyan('•')} ${chalk.white(`${protocol}://${domain.domain}`)}${targetInfo}`);

      // 显示路径重写规则
      if (domain.pathRewrite && Object.keys(domain.pathRewrite).length > 0) {
        for (const [from, to] of Object.entries(domain.pathRewrite)) {
          console.log(`    ${chalk.gray('↳')} ${chalk.blue(from)} ${chalk.gray('→')} ${chalk.green(to)}`);
        }
      }
    }
    ...
    console.log(chalk.gray('  • 请确保域名已正确解析到本服务器IP'));
    console.log(chalk.gray('  • 如果使用HTTPS，请确保SSL证书有效'));
    console.log(chalk.gray('  • 可以通过修改hosts文件进行本地测试'));
    ...
    if (localIP) {
      ...
      for (const domain of domains) {
        if (!domain.domain.includes('*')) {
          console.log(chalk.gray(`  ${localIP} ${domain.domain}`));
        }
      }
    }
...
<VirtualHost *:443>
    ...
    # 静态文件
    <Directory "${process.cwd()}/www">
        AllowOverride All
        Require all granted
    </Directory>
    
    # API代理
    ProxyPass /api https://127.0.0.1:${this.httpsPort}/api
    ProxyPassReverse /api https://127.0.0.1:${this.httpsPort}/api
    
    # WebSocket代理
    RewriteEngine On
    RewriteCond %{HTTP:Upgrade} websocket [NC]
    RewriteCond %{HTTP:Connection} upgrade [NC]
    RewriteRule ^/?(.*) "wss://127.0.0.1:${this.httpsPort}/$1" [P,L]
</VirtualHost>`));

    console.log(chalk.yellow('\n提示:'));
    console.log(chalk.gray('• 请根据您的实际情况修改域名和证书路径'));
    console.log(chalk.gray('• 静态文件建议由Web服务器直接提供以提升性能'));
    console.log(chalk.gray('• 记得重载Web服务器配置: nginx -s reload 或 service apache2 reload'));
    console.log(chalk.gray('─'.repeat(60)) + '\n');
  }
  ...
  _setupFinalHandlers() {
    ...
    this.express.use((req, res) => {
      if (req.accepts('html')) {
        // 对HTML请求，尝试加载404.html或重定向到首页
        const custom404Path = path.join(process.cwd(), 'www', '404.html');
  ...
  async run(options = {}) {
    ...
    BotUtil.makeLog('info',
      chalk.gray(`Bot服务端口: HTTP=${this.httpPort}, HTTPS=${this.httpsPort}`), 'Server');
    ...
    // 启动反向代理服务器（80和443端口）
    if (cfg.server?.proxy?.enabled !== false) {
