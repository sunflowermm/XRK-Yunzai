...
export class WorkflowManager {
  ...
  async run(name, params = {}, context = {}) {
    ...
    try {
      ...
      this.activeCalls.delete(callId);
      Bot.makeLog('warn', `工作流执行失败[${name}]: ${error.message}`, 'WorkflowManager');
      return { 
        type: 'text', 
  ...
  async runMultiple(workflows, sharedParams = {}, context = {}) {
    ...
    try {
      ...
      Bot.makeLog('error', `多工作流调用失败: ${error.message}`, 'WorkflowManager');
      return [];
    }
  }
  ...
  async callStream(streamName, e, question, config = {}) {
    ...
    if (!stream) {
      Bot.makeLog('warn', `工作流 "${streamName}" 未找到`, 'WorkflowManager');
      return null;
    }

    try {
      return await AiWorkflowLoader.executeWorkflow(stream, e, question, config);
    } catch (error) {
      Bot.makeLog('error', 
