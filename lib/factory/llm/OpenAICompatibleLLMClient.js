  ...
  async chat(messages, overrides = {}) {
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      ...
      if (toolCalls?.length > 0 && enableMcpTools) {
        ...
        const toolResults = await MCPToolAdapter.handleToolCalls(toolCalls, overrides);
        currentMessages.push(...toolResults);
        didRunTools = true;
        continue;
      }

      const text = extractOpenAIAssistantText(message);
      Bot.makeLog(
        'debug',
        `[OpenAICompatible] chat 轮次=${round + 1} 无 tool_calls finish=${choice?.finish_reason ?? '-'} textLen=${text.length}`,
        'OpenAICompatible'
      );
