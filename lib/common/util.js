...
export default class BotUtil {
  ...
  /**
   * Create formatted log ID with gradient colors
  ...
  static makeLogID(id) {
    ...
    // Include chalk availability in cache key to prevent cached plain text being used after chalk loads
    const cacheKey = `logid_${id}_${cfg.bot?.log_color}_${cfg.bot?.log_id_filler}_chalk${chalkAvailable}`;
    ...
    if (!chalk || !chalkAvailable) {
      ...
      // Shorter cache time for plain text to allow chalk to be picked up sooner
      return BotUtil.cache(cacheKey, `[${plainContent}]`, 5000);
    }
    ...
    // Apply gradient coloring - handle all parts separately
    if (!isTruncated && leftPadCount > 0) {
      // Apply gradient to left padding
      const leftPadding = filler.repeat(leftPadCount);
      result += BotUtil.#applyGradientToString(leftPadding, idColors, chalk, true);
    }

    // Apply gradient to main content (ID or truncated ID)
    const mainContent = isTruncated ? fullContent : idStr;
    result += BotUtil.#applyGradientToString(mainContent, idColors, chalk, false);

    if (!isTruncated && rightPadCount > 0) {
      // Apply gradient to right padding
      const rightPadding = filler.repeat(rightPadCount);
      result += BotUtil.#applyGradientToString(rightPadding, idColors, chalk, true);
    }
  ...
  static #applyGradientToString(text, colors, chalk, lightMode = false) {
    ...
    // Ensure chalk is properly loaded
    if (typeof chalk.hex !== 'function') {
      console.warn('Chalk not properly loaded, returning plain text');
      return text;
    }
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      try {
        ...
        console.warn('Chalk coloring failed:', e);
        result += char;
      }
    }
  ...
  static #getDisplayWidth(str) {
    ...
    // Use for...of to properly handle Unicode characters
    for (const char of str) {
