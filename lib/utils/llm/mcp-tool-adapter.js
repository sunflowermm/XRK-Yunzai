...
export class MCPToolAdapter {
  // 缓存：streams 字符串 -> 工具名称列表，用于减少重复日志
  ...
  /**
   * 解析 convert 入参：数组为 stream 白名单；对象支持 streams、workflow/streamName、excludeStreams（与 OpenAI Responses 等对齐）
   */
  static _normalizeConvertStreamsArg(streamsOrOptions) {
    let list = [];
    let excludeStreams = [];
    if (Array.isArray(streamsOrOptions)) {
      list = streamsOrOptions.filter(Boolean).map(s => String(s).trim());
    } else if (streamsOrOptions && typeof streamsOrOptions === 'object') {
      const rawStreams = streamsOrOptions.streams;
      if (Array.isArray(rawStreams)) list = rawStreams.filter(Boolean).map(s => String(s).trim());
      const wf = streamsOrOptions.workflow ?? streamsOrOptions.streamName;
      if (wf) {
        const w = String(wf).trim();
        if (!list.includes(w)) list = [w, ...list];
      }
      if (Array.isArray(streamsOrOptions.excludeStreams)) {
        excludeStreams = streamsOrOptions.excludeStreams.filter(Boolean).map(s => String(s).trim());
      }
    }
    return { list, excludeStreams };
  }

  static _filterExcludedTools(tools, excludeStreams) {
    if (!excludeStreams?.length) return tools;
    return tools.filter(t => {
      const name = t?.name ?? '';
      const seg = name.split('.')[0];
      return !excludeStreams.some(ex => name.startsWith(`${ex}.`) || seg === ex);
    });
  }
  ...
  static convertMCPToolsToOpenAI(streamsOrOptions = []) {
    ...
    if (!mcpServer) return [];
    const { list, excludeStreams } = this._normalizeConvertStreamsArg(streamsOrOptions);
    const streamsKey = `${list.join(',')}|ex:${excludeStreams.slice().sort().join(',')}`;

    let mcpTools = list.length > 0
      ? (() => {
          const uniq = new Map();
          for (const s of list) {
            const streamName = String(s).trim();
            const tools = mcpServer.listTools(streamName, false); // 传入 false 表示不打印日志
      ...
      : mcpServer.listTools(null, false); // 传入 false 表示不打印日志

    mcpTools = this._filterExcludedTools(mcpTools, excludeStreams);
    ...
    if (cached !== toolNames) {
      ...
      BotUtil.makeLog('debug', `[MCPToolAdapter] 工具列表 streams=[${streamsKey}] 工具数=${mcpTools.length}`, 'MCPToolAdapter');
    }
  ...
  static async handleToolCalls(toolCalls, _options = {}) {
    ...
    const mcpServer = this.getMCPServer();
    if (!mcpServer) {
      BotUtil.makeLog('warn', '[MCP] handleToolCalls MCP服务未启用', 'MCPToolAdapter');
      return list.map(tc => ({
        role: 'tool',
        tool_call_id: tc.id,
    ...
    return Promise.all(list.map(async (tc) => {
      ...
      if (name !== openAiName) {
        BotUtil.makeLog('debug', `[MCPToolAdapter] 工具名映射 ${openAiName} -> ${name}`, 'MCPToolAdapter');
      }
      let args = {};
      if (tc.function?.arguments) {
        try {
      ...
      try {
        ...
        BotUtil.makeLog('error', `[MCP] handleToolCalls 失败 ${name}: ${err.message}`, 'MCPToolAdapter');
