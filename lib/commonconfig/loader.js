...
class ConfigLoader {
  ...
  async _getConfigDirs() {
    ...
    // 2. 从 plugins 下的每个子目录加载 commonconfig（每个插件可以有独立的配置）
    try {
    ...
    try {
      ...
      if (FileUtils.existsSync(coreDir)) {
        ...
        for (const entry of entries) {
          if (entry.isDirectory() && !entry.name.startsWith('.')) {
            const commonconfigDir = path.join(coreDir, entry.name, 'commonconfig');
            if (FileUtils.existsSync(commonconfigDir)) {
              dirs.push(commonconfigDir);
            }
          }
  ...
  _getConfigKey(filePath) {
    ...
    // 如果是core目录下的配置，包含模块名
    const coreMatch = filePath.match(/core[\/\\]([^\/\\]+)[\/\\]commonconfig[\/\\]/);
    if (coreMatch) {
      return `${coreMatch[1]}_${fileName}`;
    }
  ...
  async _loadConfig(filePath) {
    try {
      ...
      // 验证配置实例
      if (!ObjectUtils.isPlainObject(configInstance)) {
      ...
      if (existing) {
        ...
        const isCorePath = filePath.includes('/core/') || filePath.includes('\\core\\');
        const existingIsPlugin = existingPath.includes('/plugins/') || existingPath.includes('\\plugins\\');
        const existingIsCore = existingPath.includes('/core/') || existingPath.includes('\\core\\');
        
        // 优先级：插件 > 默认 > core
        if (isPluginPath && !existingIsPlugin) {
          BotUtil.makeLog('debug', `替换配置: ${key} (插件配置优先级更高)`, 'ConfigLoader');
        } else if (!isCorePath && existingIsCore) {
  ...
  getList() {
    ...
    for (const config of this.configs.values()) {
      if (config && ObjectUtils.isFunction(config.getStructure)) {
  ...
  clearAllCache() {
    for (const config of this.configs.values()) {
      if (ObjectUtils.isFunction(config.clearCache)) {
        config.clearCache();
      }
    }
