...
export class MCPServer {
  ...
  _toolResultToText(result) {
    ...
    if (result.success === false) {
      ...
      Bot.makeLog('debug', `[MCP] _toolResultToText success=false → ${out}`, 'MCPServer');
      return out;
    }
    if (typeof result.raw === 'string' && result.raw.trim()) {
      ...
      return capToolText(result.raw.trim());
    }
    const msg = result.message;
    const data = result.data;
    if (msg != null && msg !== '') {
      return capToolText(String(msg));
    }
    if (data != null) {
      const searchText = formatSearchResultsForModel(data);
      if (searchText) return searchText;
      if (typeof data === 'string') return capToolText(data);
      try {
        const str = JSON.stringify(data);
        if (str && str !== '{}') {
          Bot.makeLog('debug', `[MCP] _toolResultToText data JSON len=${str.length}`, 'MCPServer');
          return capToolText(str);
        }
      } catch (err) {
        Bot.makeLog('debug', `[MCP] _toolResultToText stringify failed: ${err?.message || err}`, 'MCPServer');
      }
    }
    try {
      const str = JSON.stringify(result);
      if (str && str !== '{}') {
        Bot.makeLog('debug', `[MCP] _toolResultToText 兜底 JSON len=${str.length}`, 'MCPServer');
        return capToolText(str);
      }
    } catch (err) {
      Bot.makeLog('debug', `[MCP] _toolResultToText stringify failed: ${err?.message || err}`, 'MCPServer');
    }
    Bot.makeLog('debug', '[MCP] _toolResultToText 无 msg/raw/data → 已执行', 'MCPServer');
    return '已执行';
  }
