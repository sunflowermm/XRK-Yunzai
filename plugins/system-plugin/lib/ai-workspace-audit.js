/**
 * AI 工作区审计日志（对齐 XRK-AGT ai-workspace-audit）
 */
...
function getAuditMaxEntries() {
  const n = Number(getAiWorkflowConfigOptional()?.workspace?.audit?.maxEntries)
...
export async function auditToolUse(workspaceId, tool, { ok = true, detail = '' } = {}) {
  ...
  try {
    FileUtils.ensureDirSync(AUDIT_DIR)
    await FileUtils.appendFile(file, line, 'utf8')
    const st = FileUtils.statSync(file)
    if (st?.size > MAX_AUDIT_BYTES) {
      const raw = await FileUtils.readFile(file, 'utf8')
      const lines = String(raw || '').trim().split('\n')
      await FileUtils.writeFile(file, `${lines.slice(-getAuditMaxEntries()).join('\n')}\n`, 'utf8')
    }
  } catch {
    /* 审计失败不阻断 */
  }
