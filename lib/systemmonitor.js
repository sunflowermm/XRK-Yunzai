    ...
    async detectBrowserProcesses() {
        ...
        try {
            if (platform === 'win32') {
                ...
                return this.parseWin32BrowserCsv(stdout).filter(p => this.isManagedBrowserProcess(p));
            }

            const command = platform === 'darwin'
                ? 'ps -ax -o pid=,etime=,command= | grep -E "(Chrome|Edge)" | grep -v grep | grep -E "(--headless|remote-debugging-port|puppeteer|playwright)"'
                : 'ps -eo pid=,etime=,cmd= | grep -E "(chrome|chromium|msedge)" | grep -v grep | grep -E "(--headless|remote-debugging-port|puppeteer|playwright)"';

            const { stdout } = await execAsync(command, { timeout: 15000, windowsHide: true });
            return this.parseBrowserProcesses(stdout, platform).filter(p => this.isManagedBrowserProcess(p));
        } catch (error) {
