...
description: 在 XRK-Yunzai 中开发或修改 AI 工作流时使用；涉及 plugins/*/workflow/、LLM 调用、MCP 工具、记忆与 function calling 时使用。
---
...
- **基类**：`lib/ai-workflow/aistream.js` · **加载**：`lib/ai-workflow/loader.js`
...
| 项 | 要求 |
|----|------|
| 路径 | **仅** `plugins/<插件名>/workflow/*.js`（不扫 `streams/`） |
...
| 配置 | `data/server_bots/<port>/ai-workflow.yaml` 或全局 `ai-workflow.yaml`；代码读 `getAiWorkflowConfigOptional()` |
