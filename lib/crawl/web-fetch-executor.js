...
function isMostlyBinaryText(text) {
...
function describeFirecrawlFallbackStatus(params) {
  if (!params.firecrawlApiKey) {
    return '未配置 aistream.crawl.webFetch.firecrawlApiKey，无法走 Firecrawl 回退。';
  }
  if (!params.firecrawlEnabled) {
    return '已配置 Firecrawl Key 但 firecrawlEnabled 为 false。';
  }
  return '已尝试 Firecrawl 回退但未返回正文（Key 无效、额度不足或目标站仍拦截）。';
}
...
async function tryFirecrawlFallback(params, url) {
  ...
  try {
    ...
    Bot.makeLog(
      'warn',
      `[web_fetch] Firecrawl 回退失败: ${error?.message || error}`,
      'WebFetch',
      error
    );
    return null;
...
export async function runWebFetch(params) {
  ...
  if (!res.ok) {
    ...
    const fallbackHint = describeFirecrawlFallbackStatus(params);
    let detail = formatWebFetchErrorDetail({
      detail: rawDetail,
      contentType: res.headers.get('content-type'),
      maxChars: DEFAULT_ERROR_MAX_CHARS
    });
    if (!detail || isMostlyBinaryText(detail) || isMostlyBinaryText(rawDetail)) {
      detail = res.statusText || `HTTP ${res.status}`;
    }
    throw new Error(
      `Web fetch failed (${res.status}): ${detail}. ${fallbackHint}`
    );
  }
