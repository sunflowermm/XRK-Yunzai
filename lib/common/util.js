...
export default class BotUtil {
  ...
  static makeLogID(id) {
    ...
    let fullContent = '';
    let contentType = 'normal'; // 'truncated', 'exact', or 'normal'
    ...
    if (displayWidth > targetLength) {
      ...
      // Create the full content string with proper padding
      const leftFiller = filler.repeat(leftPad);
      const rightFiller = filler.repeat(rightPad);
      fullContent = leftFiller + idStr + rightFiller;
      contentType = displayWidth === targetLength ? 'exact' : 'padded';
    }

    // Apply gradient to the entire content string
    if (contentType === 'padded') {
      // For padded content, we'll apply a lighter gradient to filler parts
      const chars = Array.from(fullContent);
      const leftPadChars = Array.from(filler.repeat(Math.floor((targetLength - displayWidth) / 2)));
      const idChars = Array.from(idStr);
      const rightPadChars = Array.from(filler.repeat(Math.ceil((targetLength - displayWidth) / 2)));

      let charIndex = 0;

      // Left padding (lighter colors)
      for (let i = 0; i < leftPadChars.length; i++) {
        const colorIndex = Math.floor((i / chars.length) * (idColors.length - 1));
        const lightColor = BotUtil.#lightenColor(idColors[colorIndex], 0.6);
        result += chalk.hex(lightColor)(leftPadChars[i]);
        charIndex++;
      }

      // Main ID (full gradient)
      result += BotUtil.#applyGradientToString(idStr, idColors, chalk, false);
      charIndex += idChars.length;

      // Right padding (lighter colors)
      for (let i = 0; i < rightPadChars.length; i++) {
        const colorIndex = Math.floor(((charIndex + i) / chars.length) * (idColors.length - 1));
        const lightColor = BotUtil.#lightenColor(idColors[colorIndex], 0.6);
        result += chalk.hex(lightColor)(rightPadChars[i]);
      }
    } else {
      // For truncated or exact fit, apply gradient to entire content
      result += BotUtil.#applyGradientToString(fullContent, idColors, chalk, false);
    }
  ...
  static #applyGradientToString(text, colors, chalk, lightMode = false) {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      // Apply gradient to all characters including Chinese
      result += chalk.hex(colorsToUse[colorIndex])(char);
    }
