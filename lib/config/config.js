...
class Cfg {
    ...
    if (this._port && FileUtils.existsSync(rendererMetaFile) && !this.watcher['renderer.meta']) {
      this.watcher['renderer.meta'] = HotReloadBase.createWatcher(rendererMetaFile, {
        onChange: () => {
          if (Date.now() - this._startTime < this._initGracePeriod) return;
          this._renderer = null;
          logger.mark('[修改渲染器选择配置][renderer.yaml]');
        }
      });
    }
  ...
  /** LLM 工厂配置 getter（与 XRK-AGT 对齐，供 LLMFactory 通过 global.cfg[configKey] 直接访问） */
  _getLlmConfig(name) {
    const d = this.getdefSet(name) || {};
    const s = this.getConfig(name) || {};
    return { ...d, ...s };
  }
  get gptgod_llm() { return this._getLlmConfig('gptgod_llm'); }
  get volcengine_llm() { return this._getLlmConfig('volcengine_llm'); }
  get xiaomimimo_llm() { return this._getLlmConfig('xiaomimimo_llm'); }
  get openai_llm() { return this._getLlmConfig('openai_llm'); }
  get gemini_llm() { return this._getLlmConfig('gemini_llm'); }
  get anthropic_llm() { return this._getLlmConfig('anthropic_llm'); }
  get azure_openai_llm() { return this._getLlmConfig('azure_openai_llm'); }
  get openai_compat_llm() { return this._getLlmConfig('openai_compat_llm'); }
  get openai_responses_compat_llm() { return this._getLlmConfig('openai_responses_compat_llm'); }
  get newapi_compat_llm() { return this._getLlmConfig('newapi_compat_llm'); }
  get cherryin_compat_llm() { return this._getLlmConfig('cherryin_compat_llm'); }
  get ollama_compat_llm() { return this._getLlmConfig('ollama_compat_llm'); }
  get gemini_compat_llm() { return this._getLlmConfig('gemini_compat_llm'); }
  get anthropic_compat_llm() { return this._getLlmConfig('anthropic_compat_llm'); }
  get azure_openai_compat_llm() { return this._getLlmConfig('azure_openai_compat_llm'); }

  /**
   * 获取 LLM 提供商配置（与 XRK-AGT 对齐：优先 LLMFactory.getProviderConfig，fallback 为 cfg[key_llm]）
   */
  getLLMConfig(provider) {
    ...
    try {
      ...
      logger.debug(`[LLM] getProviderConfig(${provider}) fallback: ${err?.message || err}`);
    }
    const key = `${String(provider).toLowerCase()}_llm`;
    return this[key] || {};
  }

  /** 获取 ASR 配置（用于 ASRFactory.createClient），provider 来自 aistream.yaml asr.Provider */
  getASRConfig() {
    const provider = String(this.aistream?.asr?.Provider || 'volcengine').toLowerCase();
    const def = this.getdefSet(`${provider}_asr`) || this.getdefSet('volcengine_asr') || {};
    const config = this.getConfig(`${provider}_asr`) || this.getConfig('volcengine_asr') || {};
    return { provider, ...def, ...config };
  }

  /** 获取 TTS 配置（用于 TTSFactory.createClient），provider 来自 aistream.yaml tts.Provider */
  getTTSConfig() {
    const provider = String(this.aistream?.tts?.Provider || 'volcengine').toLowerCase();
    const onlyForASR = this.aistream?.tts?.onlyForASR;
    const def = this.getdefSet(`${provider}_tts`) || this.getdefSet('volcengine_tts') || {};
    const config = this.getConfig(`${provider}_tts`) || this.getConfig('volcengine_tts') || {};
    return {
      provider,
      ...(onlyForASR !== undefined ? { onlyForASR } : {}),
      ...def,
      ...config
    };
  }

  /** 获取所有 LLM 配置（与 XRK-AGT 对齐：优先 LLMFactory.listProviders，fallback 为内置列表） */
  get llm() {
    try {
      const providers = LLMFactory?.listProviders?.() || [];
      const result = {};
      for (const p of providers) result[p] = this.getLLMConfig(p);
      return Object.keys(result).length ? result : this._getBuiltinLlmFallback();
    } catch (err) {
      logger.debug(`[LLM] 使用默认厂商列表: ${err?.message || err}`);
      return this._getBuiltinLlmFallback();
    }
  }

  _getBuiltinLlmFallback() {
    try {
      const keys = LLMFactory?.listBuiltinProviderKeys?.() ?? [];
      const r = {};
      for (const k of keys) r[k] = this.getLLMConfig(k);
      return r;
    } catch (err) {
      logger.debug(`[LLM] 内置厂商列表 fallback: ${err?.message || err}`);
      return {};
    }
  }
