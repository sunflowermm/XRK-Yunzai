...
class ApiLoader {
  ...
  async loadApi(filePath) {
    try {
      ...
      BotUtil.makeLog('debug', `加载API文件: ${key} (${normalizedPath})`, 'ApiLoader');
      ...
      if (!validateApiInstance(apiInstance, key)) return false;
      ...
      BotUtil.makeLog('error', `加载API失败: ${filePath} | ${detail}`, 'ApiLoader');
      return false;
    }
  }
  ...
  async register(app, bot) {
    ...
    for (const api of this.priority) {
      ...
      try {
        ...
        const detail = [error.stack].filter(Boolean).join('\n');
        BotUtil.makeLog('error', `注册API失败: ${apiName} - ${error.message}${detail ? '\n' + detail : ''}`, 'ApiLoader');
      }
    }
