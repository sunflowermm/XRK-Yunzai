...
import { LanguageBridge } from '../multiplugin/LanguageBridge.js'
 ...
 * 插件加载器 - 支持多语言插件系统
 * @class PluginsLoader
 * @description 负责加载、管理和执行JavaScript、Python等多语言插件
 */
class PluginsLoader {
  constructor() {
    // 插件存储
    this.priority = []              // 普通优先级插件
    ...
    // 多语言支持
    this.languageBridge = null
    this.languagePlugins = new Map()  // language -> plugins映射
  ...
  async initLanguageBridge() {
    try {
      ...
      this.languageBridge = new LanguageBridge({
        pluginsDir: path.join(process.cwd(), this.dir),
        languages: {
          python: {
            enabled: true,
            runtime: 'python3',
            venv: path.join(process.cwd(), 'venv')  // 使用已有虚拟环境
          }
        }
      })
      
      // 监听插件事件
      this.languageBridge.on('plugin-registered', this.onPluginRegistered.bind(this))
      this.languageBridge.on('runtime-error', this.onRuntimeError.bind(this))
      this.languageBridge.on('runtime-ready', this.onRuntimeReady.bind(this))
      
      // 初始化桥接器
      await this.languageBridge.initialize()
      this.bridgeReady = true
      
      logger.success('多语言桥接器初始化成功')
    } catch (error) {
      logger.error(`多语言桥接器初始化失败: ${error.message}`)
      this.bridgeReady = false
    }
  }
  ...
  /**
   * 插件注册事件处理
   * @param {Object} event - 插件注册事件
   */
  onPluginRegistered(event) {
    const { language, plugin } = event
    logger.debug(`注册${language}插件: [${plugin.key}][${plugin.name}]`)
    
    // 创建插件包装类
    const PluginWrapper = this.createPluginWrapper(language, plugin)
    ...
    targetArray.push(pluginEntry)
    
    // 记录到语言映射
    if (!this.languagePlugins.has(language)) {
      this.languagePlugins.set(language, [])
    }
    this.languagePlugins.get(language).push(pluginEntry)
    
    // 处理定时任务
    if (plugin.task?.length > 0) {
      this.registerTasks(plugin.task, plugin.key, language)
    }
  ...
  /**
   * 创建插件包装类
   * @param {string} language - 插件语言
   * @param {Object} plugin - 插件信息
   * @returns {Function} 插件包装类
   */
  createPluginWrapper(language, plugin) {
    const bridge = this.languageBridge
    const { key, name, priority, rule } = plugin
    
    return class PluginWrapper {
      constructor(e) {
        this.e = e
        this.name = name
        this.key = key
      ...
      async _callRemote(method, ...args) {
        try {
          const serializedE = this._serializeEvent(this.e)
          const result = await bridge.callPlugin(language, key, method, [serializedE, ...args])
          
          // 处理返回结果
          if (result && typeof result === 'object') {
            if (result.reply) {
              await this._handleReply(result)
              return true
            }
            if (result.action) {
              return await this._handleAction(result.action, result.data)
            }
          }
          
          return result
        } catch (error) {
          logger.error(`[${language}]插件调用失败 [${key}][${method}]: ${error.message}`)
          return false
        }
      }
       ...
       * @private
       */
      async _handleReply(result) {
        if (this.e?.reply) {
          await this.e.reply(result.reply, result.quote, {
            at: result.at,
            recallMsg: result.recallMsg
          })
        }
      }
      ...
      async _handleAction(action, data) {
        const actionHandlers = {
          sendMsg: () => this._sendMessage(data),
          uploadFile: () => this._uploadFile(data),
          setContext: () => this._setContext(data),
          getStorage: () => this._getStorage(data),
          setStorage: () => this._setStorage(data)
        }
      ...
      async _sendMessage(data) {
        if (data.group_id && this.e.bot) {
          return await this.e.bot.pickGroup(data.group_id).sendMsg(data.message)
        }
        if (data.user_id && this.e.bot) {
          return await this.e.bot.pickFriend(data.user_id).sendMsg(data.message)
        }
        return false
      }
       ...
       * @private
       */
      async _uploadFile(data) {
        if (data.file && this.e.reply) {
          return await this.e.reply(segment.file(data.file, data.name))
        }
        return false
      }
      ...
      async accept(e) {
        this.e = e
        return await this._callRemote('accept')
      }
      
      // 默认消息处理
      async handleNonMatchMsg(e) {
        this.e = e
        return await this._callRemote('handleNonMatchMsg')
      }
    }
  }
   ...
   */
  registerTasks(tasks, pluginKey, language) {
    tasks.forEach(t => {
      if (!t.cron || !t.fnc) return
      
      this.task.push({
        name: `${t.name || pluginKey}[${language}]`,
        cron: t.cron,
        fnc: async () => {
          try {
            const result = await this.languageBridge.callPlugin(
              language,
              pluginKey,
              t.fnc,
              []
            ...
            if (result?.message) {
              logger.info(`[${language}定时任务] ${result.message}`)
            }
          } catch (error) {
            logger.error(`[${language}]定时任务执行失败 [${pluginKey}][${t.fnc}]: ${error.message}`)
          }
        },
        log: t.log !== false
      })
  ...
  onRuntimeError(event) {
    const { language, error } = event
    logger.error(`[${language}]运行时错误: ${error.message}`)
    
    // 标记对应语言的插件为不可用
    const plugins = this.languagePlugins.get(language)
    if (plugins) {
      plugins.forEach(p => {
        p.available = false
      })
  ...
  async load(isRefresh = false) {
    ...
    // 初始化多语言桥接器
    if (!this.bridgeReady) {
      await this.initLanguageBridge()
    }
    
    // 加载JavaScript插件
    const jsFiles = await this.getJSPlugins()
    const packageErr = []
    ...
    for (const [lang, count] of Object.entries(stats)) {
      logger.success(`加载${lang}插件[${count}个]`)
    }
  }
  ...
  async getJSPlugins() {
    ...
    for (const dir of files) {
      if (!dir.isDirectory() || dir.name === 'python') continue
   ...
   */
  getPluginStats() {
    const stats = { javascript: 0 }
    
    for (const [lang, plugins] of this.languagePlugins) {
      stats[lang] = plugins.length
    }
    
    // 计算JS插件数
    stats.javascript = this.pluginCount - 
      Array.from(this.languagePlugins.values()).reduce((sum, p) => sum + p.length, 0)
  ...
  async deal(e) {
    try {
      ...
      // 运行插件
      await this.runPlugins(e, true)  // 扩展插件
      const handled = await this.runPlugins(e, false)  // 普通插件
    ...
    for (const plugin of plugins) {
      if (plugin?.accept) {
        try {
          ...
          logger.error(`插件 ${plugin.name} accept错误: ${error.stack}`)
        }
      }
    }
  ...
  async initPlugins(e, isExtended = false) {
    ...
    for (const p of pluginList) {
      ...
      try {
        ...
        logger.error(`初始化插件 ${p.name} 失败: ${error.message}`)
      }
    }
  ...
  async processRules(plugins, e) {
    ...
    for (const plugin of plugins) {
      ...
      for (const v of plugin.rule) {
        ...
        // 设置日志函数名
        const langTag = plugin.language !== 'javascript' ? `[${plugin.language}]` : ''
        e.logFnc = `[${plugin.name}]${langTag}[${v.fnc}]`
  ...
  async processDefaultHandlers(e) {
    for (const handler of this.defaultMsgHandlers) {
      try {
        ...
        plugin.e = e
        plugin.language = handler.language
  ...
  async checkBypassPlugins(e) {
    ...
    // 检查所有插件（包括Python插件）
    const allPlugins = [...this.priority, ...this.extended]
  ...
  addUtilMethods(e) {
    ...
    e.throttle = (key, duration = 1000) => {
      ...
      if (this.eventThrottle[throttleKey]) return false
  ...
  identifyDefaultMsgHandlers() {
    this.defaultMsgHandlers = [...this.priority, ...this.extended].filter(p => {
  ...
  sortPlugins() {
    ...
    const sortFn = (a, b) => {
      ...
      if (a.language !== b.language) {
        return a.language === 'javascript' ? -1 : 1
      }
      return 0
    }
    
    this.priority.sort(sortFn)
    this.extended.sort(sortFn)
  }
  ...
  checkDisable(p) {
    ...
    // Python插件的特殊处理
    if (p.language === 'python' && !this.pythonReady) {
      return false
    }
  ...
  async cleanup() {
    // 关闭语言桥接器
    if (this.languageBridge) {
      await this.languageBridge.shutdown()
    }
