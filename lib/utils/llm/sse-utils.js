...
function parseOpenAIChatChunk(data) {
  ...
  if (!json) throw new SyntaxError('invalid SSE JSON chunk');
 ...
 *
 * options:
 * - mcpToolMode: 'execute' | 'passthrough'
 *   - execute（默认）：仅在本地执行 MCP 工具，tool_calls 供本地消费
 *   - passthrough：同时通过 onDelta('', { tool_calls }) 透传上游模型产生的 tool_calls（例如给 OpenClaw）
 */
export async function consumeOpenAIChatStream(resp, onDelta, options = {}) {
  ...
  for await (const { data } of iterateSSE(resp)) {
    ...
    try {
      ...
      if (Array.isArray(toolCalls)) {
        ...
        // 在透传模式下，将本次增量的 tool_calls 直接通过 metadata 传给上层（例如 v3 handler），由上游客户端自行处理
