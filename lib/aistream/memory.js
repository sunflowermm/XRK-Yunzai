...
export class MemorySystem {
  ...
  async _redisCmd(promise, fallback) {
    try {
      ...
      Bot.makeLog('debug', `[Memory] Redis: ${err?.message || err}`, 'MemorySystem');
      return fallback;
    }
  }
  ...
  async remember({ ownerId, scene, layer = 'long', content, metadata = {}, authorId }) {
    ...
    Bot.makeLog('debug', `[记忆] (${normalizedLayer}) ${scene || 'global'}:${ownerId}: ${memory.content}`, 'MemorySystem');
    return memory;
  }
  ...
  async forget(ownerId, scene, memoryId = null, content = null) {
    ...
    if (memoryId) {
      ...
      for (const item of raw) {
        ...
        if (mem.id === memoryId) {
          ...
          Bot.makeLog('debug', `[记忆删除] ${scene}:${ownerId}: ${mem.content}`, 'MemorySystem');
          return true;
        }
      }
      return false;
    ...
    Bot.makeLog('debug', `[记忆清空] ${scene}:${ownerId}`, 'MemorySystem');
    return true;
  }
