...
class PluginsLoader {
  ...
  async load(isRefresh = false) {
    try {
      ...
      logger.success(`加载消息预处理器[${this.preprocessors.length}个]`)
  ...
  async deal(e) {
    try {
      ...
      // 【关键】运行消息预处理器（扩展插件的 accept 方法）
      const preprocessed = await this.runPreprocessors(e)
      if (preprocessed === 'return') return
    ...
    try {
      for (const p of this.preprocessors) {
        ...
        try {
          ...
          if (plugin.accept && this.checkDisable(plugin)) {
            ...
            // 注意：不 break，让所有预处理器都有机会处理
          }
        } catch (error) {
          logger.error(`消息预处理器 ${p.name} 执行错误`)
          logger.error(error)
        }
      }
      return false
    } catch (error) {
      logger.error('运行消息预处理器错误')
      logger.error(error)
      return false
    }
  }
  ...
  /**
   * 识别消息预处理器
   * 扩展插件中包含 accept 方法的会被识别为预处理器
   */
  identifyPreprocessors() {
    this.preprocessors = this.extended.filter(p => {
      if (!p?.class) return false
      try {
        const instance = new p.class()
        return typeof instance.accept === 'function'
      } catch {
        return false
      }
    })
  ...
  async dealStdinEvent(e) {
    try {
      ...
      // 运行预处理器
      const preprocessed = await this.runPreprocessors(e)
      if (preprocessed === 'return') return true
