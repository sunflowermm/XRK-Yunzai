...
class AiWorkflowLoader {
  ...
  /** Bot.on('config:write')：保存 / 监视改文件后按勾选同步远程 MCP */
  ...
  /** 从 defaultWorkflows + AI 助手 mergeWorkflows（含群覆盖）收集已勾选 remote-mcp */
  async _collectSelectedRemoteMcpNames() {
    const lists = [];
    lists.push(getAiWorkflowConfigOptional()?.mcp?.defaultWorkflows);
    try {
      const cm = globalThis.Bot?.ConfigManager || globalThis.ConfigManager;
      let inst = cm?.get?.('ai_config') || cm?.get?.('system-plugin_ai_config');
      if (!inst && typeof cm?.getAll === 'function') {
        for (const [key, conf] of cm.getAll()) {
  ...
  /**
   * 按配置勾选同步远程 MCP（启动 / Bot config:write）
   * 勾选才挂；取消勾选则卸载。
   */
  ...
  async _syncSelectedRemoteMCPInner() {
    ...
    Bot.makeLog('info', `远程 MCP 同步挂载: [${[...wanted].join(', ')}]`, 'AiWorkflowLoader');
    for (const name of wanted) {
      if (this.remoteMCPServers.has(name)) continue;
      try {
        await this._mountRemoteMCP(name);
        const n = [...this.mcpServer.tools.keys()].filter((t) => t.startsWith(`remote-mcp.${name}.`)).length;
        Bot.makeLog('info', `远程 MCP[${name}] 已挂载（${n} 工具）`, 'AiWorkflowLoader');
      } catch (e) {
  ...
  /**
   * 挂载名单中尚未启动的远程 MCP（HTTP 请求临时白名单用；常态走 syncSelectedRemoteMCP）
