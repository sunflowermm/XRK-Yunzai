...
export default class BotUtil {
  ...
  static makeLogID(id) {
    ...
    if (displayWidth > targetLength) {
      ...
      let truncated = '';
      let currentWidth = 0;
      const maxWidth = targetLength - 2; // 为 '..' 预留空间
    ...
    // 应用渐变色到整个内容
    if (isTruncated || displayWidth === targetLength) {
      // 对截断或正好合适的内容直接应用渐变
      result += BotUtil.#applyGradientToString(fullContent, idColors, false);
    } else {
      // 对有填充的内容分三部分处理
      if (leftPadCount > 0) {
        // 左填充渐变（使用浅色）
        const leftPadding = filler.repeat(leftPadCount);
        result += BotUtil.#applyGradientToString(leftPadding, idColors, true, 0);
      }

      // 主内容渐变（使用正常颜色）
      result += BotUtil.#applyGradientToString(idStr, idColors, false, 1);

      if (rightPadCount > 0) {
        // 右填充渐变（使用浅色）
        const rightPadding = filler.repeat(rightPadCount);
        result += BotUtil.#applyGradientToString(rightPadding, idColors, true, 2);
      }
    }
   ...
   * @param {boolean} [lightMode=false] - 使用浅色模式
   * @param {number} [section=1] - 区段标识（0=左，1=中，2=右）
   * @returns {string} 着色后的字符串
   */
  static #applyGradientToString(text, colors, lightMode = false, section = 1) {
    if (!text || !colors?.length) return text;
    ...
    if (chars.length === 1) {
      // 单字符时使用中间颜色
      const middleIndex = Math.floor(colorsToUse.length / 2);
      return chalk.hex(colorsToUse[middleIndex])(text);
    }
    ...
    if (section === 0) {
      ...
      for (let i = 0; i < chars.length; i++) {
        ...
        const colorIndex = Math.round(progress * (colorsToUse.length - 1));
        const color = colorsToUse[Math.min(colorIndex, colorsToUse.length - 1)];
      ...
      const step = (colorsToUse.length - 1) / Math.max(1, chars.length - 1);
      for (let i = 0; i < chars.length; i++) {
        const progress = 1 - (i / Math.max(1, chars.length - 1));
        const colorIndex = Math.round(progress * (colorsToUse.length - 1));
        const color = colorsToUse[Math.min(colorIndex, colorsToUse.length - 1)];
        result += chalk.hex(color)(chars[i]);
      }
      ...
      if (chars.length === 2) {
        // 两个字符时使用首尾颜色
        result += chalk.hex(colorsToUse[0])(chars[0]);
        result += chalk.hex(colorsToUse[colorsToUse.length - 1])(chars[1]);
      } else {
        // 多字符时平滑渐变
        for (let i = 0; i < chars.length; i++) {
          const progress = i / (chars.length - 1);
          const exactIndex = progress * (colorsToUse.length - 1);
          ...
          if (lowerIndex === upperIndex) {
            color = colorsToUse[lowerIndex];
          } else {
            // 在两个颜色之间插值
            color = BotUtil.#interpolateColor(
              colorsToUse[lowerIndex],
              colorsToUse[upperIndex],
              fraction
            );
          }
   ...
   */
  static #interpolateColor(color1, color2, fraction) {
    // 移除 # 号
