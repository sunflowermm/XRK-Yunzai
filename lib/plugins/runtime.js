...
/**
 * 将 game 扩展的方法/属性代理到 e.runtime，兼容 TRSS-Yunzai 插件对 e.runtime.getMysApi 等的直接调用
 * @param {Runtime} runtime
 * @returns {Runtime}
 */
export function createGameCompatibilityProxy(runtime) {
  const gameExt = runtime.getExtension?.('game')
  if (!gameExt) return runtime
  ...
  return new Proxy(runtime, {
    get(target, prop) {
      ...
      if (prop in gameExt) {
        ...
        if (typeof value === 'function') return value.bind(gameExt)
        return value
      }
...
export default class Runtime {
  ...
  async render(plugin, path, data = {}, cfg = {}) {
    ...
    // 自动计算 pluResPath；path 派生字段须在 ...data 之后，避免 screenData 里的旧 tplFile 覆盖
    let pluResPath = `../../../${lodash.repeat("../", paths.length)}plugins/${plugin}/resources/`
    const saveId = data.saveId || data.save_id || paths[paths.length - 1]
  ...
  static async init(e) {
    ...
    if (e.runtime.getExtension('game')) {
      e.runtime = createGameCompatibilityProxy(e.runtime)
    }
