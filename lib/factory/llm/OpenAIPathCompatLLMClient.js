...
export default class OpenAIPathCompatLLMClient {
  ...
  async _executeToolCalls(toolCalls, overrides = {}, onDelta) {
    ...
    const toolResults = await MCPToolAdapter.handleToolCalls(normalizedToolCalls, overrides);
  ...
  async _runWithToolRounds(initialMessages, overrides = {}, handlers = {}) {
    ...
    Bot.makeLog('warn', `[OpenAIPathCompatLLMClient][${this._label}] 达到最大工具调用轮数: ${maxToolRounds}`, 'LLMFactory');
    return packToolRoundsExhausted(state.replyTracker);
  }
