...
import { normalizeVolcengineImageUrl } from '../../utils/llm/image-utils.js';
...
export default class VolcengineLLMClient {
  ...
  async transformMessages(messages) {
    const openaiMessages = await transformOpenAIStyleVisionMessages(messages, this.config);
    for (const msg of openaiMessages) {
      if (msg?.role !== 'user' || !Array.isArray(msg.content)) continue;
      for (const part of msg.content) {
        if (part?.type === 'image_url' && part.image_url?.url) {
          part.image_url.url = await normalizeVolcengineImageUrl(part.image_url.url, this.config);
        }
      }
    }
  ...
  async chat(messages, overrides = {}) {
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      ...
      if (message.tool_calls?.length > 0) {
        noteLlmChatToolCallsForReply(replyTracker, message.tool_calls);
        currentMessages.push(message);
        currentMessages.push(...await MCPToolAdapter.handleToolCalls(message.tool_calls));
        continue;
      }

      return finalizeLlmChatNonStreamContent(replyTracker, message.content || '');
    }

    return finalizeLlmChatNonStreamContent(
      replyTracker,
