  ...
  async ensureRemoteMCPServers(streamNames = []) {
    ...
    for (const name of wanted) {
      ...
      try {
        ...
        const toolCount = [...(this.mcpServer?.tools?.keys?.() || [])]
          .filter((t) => t.startsWith(`remote-mcp.${name}.`)).length;
        Bot.makeLog('info', `ensureRemoteMCP: ${name} 挂载成功（工具 ${toolCount} 个）`, 'AiWorkflowLoader');
      } catch (e) {
