...
export default class AIStream {
  ...
  constructor(options = {}) {
    ...
    const L = getAistreamConfigOptional().llm || {};
  ...
  async finalizeSystemPromptContent(text) {
    ...
    return appendAgentWorkspaceToPrompt(text, getAistreamConfigOptional(), streamKey);
  }
  ...
  _appendRemoteMcpStreamNames(names) {
    try {
      ...
      Bot.makeLog('debug', `读取远程 MCP 流名失败: ${err?.message || err}`, 'AIStream');
    }
  }
  ...
  _getToolStreamNames() {
    ...
    if (this._mergedStreams && Array.isArray(this._mergedStreams)) {
      Bot.makeLog('debug', `[AIStream] _getToolStreamNames (合并工作流 ${this.name}): [${names.join(', ')}]`, 'AIStream');
    }
    return names;
  }
    ...
    try {
      ...
      if (e && e.isGroup && cleanText) {
        try {
          ...
          Bot.makeLog('debug', `记录Bot回复失败: ${error.message}`, 'AIStream');
        }
      }
