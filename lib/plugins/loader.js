...
import MultiPlugin from '../multiplugin/MultiPlugin.js'
import { PythonRuntime } from '../multiplugin/PythonRuntime.js'
...
class PluginsLoader {
  constructor() {
    ...
    this.msgThrottle = {}           // 消息节流
    this.eventThrottle = {}         // 事件节流
   ...
   * 初始化Python运行时
   */
  async initPythonRuntime() {
    try {
      logger.info('初始化Python运行时...')
      ...
      // 注册到多语言插件管理器
      this.multiPlugin.registerRuntime('python', this.pythonRuntime)
      
      // 监听插件加载事件
      this.multiPlugin.on('plugin-event', (event) => {
        if (event.type === 'plugin_loaded' && event.language === 'python') {
          this.registerPythonPlugin(event.data)
        }
      })
      
      // 监听Python运行时错误
      this.multiPlugin.on('runtime-error', (event) => {
        if (event.language === 'python') {
          logger.error(`Python运行时错误: ${event.error.message}`)
          this.pythonReady = false
        }
      })
      
      // 启动Python运行时
      await this.pythonRuntime.start()
      this.pythonReady = true
      
      logger.success('Python运行时初始化成功')
    } catch (error) {
      logger.error(`Python运行时初始化失败: ${error.message}`)
      this.pythonReady = false
    }
  }
   ...
   * 注册Python插件
   */
  registerPythonPlugin(pluginData) {
    const { key, name, priority, rule, task } = pluginData
    
    logger.debug(`注册Python插件 [${key}][${name}]`)
    
    // 创建插件包装类
    class PythonPluginWrapper {
      constructor(e) {
        this.e = e
        this.name = name
        this.key = key
      ...
      // 动态创建方法
      async _callPython(method, ...args) {
        try {
          // 序列化事件数据
          const serializedE = this._serializeEvent(this.e)
          
          const result = await MultiPlugin.callPlugin(
            'python',
            key,
            method,
            [serializedE, ...args]
          ...
          if (result && typeof result === 'object') {
            // 处理回复
            if (result.reply && this.e && this.e.reply) {
          ...
          logger.error(`Python插件调用失败 [${key}][${method}]: ${error.message}`)
          return false
        }
      }
      ...
      async _handleAction(action, data) {
        switch (action) {
          ...
          case 'setContext':
            ...
            break
          default:
            logger.debug(`未知的Python插件动作: ${action}`)
        }
        return false
      }
      ...
      _serializeEvent(e) {
        ...
        return {
          ...
          member: e.member ? {
            ...
            is_admin: e.member.is_admin,
            is_owner: e.member.is_owner
          } : null,
      ...
      async accept(e) {
        this.e = e
        return await this._callPython('accept')
      }
      
      async handleNonMatchMsg(e) {
        this.e = e
        return await this._callPython('handleNonMatchMsg')
      }
    }
    ...
    if (rule && Array.isArray(rule)) {
      rule.forEach(r => {
        if (r.fnc && !PythonPluginWrapper.prototype[r.fnc]) {
          PythonPluginWrapper.prototype[r.fnc] = async function(e) {
            this.e = e
            return await this._callPython(r.fnc)
          }
        }
      ...
      task.forEach(t => {
        if (t.cron && t.fnc) {
          this.task.push({
            ...
            fnc: async () => {
              try {
                const result = await MultiPlugin.callPlugin('python', key, t.fnc, [])
                if (result && result.message) {
                  logger.info(`[Python定时任务] ${result.message}`)
                }
              } catch (error) {
                logger.error(`Python定时任务执行失败 [${key}][${t.fnc}]: ${error.message}`)
              }
            },
            log: t.log !== false
          })
    ...
    const pluginEntry = {
      ...
      key,
      name,
      priority: priority === 'extended' ? 0 : (priority ?? 50),
      plugin: new PythonPluginWrapper(),
      language: 'python',
      bypassThrottle: pluginData.bypassThrottle === true
    }
    
    // 添加到对应列表
    if (priority === 'extended') {
      this.extended.push(pluginEntry)
    } else {
      this.priority.push(pluginEntry)
    }
    
    // 保存到Python插件映射
    this.pythonPlugins.set(key, pluginEntry)
    this.pluginCount++
    
    // 注册到多语言管理器
    this.multiPlugin.registerPlugin(pluginData)
  }
  ...
  async load(isRefresh = false) {
    ...
    const pythonCount = this.pythonPlugins.size
    const jsCount = this.pluginCount - pythonCount

    logger.success(`加载定时任务[${this.task.length}个]`)
    logger.success(`加载JavaScript插件[${jsCount}个]`)
    if (this.pythonReady) {
      logger.success(`加载Python插件[${pythonCount}个]`)
    }
  ...
  async runPlugins(e, isExtended = false) {
    ...
    // 处理扩展插件 - 直接运行，不进行其他检查
    if (isExtended) {
      const handled = await this.processPlugins(plugins, e, true)
      return handled
    }
  ...
  async processPlugins(plugins, e, isExtended) {
    ...
    if (!Array.isArray(plugins)) {
      logger.error(`processPlugins: plugins不是数组, type=${typeof plugins}`)
      return false
    }
  ...
  async processRules(plugins, e) {
    // 确保plugins是数组
    if (!Array.isArray(plugins)) {
      logger.error(`processRules: plugins不是数组, type=${typeof plugins}`)
      return false
    }
    ...
    for (const plugin of plugins) {
      ...
      for (const v of plugin.rule) {
        ...
        // 设置日志函数名
        const langTag = plugin.language === 'python' ? '[Python]' : ''
  ...
  async cleanup() {
    if (this.pythonRuntime) {
      await this.pythonRuntime.shutdown()
    }
    
    await this.multiPlugin.shutdown()
