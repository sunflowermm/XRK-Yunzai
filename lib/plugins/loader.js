...
class PluginsLoader {
  ...
  /** @returns {string} plugins 根目录绝对路径 */
  get pluginsRoot() {
  ...
  _importPluginUrl(absPath, cacheBust = true) {
    return FileUtils.toImportUrl(absPath, { cacheBust })
  }

  async load(isRefresh = false) {
    try {
      if (!isRefresh && this.loaded) return
    ...
    try {
      ...
      if (e?.isDevice && typeof e?.reply === 'function') {
        e.reply('处理出错: ' + (error?.message || '未知错误')).catch((err) => {
          logger.debug(`设备端错误回复失败: ${err?.message || err}`)
        })
      }
    }
  }
  ...
  async runPluginsAndHandle(e, opts = {}) {
    // TRSS：星铁 / 绝区零命令前缀（Runtime.init 之后）
    if (e?.msg) {
    ...
    if (!handled) {
      ...
      if (opts.replyUnhandled && typeof e.reply === 'function') {
        e.reply('暂无插件处理该指令').catch((err) => {
          logger.debug(`未处理指令回复失败: ${err?.message || err}`)
        })
      }
    }
    return handled
  ...
  async runPlugins(e, isExtended = false) {
    try {
      ...
      const handled = await this.processPlugins(plugins, e, false)
      ...
      try {
        if (typeof media === 'string') {
          if (media.startsWith('http')) {
            ...
            return await FileUtils.readFileBuffer(media)
  ...
  async getPlugins() {
    try {
      ...
      logger.error(`插件目录扫描异常（${this.dir}）: ${error?.message || error}，将按「无插件」继续启动`, error)
      return []
    }
  }
  ...
  _logPluginLoadSummary() {
    ...
    logger.warn(`[Loader] 共 ${failed.length} 个插件加载失败: ${names}`)
    failed.forEach(p => {
      if (p.error) logger.debug(`  - ${p.name}: ${p.error}`)
    })
  }
  ...
  /**
   * 导入插件模块（与 XRK-AGT 对齐：分离模块加载与依赖错误收集）
  ...
  async importPluginModule(file, packageErr) {
    try {
      ...
      const app = await import(this._importPluginUrl(absPath))
      return app.apps ? { ...app.apps } : app
    } catch (error) {
      if (error.stack?.includes('Cannot find package')) {
        packageErr.push({ error, file })
      } else {
        logger.warn(`加载插件模块错误: ${file.name} - ${error.message}`)
      }
      return {}
    }
  }
  ...
  _unregisterPlugin(key) {
    ...
    for (const entry of entries) {
      ...
      entry.unsubscribes?.forEach((off) => {
        try { off() } catch (err) {
          logger.debug(`取消事件订阅失败 [${key}]: ${err?.message || err}`)
        }
      })

      if (typeof plugin?.destroy === 'function') {
        Promise.resolve(plugin.destroy()).catch((err) => {
          logger.debug(`插件 destroy 失败 [${key}]: ${err?.message || err}`)
        })
      }
    }
  ...
  watch(dirName, appName) {
    ...
    try {
      this.watcher[watchKey] = HotReloadBase.createWatcher(file, {
        ...
        onError: (error) => logger.error(`文件监听错误 [${watchKey}]`, error)
      }, {
        debounceMs: 500,
  ...
  watchDir(dirName) {
    ...
    try {
      ...
      this.watcher[dirName] = HotReloadBase.createWatcher(dirPath, {
          ...
          onError: (error) => logger.error(`目录监听错误 [${dirName}]`, error)
        }, {
          debounceMs: 500,
          loggerName: 'PluginsLoader'
        })
