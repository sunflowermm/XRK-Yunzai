...
class PluginsLoader {
  ...
  async deal(e) {
    try {
      if (!e) return
      this.initEvent(e)
      if (this.isSpecialEvent(e)) return await this.dealSpecialEvent(e)
  ...
  async dealStdinEvent(e) {
    try {
      ...
      if ((e.msg === '' || e.msg === undefined) && e.raw_message) e.msg = this.dealText(String(e.raw_message).trim())
      this.setupReply(e)
      await this.runPlugins(e, true)
      const handled = await this.runPlugins(e, false)
      if (!handled) logger.trace(`${e.logText} 暂无插件处理`)
      return true
    } catch (error) {
      logger.error(`处理${e.adapter}事件错误`, error)
      return false
    }
  }
  ...
  async dealDeviceEvent(e) {
    try {
      ...
      if (e.event_type === 'message' || e.event_data?.message || e.event_data?.text) {
        ...
        await this.dealMsg(e)
        if ((!e.msg || e.msg === '') && e.event_data?.text) e.msg = this.dealText(String(e.event_data.text))
      }
      await this.runPlugins(e, true)
      const handled = await this.runPlugins(e, false)
      if (!handled) {
        logger.trace(`${e.logText} 暂无插件处理`)
        if (typeof e.reply === 'function') e.reply('暂无插件处理该指令').catch(() => {})
      }
    } catch (error) {
      logger.error('处理设备事件错误', error)
      if (typeof e.reply === 'function') e.reply('处理出错: ' + (error.message || '未知错误')).catch(() => {})
    }
  }
  ...
  /**
   * 过滤事件：插件/规则 v.event 与事件 e 是否匹配（仅用 path + matched 集合，无特化分支）
   */
  filtEvent(e, v) {
    ...
    return events.some(evt => {
      ...
      if (path && (evt === path || (evt.endsWith('*') && path.startsWith(evt.slice(0, -1))))) return true
      return false
    })
  }
