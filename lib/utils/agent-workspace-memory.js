 ...
 * @param {string} [target] - MEMORY | today | group | user | memory/… 相对路径
 * @param {{ groupId?: string|number, userId?: string|number }} [scope]
 */
export function resolveMemoryAbs(workspaceRoot, target = 'MEMORY', scope = {}) {
  const raw = String(target ?? 'MEMORY').trim();
  let safeRel = raw.replace(/\\/g, '/').replace(/^\/+/, '');
  if (!safeRel.startsWith(`${MEMORY_DIR_REL}/`)) {
    safeRel = resolveMemoryRel(raw, scope);
  }
  if (!isAllowedMemoryRel(safeRel)) {
    throw new Error(`不允许的记忆路径: ${target}`);
  }
...
export async function readWorkspaceMemory(workspaceRoot, target = 'MEMORY', scope = {}) {
  const { abs, rel } = resolveMemoryAbs(workspaceRoot, target, scope);
