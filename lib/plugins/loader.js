...
import { PythonBridge } from '../multiplugin/PythonBridge.js'
...
class PluginsLoader {
  ...
  async initPythonBridge() {
    try {
      ...
      this.pythonBridge = new PythonBridge({
        ...
        pluginsDir: path.join(process.cwd(), 'plugins/python')
      })

      // 监听插件注册事件
      this.pythonBridge.on('plugin_registered', (pluginInfo) => {
        this.registerPythonPlugin(pluginInfo)
      })
   ...
   */
  registerPythonPlugin(pluginInfo) {
    const { key, name, priority = 50, rule = [], task = [] } = pluginInfo
    ...
    class PythonPluginWrapper {
      ...
      async _callPython(method, ...args) {
        try {
          const event = this._serializeEvent(this.e)
          const result = await this.pythonBridge.callPlugin(key, method, [event, ...args])
    ...
    task.forEach(t => {
      if (t.cron && t.fnc) {
        this.task.push({
          ...
          fnc: async () => {
            try {
              const result = await this.pythonBridge.callPlugin(key, t.fnc, [])
              if (result?.message) {
    ...
    if (Array.isArray(e.message)) {
      for (const val of e.message) {
        if (val.type === 'text') {
          ...
          e.img.push(val.url || val.file)
        } else if (val.type === 'at' && val.qq == e.bot?.uin) {
          e.atBot = true
        }
      }
    }
   ...
   */
  async runPlugins(e) {
    const plugins = [...this.priority, ...this.extended]
    ...
    for (const p of plugins) {
      ...
      try {
        ...
        for (const rule of plugin.rule) {
          if (rule.reg && !rule.reg.test(e.msg)) continue

          const method = plugin[rule.fnc]
          if (typeof method === 'function') {
            const res = await method.call(plugin, e)
            if (res !== false) {
              logger.mark(`[${plugin.name}][${rule.fnc}] 处理完成`)
              return
            }
          }
        }
      } catch (error) {
        logger.error(`插件执行错误: ${error.message}`)
      }
    }
  }
