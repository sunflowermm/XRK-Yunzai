...
/**
 * CherryIN 协议兼容 LLM 工厂配置（与 XRK-AGT 对齐）
 * 配置文件：data/server_bots/{port}/cherryin_compat_llm.yaml
 * CherryIN / OpenAI-like 兼容运营商集合，每项 key 作为 provider；baseUrl/path/apiKey/model 等。
 */
export default class CherryINCompatibleLLMConfig extends ConfigBase {
  constructor() {
    super({
      ...
      description: 'CherryIN / OpenAI-like 兼容运营商集合：每项 key 为独立 provider，baseUrl/path/apiKey/model 等',
      ...
      schema: {
        fields: {
          providers: {
            ...
            description: '每个元素为一家 CherryIN / OpenAI-like 运营商，key 需唯一且供 aistream 与前端引用',
            component: 'ArrayForm',
            itemType: 'object',
            default: [],
            itemSchema: {
