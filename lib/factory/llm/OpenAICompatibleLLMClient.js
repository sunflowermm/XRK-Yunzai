...
import {
  createLlmChatReplyTracker,
  finalizeLlmChatNonStreamContent,
  noteLlmChatToolCallsForReply
} from '../../utils/llm/llm-chat-result.js';
...
export default class OpenAICompatibleLLMClient {
  ...
  async chat(messages, overrides = {}) {
    ...
    const replyTracker = createLlmChatReplyTracker();
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      ...
      if (toolCalls?.length > 0 && enableMcpTools) {
        noteLlmChatToolCallsForReply(replyTracker, toolCalls);
      ...
      if (text != null && text.trim() !== '') {
        ...
        return finalizeLlmChatNonStreamContent(replyTracker, text);
      }
      BotUtil.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} 无有效 text 继续`, 'OpenAICompatible');
    }
      BotUtil.makeLog('debug', `[OpenAICompatible] chat 结束 didRunTools=${didRunTools} usedReplyTool=${replyTracker.usedReplyTool}`, 'OpenAICompatible');
    return finalizeLlmChatNonStreamContent(replyTracker, '');
  }
