...
export default class OpenAIPathCompatLLMClient {
  ...
  async _consumeSSEWithToolCalls(resp, onDelta) {
    ...
    for await (const { data } of iterateSSE(resp)) {
      try {
        ...
        BotUtil.makeLog('warn', `[OpenAIPathCompatLLMClient][${this._label}] SSE JSON解析失败: ${e.message}`, 'LLMFactory');
      }
    }
  ...
  async _runWithToolRounds(initialMessages, overrides = {}, handlers = {}) {
    ...
    const state = { messages: [...initialMessages], replyTracker: createLlmChatReplyTracker() };
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      ...
      if (!toolCalls.length) {
        return finalizeLlmChatNonStreamContent(state.replyTracker, roundResult?.content || '');
      }

      noteLlmChatToolCallsForReply(state.replyTracker, toolCalls);
