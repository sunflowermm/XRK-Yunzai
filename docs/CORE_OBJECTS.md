...
- **多端口隔离（核心）**: 启动选端口；配置在 `data/server_bots/<port>/`；同机可多进程多业务。单端口进程内仍可挂多个协议 Bot（`Bot.uin`），见 [VS_YUNZAI.md](./VS_YUNZAI.md)
...
| `cfg.aiWorkflow` | `Object` | AI 工作流配置（`ai-workflow.yaml` 合并结果）；**`lib/` 内请用 `getAiWorkflowConfigOptional()`** |
