...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  get fl() {
    ...
    for (const bot_id of this.uin) {
      ...
      // 检查 bot.fl 是否存在且可迭代
      if (!bot?.fl || typeof bot.fl[Symbol.iterator] !== 'function') continue;
      ...
      try {
        ...
        BotUtil.makeLog("debug",
          `Bot ${bot_id} 的好友列表不可迭代: ${err.message}`, 'Server');
        continue;
      }
    }
  ...
  get gl() {
    ...
    for (const bot_id of this.uin) {
      const bot = this.bots[bot_id];
      // 检查 bot.gl 是否存在且可迭代
      if (!bot?.gl || typeof bot.gl[Symbol.iterator] !== 'function') continue;
      ...
      try {
        ...
        BotUtil.makeLog("debug",
          `Bot ${bot_id} 的群列表不可迭代: ${err.message}`, 'Server');
        continue;
      }
    }
  ...
  get gml() {
    ...
    for (const bot_id of this.uin) {
      const bot = this.bots[bot_id];
      // 检查 bot.gml 是否存在且可迭代
      if (!bot?.gml || typeof bot.gml[Symbol.iterator] !== 'function') continue;
      ...
      try {
        ...
        BotUtil.makeLog("debug",
          `Bot ${bot_id} 的群成员列表不可迭代: ${err.message}`, 'Server');
        continue;
      }
    }
