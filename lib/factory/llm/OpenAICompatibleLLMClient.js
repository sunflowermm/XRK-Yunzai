 ...
 * - 统一多模态：`prepareOpenAIChatVisionMessages`（OpenAI 风格结构 + 图片内联），避免上游代拉 URL / MIME 问题
...
export default class OpenAICompatibleLLMClient {
  ...
  async chat(messages, overrides = {}) {
    const transformedMessages = await prepareOpenAIChatVisionMessages(messages, this.config, { timeoutMs: this.timeout });
    Bot.makeLog('debug', `[OpenAICompatible] chat 入口 messagesLen=${messages?.length} transformedLen=${transformedMessages?.length}`, 'OpenAICompatible');
    ...
    for (let round = 0; round < maxToolRounds; round++) {
      Bot.makeLog('debug', `[OpenAICompatible] chat 轮次 round=${round + 1}/${maxToolRounds} currentMessagesLen=${currentMessages.length}`, 'OpenAICompatible');
      ...
      if (!message) {
        Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} 无 message 退出`, 'OpenAICompatible');
        break;
      }
      ...
      if (toolCalls?.length > 0 && enableMcpTools) {
        noteReplyFromModelCalls(replyTracker, toolCalls);
        const names = toolCalls.map(t => t?.function?.name).filter(Boolean).join(',');
        Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} tool_calls count=${toolCalls.length} names=[${names}]`, 'OpenAICompatible');
      ...
      if (role !== 'assistant') {
        Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} role=${role} 非 assistant 跳过`, 'OpenAICompatible');
        continue;
      }
      if (content == null) {
        Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} content=null 跳过`, 'OpenAICompatible');
        continue;
      }
      ...
      if (text != null && text.trim() !== '') {
        Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} 返回正文 textLen=${text.length} text=${text}`, 'OpenAICompatible');
        return packNonStreamReturn(replyTracker, text);
      }
      Bot.makeLog('debug', `[OpenAICompatible] chat 轮次=${round + 1} 无有效 text 继续`, 'OpenAICompatible');
    }
      Bot.makeLog('debug', `[OpenAICompatible] chat 结束 didRunTools=${didRunTools} usedReplyTool=${replyTracker.usedReplyTool}`, 'OpenAICompatible');
    if (didRunTools && !replyTracker.usedReplyTool) {
      return packToolRoundsExhausted(replyTracker);
    }
    return packNonStreamReturn(replyTracker, '');
  }

  async chatStream(messages, onDelta, overrides = {}) {
    let currentMessages = await prepareOpenAIChatVisionMessages(messages, this.config, { timeoutMs: this.timeout });
    Bot.makeLog('debug', `[OpenAICompatible] chatStream 入口 messagesLen=${messages?.length} currentMessagesLen=${currentMessages?.length}`, 'OpenAICompatible');
    const maxToolRounds = this.config.maxToolRounds ?? 20;
    const mcpToolMode = overrides?.mcpToolMode || 'execute';
    const enableMcpTools = mcpToolMode !== 'passthrough';

    for (let round = 0; round < maxToolRounds; round++) {
      Bot.makeLog('debug', `[OpenAICompatible] chatStream 轮次 round=${round + 1}/${maxToolRounds} currentMessagesLen=${currentMessages?.length}`, 'OpenAICompatible');
      const resp = await fetch(
        this.endpoint,
        buildFetchOptionsWithProxy(this.config, {
          method: 'POST',
      ...
      if (!Array.isArray(tool_calls) || tool_calls.length === 0 || !enableMcpTools) {
        Bot.makeLog('debug', `[OpenAICompatible] chatStream 轮次=${round + 1} 无 tool_calls 返回`, 'OpenAICompatible');
        return;
      }
      const names = tool_calls.map(t => t?.function?.name).filter(Boolean).join(',');
      Bot.makeLog('debug', `[OpenAICompatible] chatStream 轮次=${round + 1} tool_calls count=${tool_calls.length} names=[${names}]`, 'OpenAICompatible');
      ...
      Bot.makeLog('debug', `[OpenAICompatible] chatStream 轮次=${round + 1} 追加后 currentMessagesLen=${currentMessages.length}`, 'OpenAICompatible');
    }
  }
}
