...
import fastify from 'fastify'; // 使用Fastify替换Express
import fastifyCompress from '@fastify/compress'; // 压缩中间件
import fastifyHelmet from '@fastify/helmet'; // 安全头部
import fastifyRateLimit from '@fastify/rate-limit'; // 速率限制
import fastifyStatic from '@fastify/static'; // 静态文件服务
import fastifyCors from '@fastify/cors'; // CORS支持
import fastifyWebsocket from '@fastify/websocket'; // WS支持，优化性能
...
export default class Bot extends EventEmitter {
  ...
  constructor() {
    ...
    // Fastify应用和服务器（替换Express）
    this.fastify = fastify({
      logger: false, // 禁用内置日志，使用自定义日志
      ignoreTrailingSlash: true, // 忽略尾部斜杠，提高匹配速度
      maxParamLength: 500, // 优化参数长度，降低内存
      bodyLimit: 10 * 1024 * 1024 // 10MB body限制，防止DoS
    });
    this.fastify.skip_auth = []; // 跳过认证路径
    this.fastify.quiet = []; // 安静路径
    this.server = null;
    this.httpsServer = null;
    this.wsf = Object.create(null); // WS处理函数
    this.fs = Object.create(null); // 文件系统缓存
  ...
  async _initializeMiddlewareAndRoutes() {
    ...
    await this.fastify.register(fastifyWebsocket, {
      options: { maxPayload: 1048576 } // 优化payload大小，降低内存
    });
    this.fastify.addHook('upgrade', this.wsConnect.bind(this)); // 处理WS升级
  }
  ...
  async _setupStaticServing() {
    ...
    await this.fastify.register(fastifyStatic, {
      ...
      preCompressed: true // 预压缩支持，优化速度
    });
    
    // 目录索引和安全中间件在preHandler中处理
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async _setupRateLimiting() {
    ...
    if (rateLimitConfig?.api) {
      await this.fastify.register(fastifyRateLimit, {
        prefix: '/api',
        max: rateLimitConfig.api?.max || 100,
        timeWindow: rateLimitConfig.api?.windowMs || 15 * 60 * 1000
      });
    }
  }
   ...
   * @param {Buffer} head - 头缓冲
   */
  wsConnect(req, socket, head) {
    req.rid = `${socket.remoteAddress}:${socket.remotePort}-${req.headers["sec-websocket-key"]}`;
    req.sid = `ws://${req.headers.host || `${socket.localAddress}:${socket.localPort}`}${req.url}`;
    ...
    if (!isWhitelisted && !this._isLocalConnection(socket.remoteAddress)) {
    ...
    // 使用fastify-websocket处理升级
    this.fastify.websocketServer.handleUpgrade(req, socket, head, (conn) => {
      ...
      conn.on('message', msg => {  // 修复：从 'data' 改为 'message'
