  ...
  log4js.configure({
    appenders: {
      ...
      traceFile: {
        ...
        pattern: 'yyyy-MM-dd.log',
        numBackups: cfg.bot?.log_trace_days || 1, // trace日志保留天数更短
        alwaysIncludePattern: true,
        compress: false,
        keepFileExt: true,
        layout: {
    ...
    categories: {
      default: { 
        ...
        level: 'trace', // 内部级别设为trace以捕获所有日志
          ...
          if (dateMatch) {
            ...
            if (fileAge > maxAgeMs) {
              try {
                await fsPromises.unlink(filePath);
                deletedCount++;
                logger.debug(`已删除过期trace日志文件: ${file}`);
              } catch (err) {
                logger.error(`删除trace日志文件失败: ${file}`, err.message);
              }
            }
          }
        }
  ...
  global.logger = {
    ...
    timeEnd: function (label = 'default') {
      if (timers.has(label)) {
        ...
        // 记录到trace日志
        logger.trace(`计时器 [${label}] 耗时: ${timeStr}`);
      } else {
      ...
      if (label && timers.has(label)) {
        ...
        // 记录到trace日志
        logger.trace(`操作完成 [${label}]: ${text} - 耗时${timeStr}`);
      }
      logger.info(prefix + chalk.green(message));
    },
    ...
    platform: function() {
      return {
        ...
        mainLogAge: `${cfg.bot?.log_max_days || 3}天`,
        traceLogAge: `${cfg.bot?.log_trace_days || 1}天`,
        logFiles: {
    ...
    getTraceLogs: async function(lines = 100) {
      try {
        ...
        this.error('读取trace日志失败:', err.message);
        return null;
      }
    },
