...
class ConfigLoader {
  ...
  async _loadConfig(filePath) {
    try {
      ...
      if (!module.default) {
        BotUtil.makeLog('warn', `无效的配置模块: ${derivedKey} (缺少default导出)`, 'ConfigLoader');
        return false;
      }
      ...
      if (typeof module.default === 'function') {
        try {
          ...
          BotUtil.makeLog('warn', `无法实例化配置模块: ${derivedKey} (${err?.message || err})`, 'ConfigLoader');
          return false;
        }
      } else if (typeof module.default === 'object' && module.default !== null && !Array.isArray(module.default)) {
        configInstance = module.default;
      } else {
        BotUtil.makeLog('warn', `无效的配置模块: ${derivedKey} (导出类型错误)`, 'ConfigLoader');
        return false;
      }

      if (typeof configInstance.getStructure !== 'function') {
        BotUtil.makeLog('warn', `无效的配置模块: ${derivedKey} (缺少 getStructure 方法)`, 'ConfigLoader');
        return false;
      }
      ...
      if (existing) {
        ...
        if (isPluginPath && !existingIsPlugin) {
          ...
          BotUtil.makeLog('debug', `跳过配置: ${key} (已存在插件配置，优先级更高)`, 'ConfigLoader');
          return false;
        } else {
