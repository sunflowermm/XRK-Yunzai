...
class ProcessManager {
  ...
  setupSignalHandlers() {
    const handleSignal = async (signal) => {
      const now = Date.now();
      this.signalCount = (now - this.lastSignalTime <= CONFIG.SIGNAL_TIME_THRESHOLD)
        ? this.signalCount + 1
        : 1;
      this.lastSignalTime = now;

      if (this.signalCount < 3) {
        logger.mark(chalk.yellow(`接收到${signal}信号，第 ${this.signalCount} 次，程序重启`));
        await this.restart(); // exit(1) 触发外层自动重启
      } else {
        logger.mark(chalk.yellow(`检测到第 ${this.signalCount} 次${signal}信号，程序退出到菜单`));
        await this.cleanup();
        process.exit(0);
      }
    };
