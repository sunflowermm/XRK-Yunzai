...
export default class plugin {
  ...
  /**
   * 获取工作流管理器（全局单例）
   * @returns {WorkflowManager} 工作流管理器
   */
  getWorkflowManager() {
    if (!globalWorkflowManager) {
      ...
      for (const [name, stream] of streams) {
        globalWorkflowManager.registerWorkflow(name, async (params, context) => {
          ...
          const finalConfig = { ...cfg.kuizai?.ai, ...config };
          return await stream.execute(e || params.e, question || params.question, finalConfig);
        }, {
  ...
  async executeWorkflow(streamName, question, config = {}) {
    ...
    const e = this.e;
    const finalConfig = { ...cfg.kuizai?.ai, ...config };
    return await stream.execute(e, question, finalConfig);
  }
