...
export default class ConfigBase {
  ...
  async read(useCache = true) {
    ...
    try {
      ...
      if (this.transformRead && typeof this.transformRead === 'function') {
  ...
  async write(data, options = {}) {
    ...
    try {
      ...
      if (this.transformWrite && typeof this.transformWrite === 'function') {
        dataToWrite = this.transformWrite(dataToWrite);
      }

      // 验证数据（验证过程中会进行类型转换，所以传入的数据对象可能会被修改）
      if (validate) {
        const validation = await this.validate(dataToWrite);
        if (!validation.valid) {
      ...
      if (this.schema.fields) {
        for (const [field, fieldSchema] of Object.entries(this.schema.fields)) {
          if (field in data) {
            ...
            if (fieldSchema.enum && value !== null && value !== undefined) {
              // 再次尝试类型转换（如果之前没有转换成功）
              let enumValue = value;
              if (expectedType === 'number' && typeof value === 'string') {
                const numValue = Number(value);
  ...
  getDefaults() {
    ...
    const extractDefaults = (fields) => {
      ...
      for (const [fieldName, fieldSchema] of Object.entries(fields)) {
        if (fieldSchema.default !== undefined) {
          ...
          } else if (fieldSchema.default && typeof fieldSchema.default === 'object') {
            defaults[fieldName] = JSON.parse(JSON.stringify(fieldSchema.default));
          } else {
            defaults[fieldName] = fieldSchema.default;
          }
        } else if (fieldSchema.type === 'array' && fieldSchema.itemType === 'object' && fieldSchema.itemSchema) {
          // 对象数组：使用空数组作为默认值
          defaults[fieldName] = [];
          ...
          // 嵌套对象：递归提取默认值
          defaults[fieldName] = extractDefaults(fieldSchema.fields);
        }
      }
    ...
    return extractDefaults(this.schema.fields);
  }
