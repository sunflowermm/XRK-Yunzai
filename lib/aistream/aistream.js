...
export default class AIStream {
  ...
  async runActionTimeline(timeline = [], context) {
    ...
    for (const action of timeline) {
      ...
      if (action.type === 'function' && action.data) {
        ...
        await this.handleActionResult(result, textSegments, context);
      }
    }
  ...
  async handleActionResult(result, textSegments, context) {
    ...
    if (result.type === 'segments' && Array.isArray(result.content) && context?.e) {
      await context.e.reply(result.content).catch(() => {});
    }
  }
  ...
  async execute(e, question, config) {
    try {
      ...
      const { timeline, cleanText: parsedText } = this.parseFunctions(response, context);
