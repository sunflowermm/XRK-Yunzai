...
class ApiLoader {
  ...
  async register(app, bot) {
    ...
    for (const api of this.priority) {
      try {
        ...
        BotUtil.makeLog('info', `[ApiLoader] 注册API: ${api.name} (优先级: ${api.priority})`, 'ApiLoader');
      } catch (error) {
        BotUtil.makeLog('error', `[ApiLoader] 注册API失败: ${api.name}`, 'ApiLoader', error);
      }
    }
  ...
  async changeApi(key) {
    ...
    try {
      BotUtil.makeLog('info', `[ApiLoader] 重载API: ${api.name}`, 'ApiLoader');
      ...
      if (newApi && this.app && this.bot) {
        await newApi.init(this.app, this.bot);
      }
      
      BotUtil.makeLog('info', `[ApiLoader] API重载成功: ${api.name}`, 'ApiLoader');
      return true;
    } catch (error) {
      BotUtil.makeLog('error', `[ApiLoader] API重载失败: ${api.name}`, 'ApiLoader', error);
      return false;
    }
  }
   ...
   */
  getApiList() {
    return Array.from(this.apis.values()).map(api => api.getInfo());
  }
        ...
        .on('add', filePath => {
          ...
          this.loadApi(filePath).then(() => {
            ...
            if (this.app && this.bot) {
              ...
              const api = this.apis.get(key);
              if (api) api.init(this.app, this.bot);
            }
          });
        })
        .on('change', filePath => {
