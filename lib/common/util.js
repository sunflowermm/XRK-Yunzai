...
export default class BotUtil {
  ...
  /**
   * 主题配套的ID渐变色方案
   * 每个主题都有对应的ID配色，形成和谐搭配
   * @static
   * @type {Object}
   */
  static idColorSchemes = {
    // default主题(蓝色系) -> ID用青绿色系
   ...
   * @returns {Object} 包含文本和填充位置的对象
   */
  static #formatIdText(text, filler, targetLength) {
    const currentLength = text.length;

    if (currentLength >= targetLength) {
      // 文本太长，截断
      return {
        text: text.slice(0, targetLength - 2) + '..',
        type: 'truncated',
        parts: [{ content: text.slice(0, targetLength - 2) + '..', isFiller: false }]
      };
    }
    ...
    // 创建美化的填充
    const leftFiller = filler.repeat(leftPad);
    const rightFiller = filler.repeat(rightPad);

    return {
      text: leftFiller + text + rightFiller,
      type: 'padded',
      parts: [
        { content: leftFiller, isFiller: true },
        { content: text, isFiller: false },
        { content: rightFiller, isFiller: true }
      ]
    };
  }
   ...
   * @static
   * @private
   * @param {Object} formattedText - 格式化后的文本对象
   * @param {Array<string>} colors - 颜色数组
   * @returns {string} 带渐变色的文本
   */
  static #applyIdGradient(formattedText, colors) {
    if (!global.logger?.chalk) return `[${formattedText.text}]`;

    const chalk = global.logger.chalk;
    let result = '';

    // 左方括号 - 使用第一个颜色
    result += chalk.hex(colors[0])('[');

    // 处理内容
    if (formattedText.type === 'padded') {
      // 分段处理，填充字符用浅色，实际内容用深色
      formattedText.parts.forEach((part, index) => {
        if (part.isFiller) {
          // 填充字符使用淡化的颜色
          const fillerColors = colors.map(c =>
            BotUtil.#lightenColor(c, 0.5)
          );
          result += BotUtil.#createGradientSegment(
            part.content,
            fillerColors,
            chalk
          );
    ...
    // 右方括号 - 使用最后一个颜色
    result += chalk.hex(colors[colors.length - 1])(']');
   ...
   * @private
   */
  static #createGradientSegment(text, colors, chalk) {
    if (!text) return '';

    let result = '';
    const len = text.length;
    const step = Math.max(1, Math.ceil(len / colors.length));
  ...
  static makeLogID(id) {
    ...
    // 格式化文本
    const formatted = BotUtil.#formatIdText(idStr, filler, targetLength);

    // 获取配套的ID颜色方案
    const idColors = BotUtil.idColorSchemes[currentTheme] || BotUtil.idColorSchemes.default;

    // 应用渐变色
    const result = BotUtil.#applyIdGradient(formatted, idColors);
  ...
  static makeLog(level = "info", msg, id, trace = false) {
    ...
    const maxLength = cfg.bot?.max_log_length || 5000;

    if (logMessage.length > maxLength) {
      const preview = 1000;
      const suffix = 1000;
      const truncated = logMessage.length - preview - suffix;
      logMessage = `${logMessage.slice(0, preview)}... [截断 ${truncated} 字符] ...${logMessage.slice(-suffix)}`;
    }
    ...
    setImmediate(() => {
      ...
      const plainId = `[${fileId.padEnd(cfg.bot?.log_id_length || 16)}]`;
      const fileParts = id !== false ? [plainId, ...messages] : messages;
      BotUtil.#writeLogToFile(level, fileParts);
    });
