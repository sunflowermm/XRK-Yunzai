...
export default class AIStream {
  ...
  async init() {
    ...
    try {
      ...
      if (this.embeddingConfig.enabled) {
        await this.initEmbedding().catch(err => {
          BotUtil.makeLog('warn', 
            `[${this.name}] Embedding初始化失败，将禁用: ${err.message}`,
            'AIStream'
          );
      ...
      BotUtil.makeLog('debug', `[${this.name}] 基类初始化完成`, 'AIStream');
    } catch (error) {
  ...
  async initEmbedding() {
    ...
    try {
      ...
      BotUtil.makeLog('success', 
        `[${this.name}] Embedding模型初始化成功 (${this.embeddingConfig.provider})`,
        'AIStream'
      );
    } catch (error) {
      BotUtil.makeLog('error', 
        `[${this.name}] Embedding初始化失败: ${error.message}`,
        'AIStream'
      );
      this.embeddingConfig.enabled = false;
      throw error;
  ...
  async initTensorFlowEmbedding() {
    try {
      ...
      BotUtil.makeLog('info', 
        `[${this.name}] 正在加载Universal Sentence Encoder...`,
        'AIStream'
      );
