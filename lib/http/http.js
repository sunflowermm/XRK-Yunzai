...
export default class HttpApi {
  ...
  registerRoutes(target, bot) {
    ...
    for (const route of this.routes) {
      ...
      try {
        ...
        registeredCount++;
        registeredPaths.push(`${method} ${path}`);
      } catch (error) {
        BotUtil.makeLog('error', `[HttpApi] ${this.name} 注册路由失败: ${method} ${path} - ${error.message}`, 'HttpApi');
      }
    }
    
    if (registeredCount > 0) {
      BotUtil.makeLog('info', `[HttpApi] ${this.name} 注册了 ${registeredCount} 个路由: ${registeredPaths.slice(0, 5).join(', ')}${registeredPaths.length > 5 ? '...' : ''}`, 'HttpApi');
    } else if (this.routes.length > 0) {
      BotUtil.makeLog('warn', `[HttpApi] ${this.name} 有 ${this.routes.length} 个路由定义，但未成功注册任何路由`, 'HttpApi');
    }
  }
  ...
  registerWebSocketHandlers(bot) {
        ...
        if (typeof handler === 'function') {
          bot.wsf[wsPath].push((conn, req, socket, head) => {
            try {
              ...
              BotUtil.makeLog('error', `[HttpApi] ${this.name} WebSocket处理失败 [${wsPath}]: ${error.message}`, 'HttpApi', error);
            }
          });
          registeredCount++;
        }
    ...
    if (registeredCount > 0) {
      BotUtil.makeLog('info', `[HttpApi] ${this.name} 注册了 ${registeredCount} 个WebSocket处理器`, 'HttpApi');
    }
  }
  ...
  start() {
    this.enable = true;
    BotUtil.makeLog('debug', `[HttpApi] ${this.name} 已启用`, 'HttpApi');
  }

  stop(app) {
    this.enable = false;
    this._detachRouter(app || this._app);
    BotUtil.makeLog('debug', `[HttpApi] ${this.name} 已停用`, 'HttpApi');
  }

  async reload(app, bot) {
    BotUtil.makeLog('debug', `[HttpApi] ${this.name} 开始重载`, 'HttpApi');
    this.stop(app);
    this.enable = true;
    await this.init(app, bot);
    BotUtil.makeLog('debug', `[HttpApi] ${this.name} 重载完成`, 'HttpApi');
  }
}
