...
export default class GPTGodLLMClient {
  ...
  async chat(messages, overrides = {}) {
    let msgs = await prepareOpenAIChatVisionMessages(messages, this.config, { timeoutMs: this.timeout });
    ...
    for (let i = 0; i < maxRounds; i++) {
      ...
      if (msg.tool_calls?.length > 0) {
        noteLlmChatToolCallsForReply(replyTracker, msg.tool_calls);
        msgs = [...msgs, msg, ...await MCPToolAdapter.handleToolCalls(msg.tool_calls)];
        continue;
      }
      return finalizeLlmChatNonStreamContent(replyTracker, msg.content ?? '');
    }
    return finalizeLlmChatNonStreamContent(replyTracker, msgs[msgs.length - 1]?.content ?? '');
  }
