...
export default class BotUtil {
  ...
  static makeLogID(id) {
    ...
    if (displayWidth > targetLength) {
      ...
      // Apply gradient to truncated ID
      const coloredTruncated = BotUtil.#applyGradientToString(truncated, idColors, chalk);
      result += coloredTruncated;
      result += chalk.hex(idColors[idColors.length - 1])('..');
    } else {
      // Padding case - apply gradient to ID only, then add padding
      const totalPadding = targetLength - displayWidth;
      const leftPad = Math.floor(totalPadding / 2);
      const rightPad = totalPadding - leftPad;

      // Add left padding with lighter colors
      if (leftPad > 0) {
        const lightColor = BotUtil.#lightenColor(idColors[0], 0.6);
        result += chalk.hex(lightColor)(filler.repeat(leftPad));
      }

      // Apply gradient to the actual ID string
      const coloredId = BotUtil.#applyGradientToString(idStr, idColors, chalk);
      result += coloredId;

      // Add right padding with lighter colors
      if (rightPad > 0) {
        const lightColor = BotUtil.#lightenColor(idColors[idColors.length - 1], 0.6);
        result += chalk.hex(lightColor)(filler.repeat(rightPad));
      }
    }
  ...
  static #applyGradientToString(text, colors, chalk, lightMode = false) {
    ...
    // For proper gradient distribution regardless of character type
    for (let i = 0; i < chars.length; i++) {
      // Calculate color index based on position in string
      const progress = chars.length > 1 ? i / (chars.length - 1) : 0;
      const colorIndex = Math.floor(progress * (colorsToUse.length - 1));
