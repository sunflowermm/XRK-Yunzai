        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (signStat?.isFile()) {
                Bot.makeLog('info', `检测到前端 sign.json，跳过子目录静态挂载: ${mountPath} (plugin: ${pluginName})`, 'Bot');
                continue;
              }
            } catch (err) {
              Bot.makeLog('debug', `sign.json 检查跳过: ${mountPath}`, 'Bot', err?.message || err);
            }
            ...
            Bot.makeLog('info', `挂载子目录: ${mountPath} -> ${subDirPath} (plugin: ${pluginName})`, 'Bot');
          }
        }
      } catch (error) {
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  _createProxy() {
    return new Proxy(this.bots, {
      get: (target, prop) => {
        ...
        Bot.makeLog("trace", `Bot.${String(prop)} 不存在`);
        return undefined;
      }
    });
  }
   ...
   * @param {object} req
   * @param {{ forceAuth?: boolean }} [options] forceAuth=true 时不跳过 127.* 回环
   */
  _checkApiAuthorization(req, options = {}) {
    ...
    if (!forceAuth && this._isLoopbackConnection(req.socket?.remoteAddress)) {
      return true;
    }
  ...
  wsConnect(req, socket, head) {
    ...
    const wsPathSegment = (pathStr.startsWith('/') ? pathStr.slice(1) : pathStr).split('/')[0];
    const isOneBotWs = wsPathSegment === 'OneBotv11';
    ...
    } else if (forceOneBotAuth && authConfig.apiKey?.enabled !== false && !this._checkApiAuthorization(req, { forceAuth: true })) {
      Bot.makeLog('error', `OneBot WebSocket 鉴权失败（requireLoopbackAuth）：${req.url}`, '服务器');
      socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');
      return socket.destroy();
    }
  ...
  async _shutdownLoaders() {
    const steps = [
      { label: 'StreamLoader', run: () => StreamLoader.cleanupAll() },
  ...
  async run(options = {}) {
    ...
    if (streamResult.status === 'rejected') {
      Bot.makeLog('error', `工作流加载失败: ${streamResult.reason ? streamResult.reason.message : '未知错误'}`, '服务器');
    }
