...
export default class AIStream {
  constructor(options = {}) {
    ...
    this.embeddingConfig = {
      ...
      cacheExpiry: 86400,
      // Transformers.js 配置
      transformersModel: 'Xenova/paraphrase-multilingual-MiniLM-L12-v2',
  ...
  async initEmbedding() {
    ...
    try {
      ...
      if (this.embeddingConfig.provider !== 'lightweight') {
        BotUtil.makeLog('info', `[${this.name}] 🔄 尝试降级到轻量级BM25模式...`, 'AIStream');
        try {
  ...
  async initTransformersEmbedding() {
    ...
    try {
      ...
      const modelName = this.embeddingConfig.transformersModel || 'Xenova/paraphrase-multilingual-MiniLM-L12-v2';
      BotUtil.makeLog('info', `[${this.name}] 🤖 加载模型: ${modelName}`, 'AIStream');
      BotUtil.makeLog('info', `[${this.name}] ⏳ 首次加载需下载模型文件（约50-100MB），请耐心等待...`, 'AIStream');
      ...
      this.embeddingModel = await pipeline(
        ...
        modelName,
        { 
          quantized: this.embeddingConfig.transformersQuantized !== false,
          progress_callback: (progress) => {
            if (progress.status === 'downloading') {
              const percent = ((progress.loaded / progress.total) * 100).toFixed(1);
              BotUtil.makeLog('debug', `[${this.name}] 下载进度: ${percent}% (${progress.file})`, 'AIStream');
            }
          }
        }
      );
      ...
      BotUtil.makeLog('success', `[${this.name}] ✅ Transformers模型就绪 (耗时: ${loadTime}秒)`, 'AIStream');
  ...
  async initAPIEmbedding() {
    ...
    BotUtil.makeLog('info', `[${this.name}] 🔑 使用模型: ${config.apiModel || 'text-embedding-3-small'}`, 'AIStream');
  ...
  getInfo() {
    return {
      ...
      embedding: {
        ...
        model: this.embeddingConfig.transformersModel || this.embeddingConfig.apiModel,
