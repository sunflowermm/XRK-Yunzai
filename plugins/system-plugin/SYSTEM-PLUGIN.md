...
| 工作流      | 说明与主要工具 |
|-------------|----------------|
| **chat**    | 智能聊天与群内互动。工具：`reply`、`emotion`、`send_file`、`send_image`、`setGroupAvatar`（工作区图设群头像）、`poke`、`relayPrivate`（私聊传话）、`relayPrivateImage` / `relayPrivateFile`、`getFriendRequests` / `handleFriendRequest`（加好友）、`setFriendRemark` / `deleteFriend`、`relayPrivateEmotion`、`emojiReaction`、`thumbUp`、`sign`、群管与查询类等。 |
