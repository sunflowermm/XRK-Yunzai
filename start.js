...
class Logger {
  ...
  async flushQueue() {
    ...
    try {
      ...
      await this.log('日志写入失败: ' + error.message, 'ERROR'); // 递归日志，但无 console
    } finally {
      this.isWriting = false;
...
class MenuManager {
  ...
  async pm2Menu() {
    ...
    if (availablePorts.length === 0) {
      // 移除 console.log，仅 logger
      await this.serverManager.logger.log('没有可用的服务器端口');
      return;
    }
...
function getNodeArgs() {
  ...
  // 添加 --no-warnings 以屏蔽 Node 警告（如 TimeoutOverflowWarning）
