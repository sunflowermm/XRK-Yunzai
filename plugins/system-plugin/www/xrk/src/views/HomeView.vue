          ...
          <div class="runtime-sections">
            <section aria-label="插件">
              <div class="sec-label-row">
                ...
                <button
                  v-if="!pluginMeta.error && pluginChips.length"
                  type="button"
                  class="sec-toggle"
                  :aria-expanded="pluginsOpen ? 'true' : 'false'"
                  @click="pluginsOpen = !pluginsOpen"
                >
                  {{ pluginsOpen ? '收起' : `展开 ${pluginChips.length}` }}
                </button>
              </div>
              ...
              <template v-else>
                ...
                <HomeTagCloud
                  v-if="pluginsOpen && pluginChips.length"
                  tip-prefix="plg"
                  :items="pluginChips"
                  :preview="18"
                />
                <div v-else-if="!pluginChips.length" class="cloud-empty">暂无插件条目</div>
              </template>
            </section>
            <section aria-label="工作流">
              <div class="sec-label-row">
                ...
                <button
                  ...
                  type="button"
                  class="sec-toggle"
                  :aria-expanded="workflowsOpen ? 'true' : 'false'"
                  @click="workflowsOpen = !workflowsOpen"
                >
                  {{ workflowsOpen ? '收起' : `展开 ${workflowChips.length}` }}
                </button>
              </div>
              ...
              <HomeTagCloud
                v-if="workflowsOpen && workflowChips.length"
                tip-prefix="wf"
                :items="workflowChips"
                :preview="12"
              />
              <div v-else-if="!workflowChips.length" class="cloud-empty">暂无工作流数据</div>
            </section>
          </div>
        </section>
      </div>
