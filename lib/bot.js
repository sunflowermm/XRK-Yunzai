...
export default class Bot extends EventEmitter {
  ...
  _setupReverseProxy() {
    ...
    BotUtil.makeLog('info', chalk.cyan('⚡ 初始化反向代理服务'), 'Proxy');

    // 域名路由中间件 - 必须在所有其他中间件之前
    this.express.use((req, res, next) => {
      ...
      // 查找匹配的域名配置
      const domainConfig = proxyConfig.domains?.find(d => {
        if (d.domain === hostname) return true;
      ...
      if (domainConfig.rewritePath) {
        ...
        if (req.path.startsWith(from)) {
          // 重写路径
          req.url = req.path.replace(from, to) + (req.url.slice(req.path.length));
          
          BotUtil.makeLog('debug', 
            chalk.gray(`路径重写 [${hostname}]: ${originalPath} → ${req.url}`), 
            'Proxy');
        }
      }
      ...
      if (domainConfig.target) {
        // 获取或创建代理中间件
        let proxyMiddleware = this.proxyMiddlewares.get(hostname);
        ...
        if (!proxyMiddleware) {
          ...
          const proxyOptions = {
            ...
            onProxyRes: (proxyRes, req, res) => {
              ...
              if (domainConfig.headers?.response) {
                Object.entries(domainConfig.headers.response).forEach(([key, value]) => {
              ...
              // 记录代理响应
              BotUtil.makeLog('debug', 
                chalk.green(`✓ 代理响应 [${hostname}]: ${req.method} ${originalPath} → ${proxyRes.statusCode}`), 
                'Proxy');
            },
            ...
            onProxyReq: (proxyReq, req, res) => {
              ...
              if (domainConfig.headers?.request) {
                Object.entries(domainConfig.headers.request).forEach(([key, value]) => {
            ...
            onError: (err, req, res) => {
              BotUtil.makeLog('error', 
                chalk.red(`✗ 代理错误 [${hostname}]: ${err.message}`), 
                'Proxy');
          ...
          // 处理超时配置
          if (domainConfig.timeout) {
            proxyOptions.timeout = domainConfig.timeout;
            proxyOptions.proxyTimeout = domainConfig.timeout;
          }

          proxyMiddleware = createProxyMiddleware(proxyOptions);
          this.proxyMiddlewares.set(hostname, proxyMiddleware);
          
          BotUtil.makeLog('info', 
            chalk.green(`✓ 创建代理 [${hostname}] → ${domainConfig.target}`), 
            'Proxy');
        }
  ...
  _setupStaticServing() {
    ...
    this.express.use((req, res, next) => {
      ...
      // 查找域名特定的静态文件配置
      const domainConfig = proxyConfig?.domains?.find(d => d.domain === hostname);
      
      let staticRoot = wwwPath;
      
      if (domainConfig?.staticRoot) {
        // 使用域名特定的静态文件根目录
        staticRoot = path.isAbsolute(domainConfig.staticRoot) 
          ? domainConfig.staticRoot 
          : path.join(process.cwd(), domainConfig.staticRoot);
          
        // 确保目录存在
        if (!fsSync.existsSync(staticRoot)) {
          fsSync.mkdirSync(staticRoot, { recursive: true });
          BotUtil.makeLog('info', 
            chalk.green(`✓ 创建域名静态目录 [${hostname}]: ${staticRoot}`), 
            'Server');
        }
      }
  ...
  _directoryIndexMiddleware(req, res, next) {
    ...
    // 确定静态文件根目录
    let staticRoot = path.join(process.cwd(), 'www');
    if (domainConfig?.staticRoot) {
      staticRoot = path.isAbsolute(domainConfig.staticRoot)
        ? domainConfig.staticRoot
        : path.join(process.cwd(), domainConfig.staticRoot);
    ...
    if (fsSync.existsSync(dirPath) && fsSync.statSync(dirPath).isDirectory()) {
      ...
      for (const indexFile of indexFiles) {
        ...
        if (fsSync.existsSync(indexPath)) {
          ...
          BotUtil.makeLog('debug', 
            chalk.gray(`目录重定向 [${hostname}]: ${req.path} → ${redirectUrl}`), 
            'Server');
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  _statusHandler(req, res) {
    const status = {
      ...
      server: {
        ...
        proxy: cfg.server?.proxy?.enabled || false
      }
    };
  ...
  async _displayProxyInfo() {
    const proxyConfig = cfg.server?.proxy;
    if (!proxyConfig?.enabled || !proxyConfig?.domains?.length) return;

    console.log(chalk.yellow('\n╔════════════════════════════════════════════════════════════╗'));
    console.log(chalk.yellow('║') + chalk.white.bold('              反向代理配置信息                             ') + chalk.yellow('║'));
    console.log(chalk.yellow('╚════════════════════════════════════════════════════════════╝\n'));

    console.log(chalk.cyan('▶ 域名映射:'));
    ...
    for (const domain of proxyConfig.domains) {
      ...
      if (domain.target) {
        console.log(chalk.gray(`    代理目标: ${domain.target}`));
      }
      
      if (domain.rewritePath) {
        console.log(chalk.gray(`    路径重写: ${domain.rewritePath.from} → ${domain.rewritePath.to}`));
      }
      
      if (domain.staticRoot) {
        console.log(chalk.gray(`    静态目录: ${domain.staticRoot}`));
      }
      
      if (domain.ssl?.enabled) {
        console.log(chalk.green(`    SSL证书: ✓ 已配置`));
      }
    }
  ...
  async httpsLoad() {
    ...
    // 检查是否有任何HTTPS配置
    const hasGlobalHttps = httpsConfig?.enabled;
    const hasDomainHttps = proxyConfig?.domains?.some(d => d.ssl?.enabled);
    
    if (!hasGlobalHttps && !hasDomainHttps) return;
    ...
    try {
      ...
      if (hasGlobalHttps) {
        ...
        if (!cert?.key || !cert?.cert) {
          BotUtil.makeLog("warn", chalk.yellow("⚠ HTTPS已启用但未配置默认证书"), 'Server');
        } else {
          // 检查证书文件
          if (!fsSync.existsSync(cert.key)) {
            throw new Error(`默认HTTPS密钥文件不存在: ${cert.key}`);
          }
          if (!fsSync.existsSync(cert.cert)) {
            throw new Error(`默认HTTPS证书文件不存在: ${cert.cert}`);
          }
      ...
      // 配置SNI（Server Name Indication）支持多域名证书
      if (proxyConfig?.domains?.length > 0) {
        const contexts = [];
        
        for (const domain of proxyConfig.domains) {
          if (!domain.ssl?.enabled) continue;
          
          const domainCert = domain.ssl.certificate;
          if (!domainCert?.key || !domainCert?.cert) {
            BotUtil.makeLog("warn", 
              chalk.yellow(`⚠ 域名 ${domain.domain} 启用了SSL但未配置证书`), 
              'Server');
            continue;
          }

          // 检查域名证书文件
          if (!fsSync.existsSync(domainCert.key)) {
            BotUtil.makeLog("error", 
              chalk.red(`✗ 域名 ${domain.domain} 的SSL密钥文件不存在: ${domainCert.key}`), 
              'Server');
            continue;
          }
          if (!fsSync.existsSync(domainCert.cert)) {
            BotUtil.makeLog("error", 
              chalk.red(`✗ 域名 ${domain.domain} 的SSL证书文件不存在: ${domainCert.cert}`), 
              'Server');
            continue;
          }
          ...
          const context = {
            ...
            key: await fs.readFile(domainCert.key),
            cert: await fs.readFile(domainCert.cert)
          };
          
          if (domainCert.ca && fsSync.existsSync(domainCert.ca)) {
            context.ca = await fs.readFile(domainCert.ca);
          }
          
          contexts.push(context);
          
          BotUtil.makeLog("info", 
            chalk.green(`✓ 加载域名 ${domain.domain} 的SSL证书`), 
            'Server');
        }

        // 设置SNI回调
        if (contexts.length > 0) {
          httpsOptions.SNICallback = (hostname, cb) => {
            const ctx = contexts.find(c => {
              if (c.hostname === hostname) return true;
              // 支持通配符证书
              if (c.hostname.startsWith('*.')) {
                const baseDomain = c.hostname.slice(2);
                return hostname.endsWith(baseDomain);
              }
              return false;
            });
   ...
   */
  async run(options = {}) {
    // 优先使用代理配置中的端口，如果没有则使用传入的端口
    const proxyConfig = cfg.server?.proxy;
    
    if (proxyConfig?.enabled && (proxyConfig?.httpPort || proxyConfig?.httpsPort)) {
    ...
    const httpsConfig = cfg.server?.https;
    const hasDomainHttps = proxyConfig?.domains?.some(d => d.ssl?.enabled);
    if (httpsConfig?.enabled || hasDomainHttps) {
