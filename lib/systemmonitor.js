...
class SystemMonitor extends EventEmitter {
    ...
    async checkSystem() {
        ...
        try {
            const status = {
                timestamp: Date.now(),
                memory: this.config.memory && this.config.memory.enabled ? await this.checkMemory() : null,
                cpu: this.config.cpu && this.config.cpu.enabled ? await this.checkCPU() : null,
                browser: this.config.browser && this.config.browser.enabled ? await this.checkBrowser() : null,
                leak: this.config.memory && this.config.memory.leakDetection && this.config.memory.leakDetection.enabled ? this.detectMemoryLeak() : null
            };

            // 如果检测到内存泄漏，立即执行优化
            if (status.leak && this.config.memory && this.config.memory.autoOptimize) {
    ...
    async optimizeSystem(status) {
        ...
        // 垃圾回收（由 app.js 入口保证进程已带 --expose-gc）
        global.gc();
        logger.info('已执行垃圾回收');
