...
export default class VolcengineASRClient {
  ...
  _pongTimer = null;
  reconnectAttempts = 0;
  performanceMetrics = { firstResultTime: null, audioStartTime: null };
  ...
  _parse(data) {
    try {
      ...
      if (messageType === 0x9) {
        ...
        const result = tryParseJson(payload.toString('utf-8'));
  ...
  _startPingTimer() {
    ...
    this._pingTimer = setInterval(() => {
      try {
        ...
        BotUtil.makeLog('debug', `[ASR] ws.ping: ${err?.message || err}`, this.deviceId);
      }
    ...
    this._pongTimer = setTimeout(() => {
      ...
      if (this.ws) {
        try {
          ...
          BotUtil.makeLog('debug', `[ASR] ws.terminate: ${err?.message || err}`, this.deviceId);
        }
      }
