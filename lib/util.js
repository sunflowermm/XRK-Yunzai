  ...
  static async #saveBufferToTempFile(buffer) {
    ...
    try {
      ...
      setTimeout(() => BotUtil.rm(filePath).catch((err) => {
        BotUtil.makeLog('debug', `[BotUtil] 临时文件清理失败: ${filePath} | ${err?.message || err}`, 'BotUtil');
      }), 60000);
  ...
  static async fileToUrl(file, opts = {}) {
    ...
    try {
      ...
      setTimeout(() => BotUtil.rm(destPath).catch((err) => {
        BotUtil.makeLog('debug', `[BotUtil] 媒体文件清理失败: ${destPath} | ${err?.message || err}`, 'BotUtil');
      }), options.time);
