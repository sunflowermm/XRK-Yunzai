...
static makeLog(level = "info", msg, id, trace = false) {
  ...
  if (formattedId && global.logger) {
    ...
    const selectedColor = idColors[cfg.bot?.log_color] || idColors.default;
    
    // 如果logger存在chalk，使用它来着色
    if (global.logger.chalk) {
      coloredId = global.logger.chalk.hex(selectedColor)(formattedId);
    } else {
      coloredId = formattedId;
    }
  } else {
    coloredId = formattedId;
  }
  ...
  for (const item of messages) {
    if (item == null) {
      ...
      try {
        const jsonStr = JSON.stringify(item, BotUtil.getCircularReplacer(), 2);
        // 对于对象，进行格式化以便更好地显示
        if (jsonStr.length > 200) {
          logParts.push('\n' + jsonStr);
        } else {
          logParts.push(jsonStr);
        }
      } catch {
        logParts.push(String(item));
      }
  ...
  // 截断过长日志，但保留更多内容用于调试
  const maxLength = cfg.bot?.log_max_length || 10000;
  if (logMessage.length > maxLength) {
    const truncateLength = Math.floor(maxLength / 3);
    logMessage = `${logMessage.slice(0, truncateLength)}... [截断 ${logMessage.length} 字符] ...${logMessage.slice(-truncateLength)}`;
  }
  ...
  if (global.logger && typeof global.logger[level] === 'function') {
    ...
    if (coloredId) {
      switch(level) {
        case 'error':
        case 'fatal':
          finalMessage = `${coloredId} ${global.logger.red ? global.logger.red(logMessage) : logMessage}`;
          break;
        case 'warn':
          finalMessage = `${coloredId} ${global.logger.yellow ? global.logger.yellow(logMessage) : logMessage}`;
          break;
        case 'success':
          finalMessage = `${coloredId} ${global.logger.green ? global.logger.green(logMessage) : logMessage}`;
          break;
        case 'mark':
          finalMessage = `${coloredId} ${global.logger.magenta ? global.logger.magenta(logMessage) : logMessage}`;
          break;
        case 'debug':
          finalMessage = `${coloredId} ${global.logger.gray ? global.logger.gray(logMessage) : logMessage}`;
          break;
        case 'tip':
          finalMessage = `${coloredId} ${global.logger.cyan ? global.logger.cyan(logMessage) : logMessage}`;
          break;
        default:
          finalMessage = `${coloredId} ${logMessage}`;
      }
    } else {
    ...
    // 调用对应的日志方法
    global.logger[level](finalMessage);

    // 如果是错误且需要堆栈跟踪
    if (trace && (level === 'error' || level === 'fatal')) {
    ...
    consoleMethod(`[${timestamp}] ${formattedId} ${logMessage}`);
  }

  // 异步写入文件（不包含颜色代码）
  setImmediate(() => {
    const cleanMessage = logMessage.replace(/\x1b\[[0-9;]*m/g, "");
    const cleanId = formattedId.replace(/\x1b\[[0-9;]*m/g, "");
    BotUtil.#writeLogToFile(level, cleanId ? [cleanId, cleanMessage] : [cleanMessage]);
  });
