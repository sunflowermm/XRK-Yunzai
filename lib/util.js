...
import { exec as execCallback, execFile, execSync } from "node:child_process";
...
import { TEMP_DIR, WWW_DIR, resolveProjectPath } from './config/config-constants.js';
import common from './common/common.js';
import { FileUtils } from './utils/file-utils.js';
import { ObjectUtils } from './utils/object-utils.js';
import { toFileUrl } from './renderer/screenshot-utils.js';
...
export default class BotUtil {
  ...
  static makeLog(level = "info", msg, id, trace = false) {
    ...
    const levelPriorities = { trace: 0, debug: 1, info: 2, mark: 2, success: 2, tip: 2, warn: 3, error: 4, fatal: 5 };
    const configLogLevel = cfg.bot.log_level || "info";
    const tag = id !== undefined && id !== false ? String(id) : "";
    const effectiveLevel = (tag && cfg.bot.log_modules && cfg.bot.log_modules[tag]) ? cfg.bot.log_modules[tag] : configLogLevel;

    const currentPriority = levelPriorities[level] ?? 2;
    const configPriority = levelPriorities[effectiveLevel] ?? 2;
    if (currentPriority < configPriority) return "";

    const messages = Array.isArray(msg) ? msg : [msg];
    const logParts = [];
    for (const item of messages) {
      if (item == null) logParts.push(String(item));
        ...
        try {
          ...
          logParts.push(util && util.inspect ? util.inspect(item, inspectOptions) : JSON.stringify(item, BotUtil.getCircularReplacer(), 2));
        } catch {
          try { logParts.push(JSON.stringify(item, BotUtil.getCircularReplacer())); } catch (err) { logParts.push(`[Object: ${err.message}]`); }
        }
      } else logParts.push(String(item));
    }

    let logMessage = logParts.join(" ");
    const maxLength = 20000; // 扩充总长度限制到 20000 字符
    if (logMessage.length > maxLength) logMessage = `${logMessage.slice(0, 5000)}... [截断 ${logMessage.length} 字符] ...${logMessage.slice(-5000)}`;

    try {
      if (typeof logger.logTag === "function") {
        logger.logTag(level, tag || "App", logMessage);
      } else {
        const logMethod = logger[level] || console[level] || console.log;
        logMethod(logMessage);
      }
      if (trace && logger.trace) logger.trace("堆栈跟踪:\n" + (new Error().stack || "").split("\n").slice(2).join("\n"));
    } catch {
  ...
  static async mkdir(dir) {
    ...
    try {
      ...
      Bot.makeLog('error', ['创建', dir, '错误', err.message], 'BotUtil');
      return false;
    }
  }
  ...
  static async rm(file) {
    ...
    const ok = await FileUtils.rm(file);
      if (!ok) Bot.makeLog('error', ['删除', file, '错误'], 'BotUtil');
    return ok;
  }
  ...
  static async readFile(filePath, encoding = "utf8") {
    ...
    if (content === null) throw new Error(`读取文件失败: ${filePath}`);
    return content;
  }
  ...
  static async writeFile(filePath, data, opts = {}) {
    ...
    if (typeof data === 'string') {
      const ok = await FileUtils.writeFile(filePath, data, opts.encoding ?? 'utf8');
      if (!ok) throw new Error(`写入文件失败: ${filePath}`);
      return;
    }
    const ok = await FileUtils.writeFileBuffer(filePath, data);
    if (!ok) throw new Error(`写入文件失败: ${filePath}`);
  }
  ...
  static async fileExists(filePath) {
    return filePath ? FileUtils.exists(filePath) : false;
  }
  ...
  static async #getGlobLib() {
    if (!BotUtil.#globLib) {
      // package.json 已有 glob；fast-glob 为可选加速
      try {
        const { glob } = await import('glob');
        BotUtil.#globLib = glob;
      } catch (err) {
        Bot.makeLog('debug', `glob 不可用: ${err?.message || err}`, 'BotUtil');
        try {
          const { default: fastGlob } = await import('fast-glob');
          BotUtil.#globLib = fastGlob;
        } catch (err2) {
          Bot.makeLog('debug', `fast-glob 不可用: ${err2?.message || err2}`, 'BotUtil');
   ...
   * @param {string} [opts.cwd=resolveProjectPath()] - 当前工作目录（项目根）
  ...
  static async glob(pattern, opts = {}) {
    ...
    try {
      ...
      const globOptions = {
        ...
        cwd: opts.cwd || resolveProjectPath(),
        ignore: opts.ignore || [],
      ...
      Bot.makeLog('error', ["匹配", pattern, "错误", err.message], 'BotUtil');
      return [];
    }
  }
   ...
   * @param {boolean} [opts.quiet=false] - 为 true 时不记录获取失败日志（调用方有兜底时）
  ...
  static async Buffer(data, opts = {}) {
    ...
    if (BotUtil.regexCache.url.test(dataStr)) {
      ...
      try {
        ...
        if (!opts.quiet) {
          Bot.makeLog('error', ["获取 URL 内容错误", dataStr, err.message], 'BotUtil');
        }
        return Buffer.alloc(0);
      }
    }

    const inlineBinary = FileUtils.inlineBinaryFromRef(dataStr);
    if (inlineBinary) {
    ...
    if (FileUtils.isPathLike(filePath)) {
  ...
  static async #saveBufferToTempFile(buffer) {
    const tempDir = resolveProjectPath(TEMP_DIR);

    try {
      const ext = BotUtil.#extFromMagic(buffer);
      const filename = `${ulid()}${ext}`;
      const filePath = path.join(tempDir, filename);
      await FileUtils.writeFileBuffer(filePath, buffer);

      setTimeout(() => BotUtil.rm(filePath).catch((err) => {
        Bot.makeLog('debug', `[BotUtil] 临时文件清理失败: ${filePath} | ${err?.message || err}`, 'BotUtil');
      }), 60000);
  ...
  /** 按魔数补扩展名，避免 NapCat/QQ 无后缀临时文件上传超时或识别失败 */
  static #extFromMagic(buffer) {
  ...
  static async fileType(data, opts = {}) {
    ...
    try {
      ...
      Bot.makeLog('error', ["文件类型检测错误", err.message], 'BotUtil');
    }
  ...
  static async fileToUrl(file, opts = {}) {
    ...
    try {
      const mediaDir = resolveProjectPath(WWW_DIR, 'media');
      ...
      if (Buffer.isBuffer(file)) {
        ...
        if (file.startsWith("base64://")) {
          ...
          fileBuffer = await FileUtils.readFileBuffer(file);
          if (!fileBuffer) throw new Error(`读取文件失败: ${file}`);
      ...
      if (!await FileUtils.writeFileBuffer(destPath, fileBuffer)) {
        throw new Error(`写入媒体文件失败: ${destPath}`);
      }

      setTimeout(() => BotUtil.rm(destPath).catch((err) => {
        Bot.makeLog('debug', `[BotUtil] 媒体文件清理失败: ${destPath} | ${err?.message || err}`, 'BotUtil');
      }), options.time);

      const baseUrl = options.baseUrl || cfg.server.server.url || 'http://localhost:8086';
      ...
      Bot.makeLog('error', ["文件转 URL 错误", err.message], 'BotUtil');
      throw err;
    }
  }
    ...
    return new Promise((resolve) => {
      ...
      const callback = (error, stdout, stderr) => {
        ...
        if (!opts.quiet) {
          ...
          Bot.makeLog('mark', logMessage, '命令');
        }

        if (error && !opts.quiet) {
          Bot.makeLog('error', error, '命令');
        }
  ...
  static async makeChatRecord(e, messages, title, description = ["Bot 消息"]) {
    ...
    try {
        ...
        for (const msg of messages) {
          if (Array.isArray(msg)) {
            for (const part of msg) {
              if (typeof part === 'string') segments.push({ type: 'text', text: part });
              else if (part && typeof part === 'object' && (part.type || part.file)) segments.push(part);
              else if (part != null) segments.push({ type: 'text', text: String(part) });
            }
          } else if (msg && typeof msg === 'object' && (msg.type || msg.file)) {
            segments.push(msg);
          } else if (msg != null) {
            segments.push({ type: 'text', text: String(msg) });
          }
        }
        if (segments.length) {
          const desc = Array.isArray(description) ? description.join(' ') : (description ?? '');
          await e.reply({ title: title ?? '', description: desc, segments });
        }
        return true;
      }
      ...
      Bot.makeLog('error', ["创建聊天记录错误", err.message], 'BotUtil');
      try {
  ...
  static async makeMsg(e, messages, title, description) {
    ...
    try {
      ...
      await e.reply(ngm);
      Bot.makeLog('mark', `『${title || '转发消息'}』已发送`, 'BotUtil');
      return true;

    } catch (error) {
      Bot.makeLog('error', ["转发消息错误", error.message], 'BotUtil');
...
// 启动前 Bot 代理尚未创建时，保证 lib/ 内可统一调用 Bot.makeLog
if (!globalThis.Bot?.makeLog) {
  const stub = { makeLog: BotUtil.makeLog.bind(BotUtil) };
  globalThis.Bot = globalThis.Bot ? Object.assign(globalThis.Bot, stub) : stub;
}