...
export default class Bot extends EventEmitter {
  constructor() {
    ...
    // 配置属性
    this.apiKey = ''
    this._cache = BotUtil.getMap('yunzai_cache', { ttl: SERVER_CONFIG.CACHE_TTL, autoClean: true })
    this.httpPort = null
    this.httpsPort = null
  ...
  _createUinManager() {
    return Object.assign([], {
      ...
      toString(raw, ...args) {
        return raw === true ? Array.prototype.toString.apply(this, args) : this.toJSON().toString(raw, ...args)
      },
      includes(value) {
    ...
    if (cfg.server?.logging?.requests !== false) {
      this.fastify.addHook('onResponse', async (request, reply) => {
        ...
        if (!quietPaths.some((p) => request.url.startsWith(p))) {
          ...
          const statusColor = reply.statusCode < 400 ? 'green' : reply.statusCode < 500 ? 'yellow' : 'red'
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`
  ...
  async _authMiddleware(request, reply) {
    ...
    // 静态文件检查
    const isStaticFile = /\.(html|css|js|json|png|jpg|jpeg|gif|svg|webp|ico|mp4|webm|mp3|wav|pdf|zip|woff|woff2|ttf|otf)$/i.test(
      request.url
    )
  ...
  async _registerProxyRoute(domainConfig) {
    const routeOptions = {
      ...
      handler: async (request, reply) => {
        ...
        if (hostname !== domainConfig.domain && !this._matchWildcardDomain(hostname, domainConfig.domain)) {
    ...
    if (!this.wsHandlers.has(path)) {
      ...
      this.fastify.get(path, { websocket: true }, async (connection, request) => {
        ...
        connection.socket.on('message', (msg) => {
          const logMsg =
            Buffer.isBuffer(msg) && msg.length > 1024 ? `[二进制消息，长度：${msg.length}]` : BotUtil.String(msg)
  ...
  async run(options = {}) {
    ...
    if (this.wsHandlers.size > 0) {
      BotUtil.makeLog(
        'info',
        `⚡ WebSocket服务：${this.getServerUrl().replace(/^http/, 'ws')}/ [${Array.from(this.wsHandlers.keys()).join(', ')}]`,
        '服务器'
      )
    }
  ...
  get gml() {
    ...
    for (const bot_id of this.uin)
      for (const [id, i] of this.bots[bot_id].gml || []) map.set(id, Object.assign(new Map(i), { bot_id }))
    return map
  }
