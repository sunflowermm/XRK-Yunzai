...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  _checkApiAuthorization(req) {
    ...
    const authKey = req.headers?.['x-api-key'] ??
      ...
      req.body?.api_key;
    if (!authKey) {
      Bot.makeLog('debug', `[Auth] API 认证失败：缺少密钥 path=${req.path} ip=${req.ip}`, '认证');
      return false;
    }
    try {
      ...
      if (authKeyBuffer.length !== apiKeyBuffer.length) {
        Bot.makeLog('warn', `[Auth] 未授权：密钥长度不一致 path=${req.path}`, '认证');
        return false;
      }
      const ok = crypto.timingSafeEqual(authKeyBuffer, apiKeyBuffer);
      if (!ok) Bot.makeLog('debug', `[Auth] 未授权：密钥不匹配 path=${req.path} ip=${req.ip}`, '认证');
      return ok;
    } catch (error) {
      Bot.makeLog('error', `[Auth] API 认证异常：${error.message} path=${req.path}`, '认证');
      return false;
    }
  }
