...
export default class AIStream {
  ...
  resolveLLMConfig(apiConfig = {}) {
    ...
    const model = pick(apiConfig.model, this.config?.model, providerConfig.model);
  ...
  async callAI(messages, apiConfig = {}) {
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const overrides = this._buildOverridesFromConfig(config);
  ...
  async callAIStream(messages, apiConfig = {}, onDelta) {
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const overrides = this._buildOverridesFromConfig(config, { stream: true });
