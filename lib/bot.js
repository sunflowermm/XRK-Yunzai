...
export default class Bot extends EventEmitter {
  ...
  async _setupStaticServing() {
    // 启动前按 stale 构建有 sign 的静态前端（可用 XRK_SKIP_WWW_BUILD=1 跳过）
    try {
      const { buildSignedStaticWwwBeforeRuntime } = await import('./www/www-static-build.js');
      const wwwBuild = await buildSignedStaticWwwBeforeRuntime();
      if (wwwBuild.failed?.length) {
        Bot.makeLog('warn', `前端构建失败: ${wwwBuild.failed.join(', ')}（将尝试挂已有 dist）`, 'www-build');
      }
    } catch (err) {
      Bot.makeLog('warn', `前端构建阶段异常: ${err?.message || err}`, 'www-build');
    }
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async run(options = {}) {
    ...
    // 阶段2: 加载配置与工作流（插件 init 依赖 AiWorkflowLoader）
