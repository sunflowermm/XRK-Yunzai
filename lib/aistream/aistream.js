...
export default class AIStream {
  ...
  async retrieveRelevantContexts(groupId, query) {
    ...
    try {
      ...
      const scored = parsedMessages.map(data => {
        const rawScore = this.similarityCalculator.score(query, data.message) / 10;
        const age = (now - (data.time || 0)) / 3600000;
        const recencyBoost = Math.max(0, 0.15 * (1 - Math.min(age / 24, 1)));
        const similarity = rawScore + recencyBoost;
        return {
      ...
      filtered.sort((a, b) => (b.similarity - a.similarity) || ((b.time || 0) - (a.time || 0)));
  ...
  async buildEnhancedContext(e, question, baseMessages) {
    if (!this.embeddingConfig.enabled) {
      BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext 未启用 embedding 直接返回 baseMessagesLen=${baseMessages?.length ?? 0}`, 'AIStream');
      return baseMessages;
    }
    ...
    if (!query) {
      BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext 无 query 返回 baseMessagesLen=${baseMessages?.length ?? 0}`, 'AIStream');
      return baseMessages;
    }

    try {
      const relevantContexts = await this.retrieveRelevantContexts(groupId, query);
      BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext 检索 groupId=${groupId} queryLen=${(query || '').length} 相关条数=${relevantContexts?.length ?? 0}`, 'AIStream');
      ...
      const contextPrompt = [
        ...
        relevantContexts.map((ctx, i) =>
          `${i + 1}. ${ctx.message} (相关度: ${(ctx.similarity * 100).toFixed(0)}%)`
        ).join('\n'),
        '\n以上是相关历史对话，可参考但不要重复。\n'
      ].join('\n');
      BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext contextPromptLen=${contextPrompt.length} enhancedLen=${enhanced.length}`, 'AIStream');

      if (enhanced[0] && enhanced[0].role === 'system') {
        enhanced[0].content += contextPrompt;
        BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext 追加到首条 system 后 contentLen=${(enhanced[0].content || '').length}`, 'AIStream');
      } else {
        enhanced.unshift({
          role: 'system',
          content: contextPrompt
        });
        BotUtil.makeLog('debug', `[AIStream] buildEnhancedContext 插入 system 条 contentLen=${contextPrompt.length}`, 'AIStream');
      }
  ...
  _getToolStreamNames() {
    ...
    // ⚠️ 重要：如果是合并工作流，使用合并后的工作流列表
    if (this._mergedStreams && Array.isArray(this._mergedStreams)) {
      const names = this._mergedStreams.map(s => s.name);
      try {
  ...
  async callAI(messages, apiConfig = {}) {
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const overrides = this._buildOverridesFromConfig(config, { stream: false, streams: toolStreamNames });
        // streams 日志已在 _getToolStreamNames 中处理，此处不再重复打印

        const result = await client.chat(messages, overrides);
        const resultLen = result != null ? String(result).length : 0;
        BotUtil.makeLog('debug', `[AIStream] callAI 返回 attempt=${attempt} resultLen=${resultLen} result=${result ?? ''}`, 'AIStream');
        return result || null;
      } catch (error) {
        lastError = error;
