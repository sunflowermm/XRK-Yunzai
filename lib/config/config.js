...
class Cfg {
  ...
  get renderer() {
    ...
    const playwrightCfg = this._renderer.playwright || {};
    if (botCfg.chromium_path) {
      puppeteerCfg.chromiumPath ||= botCfg.chromium_path;
      playwrightCfg.chromiumPath ||= botCfg.chromium_path;
    }
    ...
    this._renderer.playwright = playwrightCfg;
   ...
   * 优先 other.masters 或 other.master 的 bot:user 列表；否则扁平 masterQQ 广播到所有 Bot.uin
   */
  get master() {
    const other = this.other || {};
    if (other.masters && typeof other.masters === 'object' && !Array.isArray(other.masters)) {
      return other.masters;
    }

    let masterEntries = other.master;
    if (masterEntries != null) {
      if (!Array.isArray(masterEntries)) masterEntries = [masterEntries];
      const parsed = {};
      for (let entry of masterEntries) {
        if (typeof entry !== 'string' || !entry.includes(':')) continue;
        const parts = entry.split(':');
        const bot_id = parts.shift();
        const user_id = parts.join(':');
        if (Array.isArray(parsed[bot_id])) parsed[bot_id].push(user_id);
        else parsed[bot_id] = [user_id];
      }
  ...
  /** ai-workflow.enabled 为 false 时 AiWorkflowLoader 不加载工作流；仍返回合并配置供 LLMFactory 等使用。 */
  get aiWorkflow() {
    return this.getMergedConfig('ai-workflow');
  }
  ...
  /**
   * 获取群组配置（兼容 getGroup(bot_id, group_id) 或 getGroup(group_id)）
   * @param {string|number} botIdOrGroupId - bot_id（两参）或 group_id（单参）
   * @param {string|number} [groupId] - 群组ID（两参模式）
   */
  getGroup(botIdOrGroupId = '', groupId) {
    const config = this.getConfig('group') || {};
    const defCfg = this.getdefSet('group') || {};
    const hasBot = groupId !== undefined && groupId !== null;
    const botId = hasBot ? String(botIdOrGroupId ?? '') : '';
    const gid = String(hasBot ? groupId : botIdOrGroupId ?? '');

    const merged = {
      ...defCfg.default,
      ...config.default,
      ...(botId ? config[`${botId}:default`] : null),
      ...(gid ? config[gid] : null),
      ...(botId && gid ? config[`${botId}:${gid}`] : null),
    };
    // groupCD ↔ groupGlobalCD 别名
    if (merged.groupCD != null && merged.groupGlobalCD == null) {
      merged.groupGlobalCD = merged.groupCD;
    }
    if (merged.groupGlobalCD != null && merged.groupCD == null) {
      merged.groupCD = merged.groupGlobalCD;
    }
    return merged;
  }

  /** 插件兼容：cfg.getOther()（含 blackUser/whiteUser 别名） */
  getOther() {
    const other = this.other || {};
    return {
      ...other,
      blackUser: other.blackUser ?? other.blackQQ,
      whiteUser: other.whiteUser ?? other.whiteQQ,
      blackQQ: other.blackQQ ?? other.blackUser,
      whiteQQ: other.whiteQQ ?? other.whiteUser,
    };
  }
  ...
  watch(file, name, key) {
    ...
    const watcher = HotReloadBase.createWatcher(file, {
      onChange: () => {
        ...
        try {
          globalThis.Bot?.emit?.('config:write', { name, source: 'watch' });
        } catch (err) {
          Bot.makeLog('debug', `config:write 派发失败 [${name}]: ${err?.message || err}`, 'Config');
        }
      }
    });
