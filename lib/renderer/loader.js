...
/**
 * 渲染器加载器
 * 支持从多个目录加载渲染器：renderers/, plugins/插件名/renderer/, core/模块名/renderer/
 */
class RendererLoader {
  constructor() {
    this.renderers = new Map();
    this.dirs = ["renderers", "plugins", "core"];
    this.watcher = {};
  }
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            const rendererCfg = FileUtils.existsSync(configPath)
              ? yaml.parse(FileUtils.readFileSync(configPath, "utf8"))
              : {}
            ...
            BotUtil.makeLog('info', `[RendererLoader] 加载渲染器: ${renderer.id} (${name})`, 'RendererLoader')
          } catch (err) {
