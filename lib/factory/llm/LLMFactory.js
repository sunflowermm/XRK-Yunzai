...
function getCompatProviderEntries() {
  ...
  for (const factory of compatFactories) {
    ...
    let providerList = Array.isArray(factoryCfg.providers) ? factoryCfg.providers : [];
    if (providerList.length === 0 && factory.configKey === 'openai_compat_llm' && (factoryCfg.baseUrl || factoryCfg.key)) {
      providerList = [{ key: 'openai_compat', ...factoryCfg }];
    }
