...
export default class Bot extends EventEmitter {
  ...
  _domainRouter = null;
  httpPort = null;  // HTTP端口
  httpsPort = null; // HTTPS端口
  ...
  setupMiddleware() {
    ...
    if (cfg.server?.security?.helmet?.enabled !== false) {
      this.express.use(helmet({
        contentSecurityPolicy: false, // 避免与nginx的CSP冲突
        crossOriginEmbedderPolicy: false,
        hsts: false // 让nginx处理HSTS
      }));
    }
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async run(options = {}) {
    ...
    // 设置端口
    this.httpPort = port;
    this.httpsPort = port + 1;  // HTTPS端口为HTTP端口+1
