...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async loadAdapters() {
    for (const absPath of PluginDirScanner.listAdapterFiles()) {
      try {
        ...
        BotUtil.makeLog('debug', `加载适配器: ${path.relative(process.cwd(), absPath)}`, 'AdapterLoader');
      } catch (err) {
  ...
  async redisExit() {
    ...
    await BotUtil.sleep(5000, redis.save().catch((err) => {
      BotUtil.makeLog('debug', `[Bot] 关闭前 redis.save 失败: ${err?.message || err}`, '服务器');
    }));
    return process.kill();
  }
