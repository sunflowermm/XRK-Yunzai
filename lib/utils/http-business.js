  ...
  _compileRules() {
    ...
    for (const rule of redirectConfig) {
      try {
        ...
        Bot.makeLog('warn', `[重定向] 规则编译失败: ${rule.from} -> ${rule.to}`, 'RedirectManager', err);
      }
    }
  ...
  check(req, res) {
    ...
    for (const rule of this.rules) {
      try {
        ...
        Bot.makeLog('warn', `[重定向] 执行失败: ${rule.from} -> ${rule.to}`, 'RedirectManager', err);
      }
    }
...
export class ProxyManager {
  ...
  markUpstreamFailure(domain, upstreamUrl) {
    ...
    if (upstream) {
      ...
      // 如果标记为不健康，记录警告
      if (!upstream.healthy) {
        Bot.makeLog('warn', `[ProxyManager] 上游服务器标记为不健康: ${domain} -> ${upstreamUrl} (失败次数: ${upstream.failCount})`, 'ProxyManager');
      }
    }
  }
  ...
  markUpstreamSuccess(domain, upstreamUrl, responseTime = 0) {
    ...
    if (upstream) {
      ...
      if (!upstream.healthy && upstream.failCount > 0) {
        ...
        Bot.makeLog('info', `[ProxyManager] 上游服务器恢复健康: ${domain} -> ${upstreamUrl}`, 'ProxyManager');
      }
    }
  }
}
