...
export default class Bot extends EventEmitter {
  ...
  makeError(message, type = 'Error', details = {}) {
    ...
    if (BotUtil && BotUtil.makeLog) {
      if (!isSilent) {
        BotUtil.makeLog(logLevel, chalk.red(`✗ ${logMessage}${logDetails}`), type);
      }
    ...
    this.express.use((req, res, next) => {
      ...
      if (req.path.startsWith(pathPrefix) && !res.headersSent) {
        try {
          ...
          BotUtil.makeLog('debug', 'UI Cookie 设置失败', 'Bot', err?.message || err);
        }
      }
      next();
    });
      ...
      res.on('finish', () => {
        for (const assetPath of assets) {
          ...
          for (const candidate of candidates) {
            try {
              if (FileUtils.statSync(candidate)?.isFile()) {
                filePath = candidate;
                break;
              }
            } catch {
  ...
  async _setupStaticServing() {
    // ========== 动态前端开发代理（sign.json 声明的 dev server，与 XRK-AGT 对齐）==========
    try {
      ...
      if (apps && apps.size > 0) {
        ...
        for (const appInfo of devApps) {
          ...
          this.express.use(mountPath, (req, res, next) => {
            BotUtil.makeLog('debug', `[前端入口] id=${cfgApp.id} mount=${mountPath} ${req.method} ${req.originalUrl}`, 'Frontend');
      ...
      try {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            ...
            // 若子目录存在 sign.json，视为动态前端工程，由 FrontendLauncher 代理，此处不挂静态
            try {
              const signStat = FileUtils.statSync(path.join(subDirPath, 'sign.json'));
              if (signStat?.isFile()) {
                BotUtil.makeLog('info', `检测到前端 sign.json，跳过子目录静态挂载: ${mountPath} (plugin: ${pluginName})`, 'Bot');
                continue;
              }
            } catch (err) {
              BotUtil.makeLog('debug', `sign.json 检查跳过: ${mountPath}`, 'Bot', err?.message || err);
            }
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
    ...
    try {
      ...
      if (keyStat?.isFile()) {
        ...
        if (keyContent) {
          ...
          BotUtil.makeLog('debug', 'API密钥文件格式无效，将生成新密钥', '服务器');
        }
      }
    } catch (err) {
      BotUtil.makeLog('debug', 'API密钥文件不存在或读取失败，将生成新密钥', '服务器', err?.message || err);
    }
  ...
  _isPathWhitelisted(pathName, whitelist) {
    ...
    const list = this._isApiPath(p) ? (whitelist || []).filter(w => w && w.startsWith(AUTH_API_PREFIX)) : (whitelist || []);
  ...
  _authMiddleware(req, res, next) {
    ...
    if (this._isLoopbackConnection(req.ip)) {
      return next();
    }
    ...
    if (uiCookieCfg.enabled === true && uiCookieCfg.allowPublicSameOrigin === true) {
      try {
        ...
        BotUtil.makeLog('debug', '同源 UI Cookie 鉴权检查失败', 'Bot', err?.message || err);
      }
    }
  ...
  /**
   * 检查API授权（与 XRK-AGT 对齐：认证启用但密钥未加载时一律拒绝）
   */
  _checkApiAuthorization(req) {
    if (!req) return false;
    const authConfig = cfg.server.auth?.apiKey || {};
    ...
    try {
      ...
      if (authKeyBuffer.length !== apiKeyBuffer.length) {
        BotUtil.makeLog('warn', `[Auth] 未授权：密钥长度不一致 path=${req.path}`, '认证');
        return false;
      }
  ...
  async _shutdownHandler(req, res) {
    ...
    setImmediate(async () => {
      try {
        ...
        BotUtil.makeLog('error', `关闭服务器失败: ${e.message}`, '服务器');
        process.exit(1);
      }
    });
  }
  ...
  wsConnect(req, socket, head) {
    ...
    if (!isWhitelisted && !this._isLoopbackConnection(req.socket.remoteAddress)) {
  ...
  async serverLoad(isHttps) {
    ...
    await BotUtil.promiseEvent(server, "listening", isHttps && "error").catch((err) => {
      BotUtil.makeLog('debug', `[Bot] ${isHttps ? 'HTTPS' : 'HTTP'} listening 等待失败: ${err?.message || err}`, '服务器');
    });
  ...
  async loadAdapters() {
    for (const absPath of PluginDirScanner.listAdapterFiles()) {
      try {
        ...
        BotUtil.makeLog('debug', `加载适配器: ${path.relative(resolveProjectPath(), absPath)}`, 'AdapterLoader');
      } catch (err) {
        BotUtil.makeLog('error', `适配器文件加载失败: ${absPath} - ${err.message}`, 'AdapterLoader', err);
      }
    }
  }
  ...
  async startProxyServers() {
    ...
    await BotUtil.promiseEvent(this.proxyServer, "listening").catch((err) => {
      BotUtil.makeLog('debug', `[Bot] 代理 HTTP listening 等待失败: ${err?.message || err}`, '服务器');
    });
    ...
    if (this.proxyHttpsServer) {
      ...
      await BotUtil.promiseEvent(this.proxyHttpsServer, "listening").catch((err) => {
        BotUtil.makeLog('debug', `[Bot] 代理 HTTPS listening 等待失败: ${err?.message || err}`, '服务器');
      });
  ...
  async getLocalIpAddress() {
    ...
    try {
      ...
      try {
        ...
        BotUtil.makeLog('debug', `[Bot] UDP 本机 IP 探测失败: ${err?.message || err}`, '服务器');
      }
  ...
  async run(options = {}) {
    ...
    // 阶段2: 加载配置与工作流（插件 init 依赖 StreamLoader）
    this.StreamLoader = StreamLoader;
    this.PluginsLoader = PluginsLoader;

    const [configResult, streamResult] = await Promise.allSettled([
      ConfigLoader.load(),
      StreamLoader.load()
    ]);
    ...
    if (configResult.status === 'fulfilled') {
      ...
      BotUtil.makeLog('error', `配置加载失败: ${reason?.message || '未知错误'}`, '服务器');
    ...
    if (pluginsResult.status === 'rejected') {
      ...
      BotUtil.makeLog('error', `插件加载失败: ${reason?.message || '未知错误'}`, '服务器');
    ...
    await StreamLoader.watch(fileWatch);
  ...
  pickFriend(user_id, strict) {
    ...
    BotUtil.makeLog("trace", `用户 ${user_id} 不存在，回退主 Bot ${botId ?? 'unknown'}`, '服务器');
  ...
  pickGroup(group_id, strict) {
    ...
    const botId = this._getMainBotId();
    BotUtil.makeLog("trace", `群组 ${group_id} 不存在，回退主 Bot ${botId ?? 'unknown'}`, '服务器');
    const bot = botId != null ? this.bots[botId] : undefined;
  ...
  async redisExit() {
    await persistRedis().catch((err) => {
      BotUtil.makeLog('debug', `[Bot] 关闭前 redis SAVE 失败: ${err?.message || err}`, 'Server');
    });
    await closeRedis().catch((err) => {
      BotUtil.makeLog('debug', `[Bot] 关闭 redis 连接失败: ${err?.message || err}`, 'Server');
    });
    return false;
  }
