...
function getMainPort() {
  try {
    ...
    BotUtil.makeLog('debug', `[getMainPort] ${err?.message || err}`, 'FrontendLauncher');
  }
  return 8086;
}
...
function getMainOrigin() {
  try {
    ...
    BotUtil.makeLog('debug', `[getMainOrigin] ${err?.message || err}`, 'FrontendLauncher');
  }
...
async function discoverConfigs() {
  ...
  for (const file of files) {
    try {
      ...
      if (!raw) {
        BotUtil.makeLog('debug', `sign.json 读取失败: ${file}`, 'Frontend');
        continue;
      }
      const json = tryParseJson(raw);
      if (!json) {
        BotUtil.makeLog('debug', `sign.json 解析失败: ${file}`, 'Frontend');
        continue;
      }
