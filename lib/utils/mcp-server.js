...
function valueMatchesJsonSchemaType(value, expectedType) {
  switch (expectedType) {
    case 'integer':
...
export class MCPServer {
  ...
  validateArguments(args, schema) {
    ...
    for (const [key, value] of Object.entries(args)) {
      ...
      const types = jsonSchemaTypes(propSchema);
      if (!types.length) continue;
      if (types.some((t) => valueMatchesJsonSchemaType(value, t))) continue;
      throw new Error(`参数 ${key} 类型不匹配: 期望 ${types.join('|')}, 实际 ${describeValueType(value)}`);
    }
  }
  ...
  /** 获取工具列表。streamName 为空时排除 chat；否则只返回该工作流前缀的工具 */
