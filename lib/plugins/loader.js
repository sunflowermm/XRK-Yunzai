 ...
 *
 * [ext] XRK 扩展：
 *   device / stdin · extended · bypassThrottle · eventSubscribe
...
/** [compat] message/notice/request；[ext] device */
const EVENT_MAP = {
...
class PluginsLoader {
        ...
        await Promise.allSettled(
          batch.map(async (file) => {
            ...
            try {
              ...
              this.load_time[file.name] = Date.now() - pluginStartTime;
            } catch (err) {
  ...
  /** [compat] 事件入口；[ext] device/stdin 早退分支 */
  async deal(e) {
    try {
      if (!e) return
      this.initEvent(e)
  ...
  /** [compat] CD / 节流检查；[ext] deviceCD / bypass */
  checkLimit(e) {
    // 特殊事件直接通过
    if (e.isDevice || e.isStdin) return true
  ...
  /** [ext] 事件订阅（system-plugin connect 等） */
