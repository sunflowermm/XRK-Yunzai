/**
 * 插件侧按端口配置与监听（makeConfig + watcher）
 * 当前无引用，保留供插件扩展使用；主配置请使用 lib/config/config.js 的 cfg
 */
...
export default async function makeConfig(name, config = {}, keep = {}, opts = {}) {
  ...
  const serverConfigDir = path.join(cfg.PATHS.SERVER_BOTS, String(port))
  await FileUtils.ensureDir(serverConfigDir)
  const configFile = path.join(serverConfigDir, `${name}.yaml`)
  const defaultConfigFile = path.join(cfg.cfg.PATHS.DEFAULT_CONFIG, `${name}.yaml`)

  // 配置保存函数
  const configSave = util.debounce(typeof opts.replacer === "function" ?
    async () => {
      await FileUtils.ensureDir(path.dirname(configFile))
  ...
  try {
    ...
    // 如果默认配置不存在，尝试创建
    try {
      await FileUtils.ensureDir(path.dirname(defaultConfigFile))
...
export async function initServerConfigs(port) {
  ...
  try {
    const files = await fs.readdir(cfg.PATHS.DEFAULT_CONFIG)
    
    for (const file of files) {
      if (file.endsWith('.yaml')) {
        const defaultFile = path.join(cfg.PATHS.DEFAULT_CONFIG, file)
