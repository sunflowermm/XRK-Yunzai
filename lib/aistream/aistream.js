...
export default class AIStream {
   ...
   */
  _getDefaultProvider() {
    return LLMFactory.resolveProvider({}) || 'gptgod';
  }
  ...
  async callAI(messages, apiConfig = {}) {
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const text = typeof raw === 'object' && raw != null && raw.content !== undefined ? raw.content : raw;
        const resultLen = text != null ? String(text).length : 0;
        BotUtil.makeLog('debug', `[AIStream] callAI 返回 attempt=${attempt} resultLen=${resultLen} result=${text ?? ''}`, 'AIStream');
