...
export default class StreamBase {
  ...
  _compilePattern(pattern) {
    ...
    // 只转换自定义的命名捕获组语法，不转换正则表达式量词
    // {name} -> 命名捕获组（必须是字母开头的标识符）
    regStr = regStr.replace(/\{([a-zA-Z]\w*)\}/g, '(?<$1>[^\\s]+)');
    
    // [content] -> 懒惰匹配的命名捕获组
    regStr = regStr.replace(/\[([a-zA-Z]\w*)\]/g, '(?<$1>.+?)');
  ...
  async _matchRule(rule, response, context) {
    ...
    try {
      ...
      if (this.config.debug) {
        logger?.error(`[Stream ${this.name}] 规则 ${rule.name} 匹配错误:`, error.message);
      }
    }
  ...
  cleanResponse(response, results) {
    ...
    const sortedResults = results
      .filter(r => r.match && typeof r.match === 'string' && typeof r.index === 'number')
      ...
      try {
        ...
        if (metadata?.errorHandler) {
          try {
            ...
            if (this.config.debug) {
              logger?.error(`[Stream ${this.name}] 错误处理器失败:`, handlerError.message);
            }
          }
        }
