...
export default class BotUtil {
  ...
  static makeLogID(id) {
    ...
    if (!isTruncated && leftPadCount > 0) {
      ...
      result += BotUtil.#applyGradientToString(leftPadding, idColors, true);
    }

    // 主内容渐变
    const mainContent = isTruncated ? fullContent : idStr;
    result += BotUtil.#applyGradientToString(mainContent, idColors, false);

    if (!isTruncated && rightPadCount > 0) {
      // 右填充渐变
      const rightPadding = filler.repeat(rightPadCount);
      result += BotUtil.#applyGradientToString(rightPadding, idColors, true);
    }
   ...
   */
  static #applyGradientToString(text, colors, lightMode = false) {
