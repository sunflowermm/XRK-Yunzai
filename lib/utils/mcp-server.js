...
export class MCPServer {
  ...
  _textContent(obj, isError = false) {
    const text = JSON.stringify(obj != null ? obj : { success: true }, null, 2);
  ...
  _toolResultToText(result) {
    if (result == null) {
      BotUtil.makeLog('debug', '[MCP] _toolResultToText result=null → 已执行', 'MCPServer');
      return '已执行';
    }
    ...
    if (result.success === false) {
      ...
      BotUtil.makeLog('debug', `[MCP] _toolResultToText success=false → ${out}`, 'MCPServer');
      return out;
    }
    if (typeof result.raw === 'string' && result.raw.trim()) {
      BotUtil.makeLog('debug', `[MCP] _toolResultToText 使用 raw len=${result.raw.length}`, 'MCPServer');
    ...
    try {
      ...
      if (str && str !== '{}') {
        BotUtil.makeLog('debug', `[MCP] _toolResultToText 兜底 JSON.stringify len=${str.length}`, 'MCPServer');
        return str;
      }
    } catch (err) {
      BotUtil.makeLog('debug', `[MCP] _toolResultToText stringify failed: ${err?.message || err}`, 'MCPServer');
    }
    BotUtil.makeLog('debug', '[MCP] _toolResultToText 无 msg/raw/data → 已执行', 'MCPServer');
    return '已执行';
  }
  ...
  async handleToolCall(request) {
    ...
    BotUtil.makeLog('info', `[MCP] handleToolCall 入口 ${name} args=${argsStr.slice(0, 2000)}`, 'MCP');

    if (!this.tools.has(name)) {
      BotUtil.makeLog('warn', `[MCP] handleToolCall 工具未找到: ${name}`, 'MCP');
    ...
    try {
      ...
      BotUtil.makeLog('debug', `[MCP] handleToolCall 执行 handler ${name}`, 'MCP');
      const result = await tool.handler(args || {});
      BotUtil.makeLog('debug', `[MCP] handleToolCall handler 返回 ${name} hasResult=${!!result} success=${result?.success} hasContent=${!!result?.content} hasRaw=${!!result?.raw}`, 'MCP');

      if (result && typeof result === 'object' && Array.isArray(result.content)) {
        BotUtil.makeLog('debug', `[MCP] handleToolCall 直接返回 content 数组 len=${result.content?.length}`, 'MCP');
        return { content: result.content, isError: result.isError || false };
      }
      const isError = result && typeof result === 'object' && result.success === false;
      const text = this._toolResultToText(result);
      BotUtil.makeLog('debug', `[MCP] handleToolCall 返回 ${name} textLen=${text?.length ?? 0} isError=${isError} 前1000字=${(text || '').slice(0, 1000)}`, 'MCP');
      return { content: [{ type: 'text', text }], isError };
    } catch (error) {
      BotUtil.makeLog('error', `[MCP] handleToolCall 失败 ${name}: ${error.message}`, 'MCP');
  ...
  listTools(streamName = null, logEnabled = true) {
    ...
    if (logEnabled) {
      ...
      if (cached !== toolNames) {
        ...
        BotUtil.makeLog('debug', `[MCPServer] listTools streamName=${cacheKey} 工具数=${result.length}`, 'MCPServer');
      }
    }
