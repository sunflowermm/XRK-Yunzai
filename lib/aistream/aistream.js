...
export default class AIStream {
  ...
  async callAI(messages, apiConfig = {}) {
    ...
    try {
      const model = userConfig?.model || userConfig?.chatModel || config.model || config.chatModel || 'gpt-3.5-turbo';
      ...
      const response = await fetch(`${config.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          top_p: userConfig.topP ?? userConfig.top_p ?? config.topP,
          presence_penalty: userConfig.presencePenalty ?? userConfig.presence_penalty ?? config.presencePenalty,
          frequency_penalty: userConfig.frequencyPenalty ?? userConfig.frequency_penalty ?? config.frequencyPenalty,
          stream: false
        }),
