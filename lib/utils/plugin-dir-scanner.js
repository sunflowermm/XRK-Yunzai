...
export class PluginDirScanner {
  ...
  static scanSubdirs(subdir, options = {}) {
    const pluginsRoot = path.resolve(options.pluginsRoot ?? path.join(process.cwd(), 'plugins'));
    const results = [];
  ...
  /**
   * 工作流目录：plugins/<名>/stream/ 与 plugins/<名>/streams/（单次扫描）
   * @param {string} [pluginsRoot]
   * @returns {string[]}
   */
  static listStreamDirs(pluginsRoot) {
    const root = path.resolve(pluginsRoot ?? path.join(process.cwd(), 'plugins'));
    if (!FileUtils.existsSync(root)) return [];
    const dirs = [];
    for (const entry of FileUtils.readDirSync(root, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.')) continue;
      for (const sub of ['stream', 'streams']) {
        const dir = path.resolve(root, entry.name, sub);
  ...
  static getSharedSubdir(subdir, pluginsRoot) {
    const dir = path.resolve(pluginsRoot ?? path.join(process.cwd(), 'plugins'), subdir);
   ...
   */
  static listRendererEntries(cwd = process.cwd()) {
    const results = [];
    const renderersRoot = path.resolve(cwd, 'renderers');
    ...
    for (const pluginRoot of this.listPluginRoots(path.join(cwd, 'plugins'))) {
