...
class PluginsLoader {
  ...
  async processPlugins(plugins, e, isExtended) {
    ...
    if (!Array.isArray(plugins)) {
      Bot.makeLog('error', 'processPlugins: plugins参数不是数组', 'PluginsLoader')
      return false
    }
  ...
  async processRules(plugins, e) {
    // 确保plugins是数组
    if (!Array.isArray(plugins)) {
      Bot.makeLog('error', 'processRules: plugins参数不是数组', 'PluginsLoader')
      return false
    }
  ...
  async processDefaultHandlers(e) {
    ...
    for (const handler of this.defaultMsgHandlers) {
      try {
        ...
        Bot.makeLog('error', `默认消息处理器 ${handler.name} 执行错误`, 'PluginsLoader', error)
      }
    }
    return false
  }
  ...
  addUtilMethods(e) {
    ...
    e.getSendableMedia = async (media) => {
      ...
      try {
        ...
        Bot.makeLog('error', `处理媒体文件失败: ${error.message}`, 'PluginsLoader')
      }
      return null
    }
  ...
  checkBlack(e) {
    ...
    // QQ黑名单（TRSS blackUser / XRK blackQQ）
    ...
    // QQ白名单（TRSS whiteUser / XRK whiteQQ）
  ...
  async emit(eventType, eventData) {
    try {
      ...
      Bot.makeLog('error', '触发自定义事件失败', 'PluginsLoader', error)
