...
export default class BotUtil {
  ...
  static #applyGradientToString(text, colors, chalk) {
    if (!text || !colors || colors.length === 0) return text;

    let result = '';
    const chars = Array.from(text);
    const step = Math.max(1, Math.ceil(chars.length / colors.length));

    for (let i = 0; i < chars.length; i++) {
      const colorIndex = Math.min(Math.floor(i / step), colors.length - 1);
      result += chalk.hex(colors[colorIndex])(chars[i]);
    }
  ...
  static makeLogID(id) {
    ...
    if (displayWidth >= targetLength) {
      ...
      for (const char of idStr) {
        ...
        if (currentWidth + charWidth > targetLength - 2) break;
        truncated += char;
      ...
      formattedContent = filler.repeat(leftPad) + idStr + filler.repeat(rightPad);
    }

    if (!chalk) {
      return BotUtil.cache(cacheKey, `[${formattedContent}]`, 60000);
    }
    ...
    if (displayWidth < targetLength) {
      ...
      const lightColors = idColors.map(c => BotUtil.#lightenColor(c, 0.6));

      if (leftPad > 0) {
        const leftFiller = filler.repeat(leftPad);
        result += BotUtil.#applyGradientToString(leftFiller, lightColors, chalk);
      }

      result += BotUtil.#applyGradientToString(idStr, idColors, chalk);

      if (rightPad > 0) {
        const rightFiller = filler.repeat(rightPad);
        result += BotUtil.#applyGradientToString(rightFiller, lightColors, chalk);
      }
    } else {
      result += BotUtil.#applyGradientToString(formattedContent, idColors, chalk);
    }
