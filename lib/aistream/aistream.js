...
import { normalizeLlmClientChatReturn } from '../utils/llm/llm-chat-result.js';
...
export default class AIStream {
   ...
   * @returns {Promise<string|null>} AI回复文本（本轮若已用 reply 工具，会置 `this._lastCallAIUsedReplyTool`）
   */
  async callAI(messages, apiConfig = {}) {
    ...
    this._lastCallAIUsedReplyTool = false;
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        const { text, usedReplyTool } = normalizeLlmClientChatReturn(raw);
        this._lastCallAIUsedReplyTool = usedReplyTool;
  ...
  async callAIStream(messages, apiConfig = {}, onDelta) {
    ...
    if (config.enableStream === false) {
      const text = await this.callAI(messages, apiConfig);
