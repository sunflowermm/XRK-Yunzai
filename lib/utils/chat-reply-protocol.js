...
/** 把 [CQ:at,qq=…] 收成 [at:QQ]，避免模型被群友教错协议后 @ 失效 */
...
/**
 * 把 at QQ 列表前置成 [at:QQ]（已存在则不重复）。AI 优先用工具参数 at / atSender，少在正文手写标记。
...
/** reply content 校验（先 normalizeReplyAtMarkers 再调） */
 ...
 * fallbackReplyId 仅在调用方显式传入时作为引用兜底；默认不引用。
 */
