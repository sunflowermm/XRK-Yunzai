...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  wsConnect(req, socket, head) {
    ...
    const pathSegments = urlPathNormalized.split("/").filter(Boolean); // 获取所有路径段
    const firstSegment = pathSegments[0] || ''; // 获取第一个路径段
    ...
    // 方式2: 第一个路径段匹配（如 device）
    else if (firstSegment && firstSegment in this.wsf) {
      matchedPath = firstSegment;
      matchedHandlers = this.wsf[firstSegment];
    }
    // 方式3: 尝试不带斜杠的完整路径
    else if (urlPathNormalized.slice(1) in this.wsf) {
      matchedPath = urlPathNormalized.slice(1);
      matchedHandlers = this.wsf[urlPathNormalized.slice(1)];
    }
    
    if (!matchedPath || !matchedHandlers) {
      BotUtil.makeLog("warn", `WebSocket路径未找到: ${req.url}`, '服务器');
      BotUtil.makeLog("debug", `尝试匹配: 完整路径="${urlPathNormalized}", 第一段="${firstSegment}"`, '服务器');
