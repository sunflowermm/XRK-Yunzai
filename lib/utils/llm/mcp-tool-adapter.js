...
export class MCPToolAdapter {
   ...
   * @param {Array<string>} [streams] - 工作流名称白名单，仅保留这些工作流下的工具；不传或空数组表示不过滤
  ...
  static convertMCPToolsToOpenAI(streams = []) {
    ...
    let mcpTools = mcpServer.listTools?.() || [];
    if (Array.isArray(streams) && streams.length > 0) {
      const set = new Set(streams.map(s => String(s).toLowerCase()));
      mcpTools = mcpTools.filter(tool => {
        const name = tool.name || '';
        const streamPart = name.includes('.') ? name.split('.')[0].toLowerCase() : 'mcp';
        return set.has(streamPart) || set.has(name.toLowerCase());
      });
    }
