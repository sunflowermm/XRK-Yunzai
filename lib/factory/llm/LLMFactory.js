...
function readFactoryCfg(configKey) {
  ...
  const raw = cfg.getConfig(configKey) || {};
...
export default class LLMFactory {
  ...
  static getProviderConfig(providerName) {
    ...
    const { chatModel, azureDeployment, provider: providerAlias, ...entryRest } = entry;
    return {
      ...entryRest,
      model: entryRest.model || chatModel || null,
      deployment: entryRest.deployment || azureDeployment || null,
      provider: key,
      protocol,
      factoryType: factory.factoryType,
      factory: resolveFactoryId(factory.configKey),
